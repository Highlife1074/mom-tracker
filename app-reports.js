...
// Sections of the procurement report. "status" is always printed; the rest can be
// switched off from the dialog behind the 📑 button (ProcReportButton).
var PROC_REPORT_SECTIONS=[
  {key:"awards", label:"Awards & contract conditions"},
  {key:"actions",label:"Open actions"},
  {key:"mar",    label:"MAR list (MSS + approval)"},
  {key:"wmsItp", label:"WMS / ITP list"}
];
var PROC_REPORT_DEFAULTS={awards:true,actions:false,mar:true,wmsItp:true};

function buildProcurementReport(tenders,tasks,packages,opts){
  // No opts = the historical full report, so any other caller keeps its behaviour.
  var o=Object.assign({awards:true,actions:true,mar:true,wmsItp:true},opts||{});
  var t=today();
  ...
  var accApproved=tds.filter(function(x){return isApprovedStatus(((x.stepDates||{}).acc||{}).approvalStatus);}).length;
  var signedCount=tds.filter(function(x){var ct=(x.stepDates||{}).contract||{};return !!(ct.signedDone||ct.done);}).length;
  var omitted=PROC_REPORT_SECTIONS.filter(function(s){return !o[s.key];}).map(function(s){return s.label;});

  var html='<h1>Procurement Report</h1><div class="sub">Riviera Tower · generated '+esc(fmtDate(t))+' · '+tds.length+' tenders across '+pkgs.length+' packages'+
    (omitted.length?' · not included: '+esc(omitted.join(", ")):'')+'</div>';
  html+='<div class="kpi">'+
    (o.actions
      ...
      :'<div><b>'+signedCount+'</b><span>contracts signed</span></div>'+
       '<div><b>'+(tds.length-signedCount)+'</b><span>not signed</span></div>')+
    '<div><b>'+accApproved+'</b><span>ACC approved</span></div>'+
    '</div>'+
  ...
  pkgs.forEach(function(p){
    ...
    // --- awards: one line per subcontractor retained, with its conditions and bonds
    var aws=[];
    list.forEach(function(td){(td.awards||[]).forEach(function(a){aws.push({td:td,a:normAward(a)});});});
    if(o.awards&&aws.length){
      html+='<h3>Awards &amp; contract conditions ('+aws.length+')</h3><table><tr><th>Tender</th><th>Subcontractor</th><th>Status</th>'+
        '<th>Lump sum</th><th>Advance</th><th>Retention</th><th>Delay damages</th><th>Contract period</th><th>Bonds</th></tr>';
      aws.forEach(function(x){
        var a=x.a,f=a.fin,st=awardStatus(a.status);
        var stCls=a.status==="signed"?"grn":a.status==="awarded"?"blu":"gry";
        var alerts=awardAlerts(a,x.td.startOnSite||"",t);
        var red=alerts.some(function(z){return z.lvl==="red";});
        function bond(name,on,amt,tr){
          if(!on)return '';
          var cls=tr.status==="received"?"grn":tr.status==="released"?"gry":"yel";
          var exp=tr.status==="received"&&tr.expiry?' · exp. '+esc(fmtDate(tr.expiry)):'';
          if(tr.status==="received"&&tr.expiry){var d=awardDaysBetween(t,tr.expiry);if(d<0)cls="red";else if(d<=AWARD_BOND_WARN_DAYS)cls="amb";}
          return '<div><span class="pill '+cls+'">'+esc(name)+' '+esc(tr.status||"pending")+exp+'</span>'+(isNaN(amt)?'':' '+esc(momEur(amt)))+'</div>';
        }
        var advOn=a.advBond.required==="yes"||(a.advBond.required===""&&momNum(f.advance)>0);
        var advPct=momPct(f.advance,f.lumpSum);
        html+='<tr'+(red?' style="background:#fdeced"':'')+'><td>'+esc(x.td.title)+'</td>'+
          '<td><b>'+esc(a.subcontractor||"—")+'</b>'+(a.scope?'<div style="font-size:10px;color:#8b8578">'+esc(a.scope)+'</div>':'')+'</td>'+
          '<td><span class="pill '+stCls+'">'+esc(st.label)+'</span></td>'+
          '<td>'+(esc(momEur(f.lumpSum))||'—')+'</td>'+
          '<td>'+(momEur(f.advance)?esc(momEur(f.advance))+(advPct?' ('+esc(advPct)+')':''):'—')+'</td>'+
          '<td>'+(esc(String(f.retention||"").trim())||'—')+'</td>'+
          '<td>'+(momEur(f.delayDamages)?esc(momEur(f.delayDamages))+'/day':'—')+'</td>'+
          '<td>'+(a.contractStart||a.contractEnd?esc(fmtDate(a.contractStart)||"?")+' → '+esc(fmtDate(a.contractEnd)||"?"):'—')+'</td>'+
          '<td>'+(bond("PB",f.perfBond==="yes",awardPerfBondAmount(a),a.perfBondTrack)+bond("AP",advOn,awardAdvBondAmount(a),a.advBond)||'—')+'</td></tr>';
      });
      html+='</table>';
    }
...
// Last choice made in the dialog, kept for the session only (never written to Firebase).
var _procReportOpts=null;

// The 📑 button of TendersView and its options dialog. Self-contained so that TendersView
// gets no new hook (it has early returns).
function ProcReportButton({tenders,tasks,packages}){
  const [open,setOpen]=useState(false);
  const [opts,setOpts]=useState(function(){return Object.assign({},PROC_REPORT_DEFAULTS,_procReportOpts||{});});
  function run(){
    _procReportOpts=Object.assign({},opts);
    openReport(opts.actions?"Procurement Report":"Procurement Report - status",
      buildProcurementReport(tenders,tasks,packages,opts));
    setOpen(false);
  }
  return <React.Fragment>
    <button className="btn btn-gold" title="One printable PDF per package: procurement status, and optionally awards, open actions, MAR and WMS/ITP"
      onClick={function(){setOpen(true);}}>📑 Procurement report</button>
    {open&&<div className="overlay" onClick={function(e){if(e.target===e.currentTarget)setOpen(false);}}>
      <div className="modal" style={{maxWidth:420}}>
        <div className="modal-hdr">
          <div className="modal-title">📑 Procurement report</div>
          <button onClick={function(){setOpen(false);}} style={{background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#bbb"}}>×</button>
        </div>
        <div className="modal-body">
          <div style={{fontSize:10,fontWeight:700,color:"#888",textTransform:"uppercase",letterSpacing:".06em",marginBottom:8}}>Include</div>
          <label style={{display:"flex",alignItems:"center",gap:8,fontSize:12,fontWeight:600,textTransform:"none",letterSpacing:"normal",margin:"0 0 8px",opacity:.6}}>
            <input type="checkbox" checked disabled style={{width:13,height:13}}/>
            Procurement status <span style={{fontWeight:400,color:"#888"}}>· always included</span>
          </label>
          {PROC_REPORT_SECTIONS.map(function(sec){
            return <label key={sec.key} style={{display:"flex",alignItems:"center",gap:8,fontSize:12,fontWeight:600,textTransform:"none",letterSpacing:"normal",margin:"0 0 8px",cursor:"pointer"}}>
              <input type="checkbox" checked={!!opts[sec.key]} style={{width:13,height:13}}
                onChange={function(e){var n=Object.assign({},opts);n[sec.key]=e.target.checked;setOpts(n);}}/>
              {sec.label}
            </label>;
          })}
        </div>
        <div className="modal-footer">
          <button className="btn" onClick={function(){setOpen(false);}}>Cancel</button>
          <button className="btn btn-gold" onClick={run}>Open report</button>
        </div>
      </div>
    </div>}
  </React.Fragment>;
...
const MOM_SECTIONS=["General organisation / Resources","Administrative","Financial","Health and Safety","Design","Program","Quality","Others"];
// Mid and Final tender reviews share one template; only the title changes (reviewStage).
const MOM_TR_SECTIONS=["Program","Scope of works","Specs","Responsibility matrix","Price and financial conditions","Resources","Others"];
const MOM_FIN_SECTION="Price and financial conditions";
const MOM_FIN_ASSESSMENT="Interim payment intervals shall be monthly up to and including the month following the date of Completion of the Subcontract works.";
function newMomItem(o){return Object.assign({id:uuid(),ref:"",notes:"",actionWith:"",dueBy:"",status:"",taskId:""},o||{});}
function newMomFin(o){
  return Object.assign({lumpSum:"",fixedPrice:"",fixedUntil:"Completion of the main contract works",
    advance:"",advanceNote:"",perfBond:"",perfBondPct:"7.5",delayDamages:"",defectsMonths:"",
    assessment:MOM_FIN_ASSESSMENT,retention:"5%",mos:"",mosAmount:""},o||{});
}
// Section numbers follow the order on screen; recomputed after every add / move / delete.
function reindexMomSections(secs){return (secs||[]).map(function(s,i){return Object.assign({},s,{index:String(i+1)});});}
function isTenderReview(m){return !!m&&m.momType==="tender-review";}
function momTitle(m){return isTenderReview(m)?(m.reviewStage==="final"?"Final":"Mid")+" Tender Review":"Minutes of Meeting";}
function newMom(o){
...
function newTenderReview(o){
  return newMom(Object.assign({
    momType:"tender-review",reviewStage:"mid",
    sections:MOM_TR_SECTIONS.map(function(t,i){
      var s={id:uuid(),index:String(i+1),title:t,items:[newMomItem()]};
      // The price section carries a fixed two-column table; its items are optional comments.
      if(t===MOM_FIN_SECTION){s.kind="finance";s.fin=newMomFin();s.items=[];}
      return s;
    })
  },o||{}));
}
// Amounts are typed as plain numbers; "1 250 000,50" is also accepted.
function momNum(v){
  if(v===""||v===null||v===undefined)return NaN;
  var n=typeof v==="number"?v:parseFloat(String(v).replace(/\s/g,"").replace(",","."));
  return isFinite(n)?n:NaN;
}
function momEur(v){var n=momNum(v);return isNaN(n)?"":"€ "+n.toLocaleString("en-GB",{minimumFractionDigits:2,maximumFractionDigits:2});}
function momPct(part,whole){
  var a=momNum(part),b=momNum(whole);
  if(isNaN(a)||isNaN(b)||b<=0)return"";
  return (a/b*100).toFixed(2).replace(/\.?0+$/,"")+"%";
}
// One definition of the table, read by both the editor and the PDF, so they never drift.
// `value` is plain text: the PDF escapes it.
function momFinRows(fin){
  var f=newMomFin(fin);
  function yn(v){return v==="yes"?"Yes":v==="no"?"No":"";}
  var pct=String(f.perfBondPct||"7.5").trim();
  var bondAmt=(!isNaN(momNum(f.lumpSum))&&!isNaN(momNum(pct)))?momEur(momNum(f.lumpSum)*momNum(pct)/100):"";
  var advPct=momPct(f.advance,f.lumpSum);
  return [
    {key:"lumpSum",label:"Lump sum",value:momEur(f.lumpSum)},
    {key:"fixedPrice",label:"Fixed price",
      value:yn(f.fixedPrice)+(f.fixedPrice==="yes"&&String(f.fixedUntil||"").trim()?" – until "+f.fixedUntil.trim():"")},
    {key:"advance",label:"Advance payment required",note:"Amount and % of the Subcontract Price, subject to a bond",
      value:[momEur(f.advance)+(advPct?" ("+advPct+" of the Subcontract Price)":""),String(f.advanceNote||"").trim()].filter(Boolean).join(" – ")},
    {key:"perfBond",label:"Performance bond "+pct+"%",
      note:"If a Performance Bond is required this shall be provided by the Tenderer in the approved form as per the Subcontract conditions, this requirement is a condition precedent to any payment under the Subcontract. The Performance Security Bond shall be "+pct+"% of the Subcontract Price.",
      value:yn(f.perfBond)+(f.perfBond==="yes"?" – "+pct+"% of the Subcontract Price"+(bondAmt?" = "+bondAmt:""):"")},
    {key:"delayDamages",label:"Delay damages (€/day)",note:"Under the Subcontract (daily DDs for delays in Key Milestones and End/Completion of Subcontract Works)",
      value:momEur(f.delayDamages)?momEur(f.delayDamages)+" per day":""},
    {key:"defectsMonths",label:"Defects correction period",note:"In months after the Completion of the whole of the Subcontract works",
      value:String(f.defectsMonths||"").trim()?String(f.defectsMonths).trim()+" months after Completion of the Subcontract works":""},
    {key:"assessment",label:"Assessment and payment dates",value:String(f.assessment||"").trim()},
    {key:"retention",label:"Retention percentage",value:String(f.retention||"").trim()},
    {key:"mos",label:"Payment for materials on site",
      value:yn(f.mos)+(f.mos==="yes"&&momEur(f.mosAmount)?" – "+momEur(f.mosAmount):"")}
  ];
}
// "MOM-014 - ACME Construction - 2026-08-19"
// "MOM-015 - ACME Construction - Final Tender Review - 2026-09-18"
function momFileName(m){
  ...
  var dt=(m.date||today());
  return num+" - "+sub+(isTenderReview(m)?" - "+momTitle(m):"")+" - "+dt;
}
...
td.fin-cell{padding:6px 0 8px!important;background:#fff!important}
table.fin{margin:0}
table.fin td{background:#fff!important}
table.fin td.l{width:42%}
table.fin td.l b{display:block}
table.fin td.l span{display:block;font-size:9.5px;opacity:.7;margin-top:2px;line-height:1.35}
table.fin td.v{font-weight:600}
`;
function buildMomHtml(m){
  ...
  (m.sections||[]).forEach(function(sec){
    ...
    if(sec.kind==="finance"){
      body+='<tr><td></td><td colspan="4" class="fin-cell"><table class="fin">'+
        momFinRows(sec.fin).map(function(r){
          return '<tr><td class="l"><b>'+esc(r.label)+'</b>'+(r.note?'<span>'+esc(r.note)+'</span>':'')+'</td>'+
            '<td class="v">'+(r.value?esc(r.value).replace(/\n/g,"<br/>"):'<span class="none">—</span>')+'</td></tr>';
        }).join("")+'</table></td></tr>';
  ...
  return '<style>'+MOM_CSS+'</style>'+
    '<div class="mom-head">'+
      '<div class="t"><h1>'+esc(momTitle(m))+'</h1>'+
...
function MomView({moms,saveMoms,tenders,packages,people,contractors,tasks,saveTasks,onNavTender,memory,setMemory}){
  // The tender sheet reads the reviews from here to import their price table (see AwardsPanel).
  window._ppMoms=moms||[];
  var mem=memory||{};
  ...
  function duplicateMom(src){
    ...
    var sections=(src.sections||[]).map(function(sec){
      ...
      var out={id:uuid(),index:sec.index,title:sec.title,items:items.length?items:[newMomItem()]};
      if(sec.kind==="finance"){out.kind="finance";out.fin=newMomFin(sec.fin);if(!items.length)out.items=[];}
      return out;
    });
    var num=nextNumber();
    ...
    var m=newMom({
      ...
      sections:sections,
      momType:src.momType||"",reviewStage:src.reviewStage||"",
  ...
  if(!sel){
    ...
    var shown=list.filter(function(m){
      ...
      if(q.trim()){
        var hay=[m.number,m.subcontractor,m.package,m.location,m.preparedBy,momTitle(m)].join(" ").toLowerCase();
    ...
    return <div>
      <div className="page-hdr">
        ...
        {!isReadOnly()&&<div style={{display:"flex",gap:8}}>
          <button className="btn" onClick={function(){
            var m=newTenderReview({number:nextNumber(),preparedBy:(window._currentUser?window._currentUser.name:"")});
            save([m,...list]);setSelId(m.id);
          }} title="Mid or Final tender review: program, scope, specs, responsibility matrix, price and financial conditions">＋ Tender review</button>
      ...
      {shown.length===0
        ...
        :<table className="tbl">
          ...
          <tbody>{shown.map(function(m){
            var oc=openCount(m);
            return <tr key={m.id}>
              <td><span onClick={function(){setSelId(m.id);}} style={{fontFamily:"var(--font-mono)",fontWeight:700,color:"var(--blue,#0f5299)",cursor:"pointer"}}>{m.number||"—"}</span>
                {isTenderReview(m)&&<div><span className="badge" style={{background:"var(--blue-soft,#e8f0fe)",color:"var(--blue,#0f5299)",fontSize:10}}>
                  {m.reviewStage==="final"?"Final review":"Mid review"}</span></div>}</td>
  ...
  function itemHas(it){return !!((it.notes||"").trim()||(it.actionWith||"").trim());}
  // Categories: add, move, remove. Numbers are recomputed each time.
  function moveSection(si,d){
    var s=(sel.sections||[]).slice(),j=si+d;
    if(j<0||j>=s.length)return;
    var t=s[si];s[si]=s[j];s[j]=t;
    updM({sections:reindexMomSections(s)});
  }
  function deleteSection(si){
    var sec=(sel.sections||[])[si];
    if(sec.kind==="finance"){safeAlert("The price and financial conditions table cannot be removed.");return;}
    var n=(sec.items||[]).filter(itemHas).length;
    if(n>0){safeAlert("“"+sec.title+"” still holds "+n+" item"+(n!==1?"s":"")+".\n\nDelete them first, or rename the category instead.");return;}
    if(!safeConfirm("Remove the category “"+(sec.title||"untitled")+"”?"))return;
    updM({sections:reindexMomSections((sel.sections||[]).filter(function(_,i){return i!==si;}))});
  }
  function addSection(){
    var s=(sel.sections||[]).slice();
    var ns={id:uuid(),index:"",title:"New category",items:[newMomItem()]};
    // "Others" stays last: a new category goes just before it.
    var last=s.length?s[s.length-1]:null;
    if(last&&String(last.title||"").trim().toLowerCase()==="others")s.splice(s.length-1,0,ns);else s.push(ns);
    updM({sections:reindexMomSections(s)});
  }
  // Final review: bring over the Mid review of the same tender (or, without a tender, the
  // same subcontractor). The table is replaced, open items are added; closed ones stay behind.
  function importFromMid(){
    var cands=list.filter(function(m){
      if(m.id===sel.id||!isTenderReview(m)||m.reviewStage==="final")return false;
      if(sel.tenderRef)return m.tenderRef===sel.tenderRef;
      return !!sel.subcontractor&&m.subcontractor===sel.subcontractor;
    }).sort(function(a,b){return (b.date||"").localeCompare(a.date||"");});
    if(!cands.length){
      safeAlert("No Mid tender review found for "+(sel.tenderRef?"this tender":sel.subcontractor?sel.subcontractor:"this review")+".\n\n"+
        "The Mid review is matched on the Tender, or on the Subcontractor when no tender is set.");
      return;
    }
    var src=cands[0];
    function norm(t){return String(t||"").trim().toLowerCase();}
    var used={},added=0,finDone=false;
    var srcFin=(src.sections||[]).find(function(x){return x.kind==="finance";});
    function openCopies(sec){
      return (sec.items||[]).filter(function(it){return itemHas(it)&&it.status!=="closed";}).map(function(it){
        added++;
        return newMomItem({notes:it.notes,actionWith:it.actionWith,dueBy:it.dueBy,status:it.status||"",taskId:""});
      });
    }
    var secs=(sel.sections||[]).map(function(sec){
      var match=sec.kind==="finance"&&srcFin?srcFin
        :(src.sections||[]).find(function(x){return !used[x.id]&&x.kind!=="finance"&&norm(x.title)===norm(sec.title);});
      if(!match)return sec;
      used[match.id]=true;
      var out=Object.assign({},sec);
      if(sec.kind==="finance"){out.fin=newMomFin(match.fin);finDone=true;}
      var items=(sec.items||[]).filter(itemHas).concat(openCopies(match));
      out.items=items.length?items:(sec.kind==="finance"?[]:[newMomItem()]);
      return out;
    });
    // Categories that exist only in the Mid review are appended, so nothing is lost.
    (src.sections||[]).forEach(function(x){
      if(used[x.id]||(x.kind==="finance"&&finDone))return;
      var items=openCopies(x);
      if(!items.length&&x.kind!=="finance")return;
      var ns={id:uuid(),index:"",title:x.title,items:items};
      if(x.kind==="finance"){ns.kind="finance";ns.fin=newMomFin(x.fin);finDone=true;}
      var last=secs.length?secs[secs.length-1]:null;
      if(last&&norm(last.title)==="others")secs.splice(secs.length-1,0,ns);else secs.push(ns);
    });
    var takePeople=(sel.participants||[]).filter(function(p){return (p.name||"").trim();}).length===0&&(src.participants||[]).length>0;
    if(!safeConfirm("Import from "+(src.number||"the Mid review")+" (Mid tender review, "+fmtDate(src.date)+")?\n\n"+
      (srcFin?"· price and financial conditions table replaced\n":"")+
      "· "+added+" open item"+(added!==1?"s":"")+" added"+
      (takePeople?"\n· participants copied":"")+
      "\n· no link to the actions already created"))return;
    var patch={sections:reindexMomSections(secs),importedFromId:src.id,importedFromNumber:src.number||""};
    if(takePeople)patch.participants=(src.participants||[]).map(function(p){return Object.assign({},p,{id:uuid()});});
    if(!sel.subcontractor&&src.subcontractor)patch.subcontractor=src.subcontractor;
    if(!sel.package&&src.package)patch.package=src.package;
    if(!sel.tenderRef&&src.tenderRef)patch.tenderRef=src.tenderRef;
    updM(patch);
  }
  ...
  return <div>
    <div className="page-hdr">
      ...
      <div style={{display:"flex",gap:8}}>
        {isTenderReview(sel)&&sel.reviewStage==="final"&&<button className="btn" onClick={importFromMid}
          title="Copy the price table and the open items of the Mid review of the same tender">⤓ Import from Mid review</button>}
    ...
    <div className="titleblock" style={{display:"flex",alignItems:"stretch",overflow:"hidden",
      ...
      <div className="tb-main" style={{flex:1,minWidth:0,padding:"12px 16px"}}>
        <div className="tb-eyebrow" style={{fontSize:11,fontWeight:600,letterSpacing:".09em",textTransform:"uppercase",color:"var(--ink-3,#6f6b62)",marginBottom:3}}>{momTitle(sel)}</div>
        ...
        <div className="tb-sub" style={{fontSize:12,color:"var(--ink-3,#6f6b62)",marginTop:3}}>
          ...
          {sel.previousMomNumber&&<span style={{marginLeft:8}}>· follows <b>{sel.previousMomNumber}</b></span>}
          {sel.importedFromNumber&&<span style={{marginLeft:8}}>· imported from <b>{sel.importedFromNumber}</b></span>}</div>
      </div>
    </div>

    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:10,marginBottom:16}}>
      {(isTenderReview(sel)?[["reviewStage","Review","stage"]]:[]).concat([["number","MoM Number","text"],["date","Date","date"],["projectName","Project name","text"],
        ...
        return <div className="fg" key={f[0]}>
          ...
          {f[2]==="date"
            ...
            :f[2]==="stage"
              ?<select value={sel.reviewStage==="final"?"final":"mid"} onChange={function(e){updM({reviewStage:e.target.value});}} style={{fontWeight:700}}>
                <option value="mid">Mid Tender Review</option><option value="final">Final Tender Review</option></select>
            :f[2]==="tender"
              ?<select value={sel[f[0]]||""} onChange={function(e){
                  // Choosing a tender fills an empty package — one write, not two.
                  var v=e.target.value, td=(tenders||[]).find(function(t){return t.id===v;});
                  var patch={tenderRef:v};
                  if(td&&td.package&&!sel.package)patch.package=td.package;
                  updM(patch);
                }}>
    ...
    {(sel.sections||[]).map(function(sec,si){
      return <div key={sec.id} style={{marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",background:"#f0ede6",borderRadius:7,marginBottom:6}}>
          ...
          <button className="btn btn-sm" onClick={function(){updSection(si,{items:[...(sec.items||[]),newMomItem()]});}}>＋ {sec.kind==="finance"?"Comment":"Item"}</button>
          <button className="btn btn-sm" disabled={si===0} onClick={function(){moveSection(si,-1);}} title="Move the category up">▲</button>
          <button className="btn btn-sm" disabled={si===(sel.sections||[]).length-1} onClick={function(){moveSection(si,1);}} title="Move the category down">▼</button>
          {sec.kind!=="finance"&&<button onClick={function(){deleteSection(si);}} title="Remove this category (only when it holds no item)"
            style={{background:"none",border:"none",color:"#bbb",cursor:"pointer",fontSize:13}}>🗑</button>}
        </div>
        {sec.kind==="finance"&&<MomFinEditor fin={sec.fin} onChange={function(patch){updSection(si,{fin:Object.assign({},newMomFin(sec.fin),patch)});}}/>}
        {(sec.kind!=="finance"||(sec.items||[]).length>0)&&<table className="tbl" style={{fontSize:12}}>
    ...
    <button className="btn btn-sm" onClick={addSection} title="Add a category before “Others”; rename it in place">＋ Add category</button>
  </div>;
}

// The two-column table of a tender review. Column 1 is fixed; column 2 is typed, with the
// derived figures (advance %, bond amount) shown next to the input.
function MomFinEditor({fin,onChange}){
  var f=newMomFin(fin);
  var rows=momFinRows(f);
  var inp={fontSize:12,padding:"4px 7px"};
  var num=Object.assign({},inp,{width:150,fontFamily:"var(--font-mono)"});
  var derived={fontSize:11.5,color:"var(--ink-3,#6f6b62)",fontFamily:"var(--font-mono)"};
  function set(k){return function(e){onChange({[k]:e.target.value});};}
  function yn(k){
    return <select value={f[k]||""} onChange={set(k)} style={Object.assign({},inp,{width:80})}>
  ...
  function editor(key){
    ...
    if(key==="lumpSum")return <div style={row}><input type="number" step="any" min="0" value={f.lumpSum} onChange={set("lumpSum")} placeholder="€" style={num}/>
      <span style={derived}>{momEur(f.lumpSum)}</span></div>;
    if(key==="fixedPrice")return <div style={row}>{yn("fixedPrice")}
      {f.fixedPrice==="yes"&&<><span style={{fontSize:12}}>until</span>
        <input type="text" value={f.fixedUntil} onChange={set("fixedUntil")} style={Object.assign({},inp,{flex:1,minWidth:180})}/></>}</div>;
    if(key==="advance")return <div style={{display:"grid",gap:5}}>
      <div style={row}><input type="number" step="any" min="0" value={f.advance} onChange={set("advance")} placeholder="€" style={num}/>
        <span style={derived}>{momPct(f.advance,f.lumpSum)?momPct(f.advance,f.lumpSum)+" of the lump sum":(f.advance?"enter the lump sum for the %":"")}</span></div>
      <input type="text" value={f.advanceNote} onChange={set("advanceNote")} placeholder="Conditions as per Subcontract (bond…)" style={inp}/></div>;
    if(key==="perfBond"){
      var amt=(!isNaN(momNum(f.lumpSum))&&!isNaN(momNum(f.perfBondPct)))?momEur(momNum(f.lumpSum)*momNum(f.perfBondPct)/100):"";
      return <div style={row}>{yn("perfBond")}
        <input type="number" step="any" min="0" value={f.perfBondPct} onChange={set("perfBondPct")} style={Object.assign({},num,{width:70})}/>
        <span style={{fontSize:12}}>% of the Subcontract Price</span>
        {amt&&<span style={derived}>= {amt}</span>}</div>;
    }
    if(key==="delayDamages")return <div style={row}><input type="number" step="any" min="0" value={f.delayDamages} onChange={set("delayDamages")} placeholder="€" style={num}/>
      <span style={{fontSize:12}}>per day</span></div>;
    if(key==="defectsMonths")return <div style={row}><input type="number" step="1" min="0" value={f.defectsMonths} onChange={set("defectsMonths")} style={Object.assign({},num,{width:80})}/>
      <span style={{fontSize:12}}>months after Completion of the Subcontract works</span></div>;
    if(key==="assessment")return <textarea value={f.assessment} onChange={set("assessment")} style={Object.assign({},inp,{minHeight:46,width:"100%"})}/>;
    if(key==="retention")return <input type="text" value={f.retention} onChange={set("retention")} style={Object.assign({},inp,{width:120})}/>;
    if(key==="mos")return <div style={row}>{yn("mos")}
      {f.mos==="yes"&&<><input type="number" step="any" min="0" value={f.mosAmount} onChange={set("mosAmount")} placeholder="€" style={num}/>
        <span style={derived}>{momEur(f.mosAmount)}</span></>}</div>;
    return null;
  }
  return <table className="tbl" style={{fontSize:12,marginBottom:8}}>
    <thead><tr><th style={{width:"38%"}}>Item</th><th>Agreed</th></tr></thead>
    <tbody>{rows.map(function(r){
      return <tr key={r.key}>
        <td><b>{r.label}</b>{r.note&&<div style={{fontSize:11,color:"var(--ink-3,#6f6b62)",marginTop:2,lineHeight:1.35}}>{r.note}</div>}</td>
        <td>{editor(r.key)}</td>
      </tr>;
    })}</tbody>
  </table>;
