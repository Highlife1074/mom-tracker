...
function newContractor(overrides){return Object.assign({id:uuid(),name:"",package:"",owner:"",tenderRefs:[],contracts:[],createdAt:today(),advancePayment:{amount:"",date:"",note:""},notes:""},overrides||{});}
...
function KPICurveChart({kpi,weeks}){
  ...
  var latestDone=actualWeeks.length?actualWeeks[actualWeeks.length-1]:null;

  // Projections only make sense once real production has started — anchoring the
  // "avg to date" rate on week 0 of an empty schedule would understate it badly.
  var firstProdIdx=weeks.findIndex(function(w){return w.actualThis>0;});
  var projAvg=null,projLast3=null;
  function buildProjection(rate){
    if(!latestDone||!(rate>0))return{pts:[],finishLabel:"no finish at current rate"};
    var pts=[[latestDone.weekIndex,latestDone.actualCum]];
    var finishLabel=null;
    for(var j=latestDone.weekIndex+1;j<weeks.length;j++){
      var v=latestDone.actualCum+rate*(j-latestDone.weekIndex);
      if(target>0&&v>=target){pts.push([j,target]);finishLabel=fmtDate(weeks[j].monday);break;}
      pts.push([j,v]);
    }
    if(!finishLabel)finishLabel="beyond "+fmtDate(weeks[weeks.length-1].monday);
    return{pts:pts,finishLabel:finishLabel};
  }
  if(latestDone&&latestDone.actualCum>0&&firstProdIdx>-1){
    var elapsedSinceStart=Math.max(1,latestDone.weekIndex-firstProdIdx+1);
    projAvg=buildProjection(latestDone.actualCum/elapsedSinceStart);
    var pastWeeks=weeks.filter(function(w){return w.isPast;});
    var last3=pastWeeks.slice(-3);
    if(last3.length===3){
      projLast3=buildProjection(last3.reduce(function(s,w){return s+w.actualThis;},0)/3);
    }
  }

  var maxVal=Math.max(target,...weeks.map(function(w){return w.actualCum;}),
    projAvg?Math.max(0,...projAvg.pts.map(function(p){return p[1];})):0,
    projLast3?Math.max(0,...projLast3.pts.map(function(p){return p[1];})):0,1);
  ...
  var actualPts=actualWeeks.map(function(w){return xAt(w.weekIndex)+","+yAt(w.actualCum);}).join(" ");
  var projAvgPts=projAvg&&projAvg.pts.length?projAvg.pts.map(function(p){return xAt(p[0])+","+yAt(p[1]);}).join(" "):"";
  var projLast3Pts=projLast3&&projLast3.pts.length?projLast3.pts.map(function(p){return xAt(p[0])+","+yAt(p[1]);}).join(" "):"";
  var gridLines=[0,0.25,0.5,0.75,1].map(function(f){return Math.round(maxVal*f);});

  return <div style={{marginBottom:14}}>
  <svg viewBox={"0 0 "+W+" "+H} style={{width:"100%",height:"auto",background:"#fafaf8",borderRadius:8,border:"1px solid #e8e6df"}}>
    ...
    {weeks.filter(function(w,i){return i%Math.ceil(weeks.length/8||1)===0||i===weeks.length-1;}).map(function(w){
      var i=weeks.indexOf(w);
      return <text key={"pv"+w.monday} x={xAt(i)} y={yAt(w.plannedCum)-6} fontSize="8" fill="#c9a84c" textAnchor="middle">{w.plannedCum.toLocaleString()}</text>;
    })}
    <polyline points={plannedPts} fill="none" stroke="#c9a84c" strokeWidth="2" strokeDasharray="5,4"/>
    {projAvgPts&&<polyline points={projAvgPts} fill="none" stroke="#1a73e8" strokeWidth="1.5" strokeDasharray="6,3" opacity="0.55"/>}
    {projLast3Pts&&<polyline points={projLast3Pts} fill="none" stroke="#7b1fa2" strokeWidth="1.2" strokeDasharray="2,3" opacity="0.65"/>}
    {actualPts&&<polyline points={actualPts} fill="none" stroke="#1a73e8" strokeWidth="2.5"/>}
    {actualWeeks.map(function(w){return <g key={w.monday}>
      <circle cx={xAt(w.weekIndex)} cy={yAt(w.actualCum)} r="3" fill="#1a73e8"/>
      <text x={xAt(w.weekIndex)} y={yAt(w.actualCum)-7} fontSize="8" fontWeight="700" fill="#1a73e8" textAnchor="middle">{w.actualCum.toLocaleString()}</text>
    </g>;})}
  ...
  {(projAvg||projLast3)&&<div style={{display:"flex",gap:14,flexWrap:"wrap",marginTop:6,fontSize:10,color:"#888"}}>
    {projAvg&&<div><span style={{display:"inline-block",width:10,borderTop:"1.5px dashed #1a73e8",marginRight:4,opacity:.7}}>&nbsp;</span>Avg to date → {projAvg.finishLabel}</div>}
    {projLast3&&<div><span style={{display:"inline-block",width:10,borderTop:"1.2px dotted #7b1fa2",marginRight:4,opacity:.7}}>&nbsp;</span>Last 3 wks → {projLast3.finishLabel}</div>}
  </div>}
  </div>;
}
