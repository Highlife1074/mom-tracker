...
function buildZoneWeeklyReport(zone,schedules,rooms,tasks,tenders,opts){
  ...
  windows.forEach(function(win){
    ...
    scs.forEach(function(sc){
      (sc.rows||[]).forEach(function(r){
        if(r.kind==="category"||!r.startWeek)return;
        if(r.startWeek>=win.start&&r.startWeek<=end)starting.push({r:r,sc:sc});
      });
    });
    ...
    html+='<table class="q"><tr><th style="width:26%">Task</th><th>Subcontractor</th>'+
      READINESS_Qs.map(function(q){return '<th style="text-align:center">'+esc(q)+'</th>';}).join("")+'</tr>';
    starting.forEach(function(x){
      html+='<tr><td><b>'+esc(x.r.label)+'</b><div style="font-size:9px;color:#8b8578">'+esc(x.sc.title)+'</div></td>'+
        '<td>'+esc(x.r.group||"—")+'</td>'+
...
function buildProcurementReport(tenders,tasks,packages){
  ...
  pkgs.forEach(function(p){
    ...
    // --- procurement recap
    html+='<h3>Procurement status</h3><table><tr><th>Tender</th><th>Contract</th><th>ACC / Aconex</th><th>Approval</th><th>Delivery</th><th>Start on site</th></tr>';
    list.forEach(function(td){
      ...
      html+='<tr><td><b>'+esc(td.title)+'</b></td>'+
        ...
        '<td>'+(td.startOnSite?esc(fmtDate(td.startOnSite)):'—')+'</td></tr>';
    });
    html+='</table>';
    ...
    if(mars.length===0)html+='<div class="none">No material registered.</div>';
      ...
      html+='<table><tr><th style="width:32%">Material</th><th>Tender</th><th>MSS</th><th>MAR submitted</th><th>Approval</th><th>Lead time</th></tr>';
    ...
    // --- WMS / ITP
    html+='<h3>WMS &amp; ITP</h3><table><tr><th style="width:32%">Tender</th><th>Doc</th><th>Theoretical</th><th>Target</th><th>Done</th><th>Approval</th></tr>';
    list.forEach(function(td){
...
function MaterialsPanel({td,updTd,saveT,tasks,tenders,saveTenders,setSelTender,pkgOwners}){
  ...
  // MAR is due a fixed number of days after the contract is signed (Settings › Durations).
  var matTheo=(function(){
    var ct=(td.stepDates||{}).contract||{};
    var signed=ct.signedAllDone||ct.signedDone||ct.done||"";
    return isValidDate(signed)?addCalDays(signed,getDur("marAfterContract")):"";
  })();
    ...
    {open&&<div>
      ...
      {mats.length>0&&<table className="tbl" style={{fontSize:12}}>
        <thead><tr>
          ...
          <th style={{width:52,textAlign:"center"}} title="Tick when this material needs a Material Submission Sheet. Most do not.">MSS</th>
          <th style={{textAlign:"center",minWidth:96}} title="Contract signed + the MAR lead time set in Settings › Durations">Theoretical</th>
              ...
              return <tr key={kind} style={{background:di>0?"#fafaf8":"#fff"}}>
                ...
                <td style={{textAlign:"center"}}>
                  <span className="badge" style={{background:kind==="MSS"?"var(--blue-soft,#e8f0fe)":"var(--gold-soft,#faf3e0)",
                    color:kind==="MSS"?"var(--blue,#0f5299)":"var(--gold-ink,#8a6a1e)",marginRight:6}}>{kind}</span>
                  {matTheo?<span style={{fontSize:11,color:"var(--ink-3,#6f6b62)"}}>{fmtDate(matTheo)}</span>
                    :<span style={{color:"#ddd",fontSize:11}} title="No signed contract yet">—</span>}
                </td>
                <td style={{textAlign:"center"}}>
...
function SubmissionSteps({td,TENDER_STEPS,updateStep,tenders,saveTenders,setSelTender,tasks,saveTasks,pkgOwners}){
  ...
  return <div>
    <div onClick={function(){setStepsOpen(!stepsOpen);}} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",marginBottom:stepsOpen?12:0}}>
      ...
      <h3 style={{fontFamily:"var(--font-display)",fontWeight:700,fontSize:"var(--fs-title)",letterSpacing:"-.005em"}}>Submission steps</h3>
      <span className="hint" style={{fontSize:"var(--fs-small)",color:"var(--ink-3)"}}>bidders → tender package → ACC/Aconex</span>
    </div>
    {stepsOpen&&<div>
    ...
    <div className="sheet-h">
      <h3>WMS &amp; ITP</h3>
      <span className="hint">driven by the start on site, not by the submission chain</span>
    </div>
    <table className="tbl" style={{fontSize:12,marginBottom:16}}>
      <thead><tr>
        ...
        <div className="page-hdr" style={{marginBottom:12}}>
          <div style={{flex:1}}>
            ...
            <div className="titleblock accent" style={{display:"flex",alignItems:"stretch",overflow:"hidden",
              border:"1.5px solid var(--gold,#c9a84c)",borderLeftWidth:5,borderRadius:8,background:"#fff",marginBottom:16}}>
              <div className="tb-main" style={{flex:1,minWidth:0,padding:"12px 16px"}}>
                ...
                <div className="tb-sub" style={{fontSize:12,color:"#6f6b62",marginTop:3}}>{td.ownerTender||"no owner"}{td.contractor?" · "+td.contractor:""}</div>
              </div>
              <div className="tb-cells" style={{display:"flex",borderLeft:"1.5px solid var(--gold,#c9a84c)",flexShrink:0}}>
                {(function(){
                  ...
                  var cells=[
                    ...
                    {k:"Delivery",v:proc.deliveryDate?fmtDate(proc.deliveryDate):"—",cls:""},
                    {k:"Start",v:td.startOnSite?fmtDate(td.startOnSite):"—",cls:"gold"},
                    {k:"Margin",v:(proc.margin===undefined||proc.margin===null||proc.margin==="")?"—":proc.margin+"d",
                      cls:Number(proc.margin)<0?"red":Number(proc.margin)>=0?"green":""}
                  ];
                  return cells.map(function(c){
                    var vc=c.cls==="red"?"#b3302a":c.cls==="green"?"#1e6b3a":c.cls==="gold"?"#8a6a1e":"#16181d";
                    return <div className="tb-cell" key={c.k} style={{padding:"10px 15px",borderRight:"1px solid #ddd9cf",minWidth:96}}>
                      ...
                      <span className={"v "+(c.cls||"")} style={{display:"block",fontSize:13,fontWeight:600,color:vc,whiteSpace:"nowrap"}}>{c.v}</span>
                    </div>;
                  });
                })()}
              </div>
