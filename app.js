...
const KEYS={
  ...
  pkgowners:"pp_pkgowners", zones:"pp_zones", pkgsubcontractors:"pp_pkgsubcontractors",
...
function TendersView({tenders,saveTenders,packages,people,tasks,saveTasks,contractors,pkgOwners,jumpTender,clearJumpTender,jumpFrom,onBack}){
  ...
  var SUBMIT_RESULT_STEPS={
    ...
    itp:{text:function(t){return"Submit the ITP Result — "+t.title;},tag:function(t){return qualityTag(t.package||"");}},
    wms:{text:function(t){return"Submit the WMS Result — "+t.title;},tag:function(){return null;}}
  };
...
function SettingsView({tags,saveTags,people,savePeople,packages,savePackages,tagrules,saveTagrules,pkgrules,savePkgrules,apiKey,saveApiKey,improvements,saveImprovements,pkgOwners,savePkgOwners,pkgSubcontractors,savePkgSubcontractors,peopleEmails,savePeopleEmails,defaultCC,saveDefaultCC,peopleAccess,savePeopleAccess,zones,saveZones,userPrefs,saveUserPrefs,allData,onImport}){
  ...
  return <div>
    ...
    {tab==="zones"&&<div className="card">
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
        {(zones||[]).map(z=><div key={z} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 12px",borderRadius:20,background:"#f0ede6",border:"1.5px solid #e0ddd6"}}>
          <span style={{fontSize:12,fontWeight:600,color:"#555"}}>🏢 {z}</span>
          <button onClick={()=>removeZone(z)} style={{background:"none",border:"none",cursor:"pointer",color:"#bbb",fontSize:12,padding:"0 2px"}} onMouseEnter={e=>e.currentTarget.style.color="#c62828"} onMouseLeave={e=>e.currentTarget.style.color="#bbb"}>×</button>
        </div>)}
      </div>
    </div>}
    ...
    {tab==="access"&&<div className="card">
      ...
      <div style={{fontSize:12,color:"#888",marginBottom:14}}>Controls what each person sees when they open the app. <strong>Full Access</strong> sees everything (like today). Without it, people see only Zone (if pilot), Actions, and their own assigned tenders/packages — plus any packages given to them below as extra access (for people responsible for a package without being its formal owner).</div>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {people.map(function(p){
          var acc=(peopleAccess||{})[p]||{};
          ...
          return <div key={p} style={{padding:"10px 12px",borderRadius:10,border:"1.5px solid #e8e6df",background:"#fafaf8"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:acc.fullAccess?0:8}}>
              <span style={{fontWeight:700,fontSize:13,minWidth:130}}>{p.split(",")[0]}</span>
              <label style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",textTransform:"none",letterSpacing:"normal",fontSize:12,fontWeight:600,color:acc.fullAccess?"#2e7d32":"#888"}}>
                <input type="checkbox" checked={!!acc.fullAccess} onChange={function(){toggle("fullAccess");}} style={{width:14,height:14}}/>
                Full Access
              </label>
              <label style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",textTransform:"none",letterSpacing:"normal",fontSize:12,fontWeight:600,color:acc.zonePilot?"#7b1fa2":"#888"}}>
...
function GlobalView({tasks,trackers,tenders,contractors,people,packages,tags,saveTasks,saveTrackers,tagrules,pkgrules,jumpOwner,clearJump,onNavTender,memory,setMemory,peopleEmails,defaultCC,zones}){
  var mem=memory||{};
  ...
  const [fOwners,setFOwners]=useState(mem.fOwners||[]);
...
function ZoneView({tasks,saveTasks,rooms,saveRooms,zones,people,tags,memory,setMemory,peopleEmails,defaultCC,kpis,saveKpis,meetings,saveMeetings}){
  var mem=memory||{};
  ...
  function addTask(){
    ...
    var td={text:qText.trim(),due:qDue,owner:qOwner,tags:qTags,zone:curZone,importance:1,urgence:1};
    saveTasks([newTask(td),...(tasks||[])]);
  ...
  function buildZoneReport(){
    ...
    var toNames=[...new Set([...ownerNames,...attendeeNames])];
    var missing=[];
  ...
  return <div style={{padding:"16px 20px",overflowY:"auto",flex:1}}>
    <div className="page-hdr">
      <div>
        ...
        <div className="page-sub">Dedicated space for zone pilots — actions &amp; room-by-room progress</div>
      </div>
          ...
          return <div key={kpi.id} className="card" style={{marginBottom:10,padding:0,overflow:"hidden"}}>
            ...
            {isOpen&&<div style={{padding:"14px 16px",borderTop:"1.5px solid #f0ede6"}}>
              ...
              <div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}>
                <button className="btn btn-sm btn-danger" onClick={function(){delKpi(kpi.id);}}>🗑 Delete KPI</button>
              </div>
...
function App(){
  ...
  const saveRooms=d=>{setRooms(d);mergeSync(KEYS_ROOMS,d,"id");};
  const [kpis,setKpis]=useState([]);
  const saveKpis=d=>{setKpis(d);mergeSync(KEYS_KPIS,d,"id");};
  const [meetings,setMeetings]=useState([]);
  const saveMeetings=d=>{setMeetings(d);mergeSync(KEYS_MEETINGS,d,"id");};
  ...
  useEffect(()=>{
    const load=async()=>{
      try{
        const [t,x,td,ct,p,pk,g,tr,pr,imp,corr,awn,pko,prefs,zn,rm,pkgsub,pemail,dcc,kp,pacc,mtg]=await Promise.all([
          ...
          cloudStore.get(KEYS.tags),cloudStore.get(KEYS.tagrules),cloudStore.get(KEYS.pkgrules),cloudStore.get(KEYS_IMP),cloudStore.get(KEYS_CORR),cloudStore.get(KEYS_AWN),cloudStore.get(KEYS.pkgowners),cloudStore.get(KEYS_PREFS),cloudStore.get(KEYS.zones),cloudStore.get(KEYS_ROOMS),cloudStore.get(KEYS.pkgsubcontractors),cloudStore.get(KEYS.peopleemails),cloudStore.get(KEYS.defaultcc),cloudStore.get(KEYS_KPIS),cloudStore.get(KEYS.peopleaccess),cloudStore.get(KEYS_MEETINGS)
        ]);
        if(t){setTasks(t);}
        else{
      ...
      try{
        ...
        if(curTasks.length>0){
          var needsSync=false;
          var AUTO_RESULT_PREFIXES=["Submit the Tender Result — ","Submit the ITP Result — ","Submit the WMS Result — ","Get the approval of the Tender result from the client — "];
  ...
  // Access model: Full Access sees everything (NAV above, unchanged).
  // Without it, people see only Zone (if Zone Pilot), Actions, My Procurement (their own tenders/packages + any extra package access), and Settings.
  var myAccess=(peopleAccess||{})[currentUser]||{};
  var hasFullAccess=!!myAccess.fullAccess||!Object.keys(peopleAccess||{}).length;
  var myPkgs=[],myTenders=[],effectiveNAV=NAV;
  if(!hasFullAccess){
    ...
    // If the last-restored view isn't in this person's allowed nav, fall back to their first available tab
    if(!effectiveNAV.some(function(n){return n.id===view;})){
  ...
  if(!currentUser)return <UserLogin people={people} onLogin={function(name){setCurrentUser(name);window._currentUser={name:name};}}/>;  return <div className="layout">
    ...
    <div className="main-area">
      <div className="content">
        ...
        {view==="myprocurement"&&<MyProcurementView myPkgs={myPkgs} myTenders={myTenders} tenders={tenders} tasks={tasks} onNavTender={navToTender}/>}
        ...
        {view==="zone"&&<ZoneView tasks={tasks} saveTasks={saveT} rooms={rooms} saveRooms={saveRooms} zones={zones} people={people} tags={tags} memory={memoryFor("zone")} setMemory={function(p){setMemoryFor("zone",p);}} peopleEmails={peopleEmails} defaultCC={defaultCC} kpis={kpis} saveKpis={saveKpis} meetings={meetings} saveMeetings={saveMeetings}/>}
