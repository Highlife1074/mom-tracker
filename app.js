...
function TendersView({tenders,saveTenders,packages,people,tasks,saveTasks,contractors,pkgOwners,jumpTender,clearJumpTender,jumpFrom,onBack}){
  ...
  function autoActionForTender(td2,text,tag){
    if(!saveTasks||!tasks)return;
    ...
    saveTasks([newTask({text:text,owner:owner,tenderRef:td2.id,package:td2.package||"",tags:[tag||"Contract"],status:"pending",note:"Auto-created from contract/ACC status",addedBy:"System"}),...(tasks||[])]);
  }
  ...
  function updateStep(tdId,step,field,val){
    ...
    if(updatedTd&&step==="acc"&&field==="done"&&val){
    ...
    // Trigger "Submit the Tender Result" when ACC/Aconex gets a Target date but no Date done yet
    if(updatedTd&&step==="acc"&&field==="target"&&val){
      var accDoneNow=((updatedTd.stepDates||{}).acc||{}).done||"";
      if(!accDoneNow)autoActionForTender(updatedTd,"Submit the Tender Result — "+updatedTd.title);
    }
    // Close "Submit the Tender Result" once ACC/Aconex Date done is filled
    if(updatedTd&&step==="acc"&&field==="done"&&val&&!prevAccDone){
      closeActionForTender(updatedTd,"Submit the Tender Result — "+updatedTd.title);
    }
  }
