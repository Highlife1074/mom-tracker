...
function newScheduleRow(kind,label){return{id:uuid(),kind:kind||"task",label:label||"",cells:{},afterId:"",lagWeeks:0,roomId:"",progress:"",weekProgress:{}};}
...
function isAppAdmin(name){return (name||"").trim().toLowerCase()===APP_ADMIN.toLowerCase();}
  ...
  function duplicateCategoryBlock(catRowId){
    ...
    var copies=block.map(function(r,bi){
      return Object.assign({},r,{
        ...
        cells:Object.assign({},r.cells||{}),
        afterId:idMap[r.afterId]||r.afterId||""
      });
    });
    rows.splice(end,0,...copies);
    upd(sc.id,{rows:rows});
              ...
              <tr>
                ...
                {wks.map(function(wk){
                  ...
                  return <th key={wk} onClick={function(){toggleHolidayWeek(wk);}}
                    ...
                    style={{position:"sticky",top:0,zIndex:3,minWidth:38,padding:"4px 2px",borderBottom:"1.5px solid #e8e6df",borderLeft:"1px solid #f0ede6",cursor:"pointer",
...
function App(){
  ...
  useEffect(()=>{
    const load=async()=>{
      ...
      // One-off rename: older auto-tasks were called "Submit the ITP/WMS Result", now just "Submit the ITP/WMS"
      try{
        var renameMap=[["Submit the ITP Result — ","Submit the ITP — "],["Submit the WMS Result — ","Submit the WMS — "]];
        var tRenamed=false;
        var renamedTasks=(t||[]).map(function(tk){
          var nt=tk;
        ...
        if(tRenamed){t=renamedTasks;setTasks(renamedTasks);sync(KEYS.tasks,renamedTasks);}
