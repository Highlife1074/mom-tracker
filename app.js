...
function SettingsView({subList,saveSubList,subColors,saveSubColors,contractors,tags,saveTags,people,savePeople,packages,savePackages,tagrules,saveTagrules,pkgrules,savePkgrules,apiKey,saveApiKey,improvements,saveImprovements,pkgOwners,savePkgOwners,pkgSubcontractors,savePkgSubcontractors,peopleEmails,savePeopleEmails,defaultCC,saveDefaultCC,peopleAccess,savePeopleAccess,durations,saveDurations,isAdmin,zones,saveZones,zoneOwners,saveZoneOwners,userPrefs,saveUserPrefs,allData,onImport}){
          ...
          <div className="ac-meta">
            ...
            {(task.tags||[]).includes("Prerequisite")&&(function(){
              ...
              return <button onClick={function(){onUpdate({dateConfirmed:!ok});}}
                ...
                style={{fontSize:9,fontWeight:800,padding:"1px 8px",borderRadius:10,cursor:"pointer",fontFamily:"inherit",
...
function ScheduleView({curZone,schedules,saveSchedules,tasks,saveTasks,people,tags,zones,rooms,saveRooms,canEdit,roomBlockersOf,tenders,saveTenders,pkgOwners}){
  ...
  function printSchedule(){
    ...
    var tbl=w?w.querySelector("table"):null;
    var prevZoom="";
    if(tbl){
      // A4 landscape minus 8mm margins ≈ 281mm ≈ 1062px at 96dpi.
      var PAGE=1062;
      var full=tbl.scrollWidth;
      prevZoom=tbl.style.zoom||"";
      if(full>PAGE){
        var z=PAGE/full;
        if(z<0.28)z=0.28;                       // below this nothing is readable anyway
        tbl.style.zoom=String(z);
        if(z<=0.45&&!safeConfirm("This schedule has "+wks.length+" weeks. To fit them all on one page it must be shrunk to "+Math.round(z*100)+"% — the text will be very small.\n\nPrint anyway?\n\n(Cancel, then use the week filter or split the schedule, for a more readable output.)")){
          tbl.style.zoom=prevZoom;
          return;
        }
      }
    }
    ...
    setTimeout(function(){
      ...
      setTimeout(function(){
        ...
        if(tbl)tbl.style.zoom=prevZoom;
      },500);
    },120);
  }
            ...
            <tbody>
              ...
              {visibleRows.map(function(row){
                ...
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  <td style={{position:"sticky",left:0,zIndex:1,background:focusRow===row.id?"#fff8e1":isSel?"#e8f0fe":isCat?"#f0ede6":"#fff",padding:"3px 10px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df",boxShadow:focusRow===row.id?"inset 3px 0 0 #f57f17":"none"}}>
    ...
    {subTab==="actions"&&<div>
      ...
      <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:16,alignItems:"center"}}>
        ...
        <input type="text" value={qAct} onChange={function(e){setQAct(e.target.value);}}
          placeholder="🔎 Search…" style={{width:180,padding:"3px 9px",fontSize:11,marginLeft:6}}/>
        <label style={{display:"flex",alignItems:"center",gap:4,fontSize:10,textTransform:"none",letterSpacing:"normal",cursor:"pointer",color:"#888",margin:0}}>
      ...
      {orderedCats.map(function(cat){
        ...
        return <div key={cat} style={{marginBottom:18}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,paddingBottom:6,borderBottom:"2px solid "+tc.color}}>
            <span style={{padding:"3px 12px",borderRadius:20,background:tc.bg,color:tc.color,fontWeight:800,fontSize:12,letterSpacing:".3px"}}>{cat==="Blocking Point"?"🚧 ":cat==="Prerequisite"?"🔒 ":cat==="Warning"?"🟠 ":""}{cat.toUpperCase()}</span>
