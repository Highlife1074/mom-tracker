  ...
  return <div className="ac-item" style={{background:editMode?"#f8f9ff":task.status==="done"?"#fafaf8":"#fff",borderColor:editMode?"#3949ab":"#e8e6df",flexDirection:"column",gap:0}}>
    <div style={{display:"flex",alignItems:"flex-start",gap:8,width:"100%"}}>
      ...
      {!editMode&&<div style={{display:"flex",gap:4,flexShrink:0}}>
        ...
        {onDelete&&<button className="btn btn-sm btn-danger" onClick={function(e){e.stopPropagation();if(window.confirm&&window.confirm("Delete this action?")||!window.confirm){onDelete();}}} style={{padding:"3px 7px"}}>🗑</button>}
      </div>}
    </div>
  </div>;
}
...
function MaterialsPanel({td,updTd,saveT,tasks,tenders,saveTenders,setSelTender}){
  ...
  function updMat(mi,field,val){
    ...
    updTd("materials",ms);
    // Auto-create tasks for MSS/MAR submission and approval
    var mat=ms[mi];
    var matName=mat.name||("Material "+(mi+1));
    var owner=td.ownerTender||"";
    var tenderRef=td.id;
    var pkg=td.package||"";
    if(field==="mssTarget"||field==="mssDone"||field==="mssApprovalTarget"||
       field==="marTarget"||field==="marDone"||field==="marApprovalTarget"){
      var prefix=field.startsWith("mss")?"MSS":"MAR";
      var matId=mat.id||(tenderRef+"-mat-"+mi);
      // Ensure mat has stable id
      if(!mat.id){ms=ms.map(function(m,j){return j!==mi?m:Object.assign({},m,{id:matId});});updTd("materials",ms);}
      var subTaskId="auto-"+prefix+"-sub-"+matId;
      var appTaskId="auto-"+prefix+"-app-"+matId;
      var curTasks=tasks||[];
      var targetField=prefix==="MSS"?"mssTarget":"marTarget";
      var doneField=prefix==="MSS"?"mssDone":"marDone";
      var target=ms[mi][targetField]||"";
      var done=ms[mi][doneField]||"";
      // Submission task
      var subTask=curTasks.find(function(t){return t.id===subTaskId;});
      if(target){
        if(!subTask){
          subTask={id:subTaskId,text:prefix+" "+matName+" — à soumettre",owner:owner,
            package:pkg,status:done?"done":"pending",importance:2,urgence:2,
            due:target,note:"",tags:[prefix],tenderRef:tenderRef,autoGenerated:true};
          curTasks=[subTask,...curTasks];
        } else {
          curTasks=curTasks.map(function(t){return t.id!==subTaskId?t:Object.assign({},t,
            {due:target,status:done?"done":"pending",owner:owner,text:prefix+" "+matName+" — à soumettre"});});
        }
      }
      // Approval task (created when done is set)
      if(done){
        var appDue=(function(){var d=new Date(done);d.setDate(d.getDate()+14);return d.toISOString().slice(0,10);}());
        var appTask=curTasks.find(function(t){return t.id===appTaskId;});
        if(!appTask){
          appTask={id:appTaskId,text:prefix+" "+matName+" — approval (+14j)",owner:owner,
            package:pkg,status:"pending",importance:2,urgence:2,
            due:appDue,note:"⚠️ Approval due 14 days after submission",tags:[prefix],tenderRef:tenderRef,autoGenerated:true};
          curTasks=[appTask,...curTasks];
        } else {
          curTasks=curTasks.map(function(t){return t.id!==appTaskId?t:Object.assign({},t,{due:appDue,owner:owner});});
        }
      }
      if(saveT)saveT(curTasks);
    }
  }
  ...
  return <div className="card" style={{marginBottom:10}}>
    ...
    {open&&<div>
      ...
      {mats.map(function(mat,mi){
        ...
        var mssOpen=isSectOpen(mi,'mss');
        var marOpen=isSectOpen(mi,'mar');
        ...
        return <div key={mat.id||mi} style={{padding:"8px 10px",background:"#fafaf8",borderRadius:7,border:"1px solid #f0ede6",marginBottom:8}}>
          ...
          <div style={{marginBottom:4}}>
            <div onClick={function(){toggleSect(mi,'mss');}} style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",padding:"4px 6px",background:mssOpen?"#e8f0fe33":"transparent",borderRadius:5,marginBottom:mssOpen?4:0}}>
          ...
          <div>
            <div onClick={function(){toggleSect(mi,'mar');}} style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",padding:"4px 6px",background:marOpen?"#f3e5f533":"transparent",borderRadius:5,marginBottom:marOpen?4:0}}>
...
function SubmissionSteps({td,TENDER_STEPS,updateStep,tenders,saveTenders,setSelTender}){
  ...
  return <div>
          ...
          <thead style={{position:"sticky",top:0,zIndex:10,background:"#f5f4f0",boxShadow:"0 1px 0 #e0d9cc"}}><tr>
            <th>Step</th>
            <th>Status</th>
          ...
          <tbody>{TENDER_STEPS.map(function(s){
            ...
            var _isReqSent=val&&val.toLowerCase().includes("request sent");
            var _todaySS=today();
            var _stepOverdue=!dates.done&&dates.target&&(
              _isReqSent
                ?(function(){var d=new Date(dates.target);d.setDate(d.getDate()+10);return _todaySS>d.toISOString().slice(0,10);}())
                :(dates.target<_todaySS&&val&&!val.toLowerCase().includes("approved")&&val!=="—"&&val!=="N/A")
            );
            return <tr key={s.key} style={{background:_stepOverdue?"#fff5f5":""}}>
              ...
              <td style={{textAlign:"center"}}>
                <div style={{display:"flex",alignItems:"center",gap:3,justifyContent:"center"}}>
                  <input type="date" value={dates.target||""} onChange={function(e){updateStep(td.id,s.key,"target",e.target.value);}} style={{border:"1px solid "+(_stepOverdue?"#c62828":"#e8e6df"),borderRadius:5,padding:"3px 6px",fontSize:11,color:_stepOverdue?"#c62828":"inherit"}}/>
                  {_stepOverdue&&<span title="Overdue" style={{color:"#c62828",fontSize:12,fontWeight:700}}>⚠️</span>}
                </div>
              </td>
...
function TendersView({tenders,saveTenders,packages,people,tasks,saveTasks,contractors,pkgOwners,jumpTender,clearJumpTender,jumpFrom,onBack}){
  ...
  if(selTender){
    ...
    // Contracts NOT included in ACC add to extra cost
    var contractsInACC=linkedContracts.filter(function(x){return x.ct.inclACC;});
    var contractsOutACC=linkedContracts.filter(function(x){return !x.ct.inclACC;});
    var contractsOutTotal=contractsOutACC.reduce(function(s,x){return s+Number(x.ct.amount||0)+(x.ct.addendums||[]).reduce(function(s2,ad){return s2+Number(ad.amount||0);},0);},0);
    ...
    // Total cost = ACC + contracts outside ACC
    var totalCost=accTotal+contractsOutTotal;

    return <div style={{display:"flex",gap:16,alignItems:"stretch",height:"100%",overflow:"hidden"}}>

      <div style={{width:200,flexShrink:0,background:"#fff",borderRadius:12,border:"1.5px solid #e8e6df",overflow:"hidden",position:"sticky",top:0,height:"fit-content"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
          ...
          {contractGrand>0&&<div className="card" style={{flex:1,minWidth:120,marginBottom:0,padding:"10px 14px",background:"#fff8f0"}}>
            ...
            <div style={{fontSize:10,color:"#aaa"}}>
              {contractsInACC.length>0&&<span style={{color:"#2e7d32"}}>✅ In ACC: {contractsInACC.reduce(function(s,x){return s+Number(x.ct.amount||0);},0).toLocaleString()}</span>}
              {contractsInACC.length>0&&contractsOutACC.length>0&&" · "}
              {contractsOutACC.length>0&&<span style={{color:"#c62828"}}>⚠️ Extra: {contractsOutTotal.toLocaleString()}</span>}
            </div>
          </div>}
          {contractsOutTotal>0&&<div className="card" style={{flex:1,minWidth:120,marginBottom:0,padding:"10px 14px",background:"#fff0f0",borderLeft:"3px solid #c62828"}}>
            <div style={{fontSize:10,color:"#c62828",marginBottom:2}}>Total Cost (ACC + Extra)</div>
            <div style={{fontSize:16,fontWeight:800,color:"#c62828"}}>{totalCost.toLocaleString()} {cur}</div>
            <div style={{fontSize:10,color:"#aaa"}}>ACC {accTotal.toLocaleString()} + {contractsOutTotal.toLocaleString()}</div>
          </div>}
        </div>
      ...
      <div className="card">
        ...
        <SubmissionSteps td={td} TENDER_STEPS={TENDER_STEPS} updateStep={updateStep} tenders={tenders} saveTenders={saveTenders} setSelTender={setSelTender}/>
  ...
  setTimeout(function(){
    ...
    {filtered.length===0
      ...
      :<div style={{overflowX:"auto",flex:1}}><table className="tbl">
        <thead><tr>
          ...
          <th style={{textAlign:"right",color:"#b45309",minWidth:90}}>Contracts</th>
        ...
        <tbody>{filtered.map(function(td){
          ...
          return <tr key={td.id} style={{cursor:"pointer"}} onClick={function(){setSelTender(td);}}>
            ...
            <td style={{textAlign:"right",whiteSpace:"nowrap"}}>
              {(function(){
                var lc=(contractors||[]).flatMap(function(ctr){return (ctr.contracts||[]).filter(function(ct){return ct.tenderRef===td.id;}).map(function(ct){return ct;});});
                if(lc.length===0)return <span style={{color:"#ddd"}}>—</span>;
                var total=lc.reduce(function(s,ct){return s+Number(ct.amount||0)+(ct.addendums||[]).reduce(function(s2,ad){return s2+Number(ad.amount||0);},0);},0);
                var acc=Number(td.accAmountSubcontract||0)+Number(td.accAmountOther||0);
                var gap=acc>0?total-acc:0;
                return <div>
                  <span style={{fontSize:11,fontWeight:700,color:"#b45309"}}>{total.toLocaleString()}</span>
                  {acc>0&&<div style={{fontSize:9,color:gap<0?"#c62828":gap>0?"#2e7d32":"#888",fontWeight:gap!==0?700:400}}>
                    {gap>0?"▲ +":"▼ "}{gap!==0?Math.abs(gap).toLocaleString():"= ACC"}
                  </div>}
                </div>;
              })()}
            </td>
            ...
            {TENDER_STEPS.filter(function(s){return s.key!=="process"&&s.key!=="bidders";}).map(function(s){
              ...
              var isNA=v==="N/A";
              var _today=today();
              var isRequestSent=v&&v.toLowerCase().includes("request sent");
              var isOverdue=!done&&!isNA&&(
                isRequestSent
                  ? (target&&(function(){var d=new Date(target);d.setDate(d.getDate()+10);return _today>d.toISOString().slice(0,10);}()))
                  : (target&&target<_today&&!v.toLowerCase().includes("approved"))
              );
              var isEmpty=!v||v==="—";
              ...
              return <td key={s.key} style={{verticalAlign:"top",minWidth:90,borderLeft:needsBorderLeft?"2px solid #e0d9cc":""}}>
                ...
                {!isNA&&!done&&target&&<div style={{fontSize:9,marginTop:2,color:isOverdue?"#c62828":"#888",fontWeight:isOverdue?700:400}}>
                  {isOverdue?"⚠️ ":""}{fmtDate(target)}
                </div>}
                {!isEmpty&&!isNA&&done&&<div style={{fontSize:9,marginTop:1,color:"#2e7d32",fontWeight:600}}>✓ {fmtDate(done)}</div>}
              </td>;
            })}
          </tr>;
        })}</tbody>
  ...
  return <div>
    ...
    <div className="card" style={{marginBottom:10}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
        <label style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer",padding:"6px 10px",background:ct.inclACC?"#e8f5e9":"#fff8f0",borderRadius:6,border:"1.5px solid "+(ct.inclACC?"#2e7d32":"#f57c00"),fontSize:12,fontWeight:600,userSelect:"none"}}>
          <input type="checkbox" checked={ct.inclACC||false} onChange={function(e){updateCtField(ctr.id,ct.id,"inclACC",e.target.checked);}} style={{width:14,height:14,accentColor:"#2e7d32"}}/>
          {ct.inclACC?"✅ Inclus dans ACC":"⚠️ Hors ACC (s'ajoute au coût)"}
        </label>
      </div>
