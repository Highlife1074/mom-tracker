...
function newSchedule(overrides){return Object.assign({id:uuid(),zone:"",title:"New schedule",startDate:today(),weeks:12,rows:[],createdAt:today(),updatedAt:today(),updatedBy:window._currentUser?window._currentUser.name:""},overrides||{});}
function newScheduleRow(kind,label){return{id:uuid(),kind:kind||"task",label:label||"",cells:{},afterId:"",lagWeeks:0};}
...
function kpiWeeks(kpi){
  ...
  while(cur<=end){
    ...
    weeks.push({monday:monday.toISOString().slice(0,10),sunday:sunday.toISOString().slice(0,10)});
        ...
        return <div key={ac.id} className="ac-item">
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{display:"flex",gap:6,marginBottom:5}}>
              <input type="date" value={ac.due||""} onChange={e=>updAction(ac.id,"due",e.target.value)} style={{flex:1,fontSize:11,padding:"3px 6px"}}/>
...
function TendersView({tenders,saveTenders,packages,people,tasks,saveTasks,contractors,pkgOwners,jumpTender,clearJumpTender,jumpFrom,onBack}){
  ...
  function updateStep(tdId,step,field,val){
    ...
    if(updatedTd&&step==="acc"&&field==="done"&&val&&!prevAccDone){
      var apprDueVal=addWorkingDays(val,14);
                  ...
                  <td style={{padding:"6px 8px",textAlign:"center"}}>
                    {(s.manual||s.key==="accSub")
                      ...
                      :<div style={{display:"flex",alignItems:"center",gap:3,justifyContent:"center"}}>
                        ...
                        {!s.overridden&&s.autoDate&&<span style={{fontSize:9,color:"#c9a84c",lineHeight:1}} title="Auto-calculated">⚙️</span>}
                      </div>}
                  </td>
          ...
          return <tr key={td.id} style={{cursor:"pointer"}} onClick={function(){setSelTender(td);}}>
            ...
            {TENDER_STEPS.filter(function(s){return s.key!=="process"&&s.key!=="bidders";}).map(function(s){
              if(s.key==="contract"){
                ...
                var autoReqTarget=ctAccApproval?addWorkingDays(ctAccApproval,3):"";
                var autoCircTarget=ctc.requestDone?addWorkingDays(ctc.requestDone,7):"";
                var autoSignAllTarget=ctc.circulateDone?addWorkingDays(ctc.circulateDone,14):"";
                var stage=0;
                var nextTarget="";
...
function SettingsView({tags,saveTags,people,savePeople,packages,savePackages,tagrules,saveTagrules,pkgrules,savePkgrules,apiKey,saveApiKey,improvements,saveImprovements,pkgOwners,savePkgOwners,pkgSubcontractors,savePkgSubcontractors,peopleEmails,savePeopleEmails,defaultCC,saveDefaultCC,peopleAccess,savePeopleAccess,zones,saveZones,zoneOwners,saveZoneOwners,userPrefs,saveUserPrefs,allData,onImport}){
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {(zones||[]).map(z=><div key={z} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",borderRadius:10,background:"#fafaf8",border:"1.5px solid #e8e6df"}}>
          ...
          <div style={{flex:1,display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:10,fontWeight:700,color:"#aaa",textTransform:"uppercase"}}>Responsible</span>
            <select value={(zoneOwners||{})[z]||""} onChange={function(e){var u=Object.assign({},zoneOwners||{});if(e.target.value)u[z]=e.target.value;else delete u[z];saveZoneOwners(u);}} style={{flex:1,padding:"4px 8px",fontSize:12,fontFamily:"inherit",border:"1px solid #e8e6df",borderRadius:6}}>
...
function GlobalView({tasks,trackers,tenders,contractors,people,packages,tags,saveTasks,saveTrackers,tagrules,pkgrules,jumpOwner,clearJump,onNavTender,memory,setMemory,peopleEmails,defaultCC,zones,actionsOnly,currentUser}){
  ...
  return <div>
    ...
    {selIds.length>0&&<div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",padding:"10px 14px",background:"#1c1c1e",borderRadius:10,marginBottom:10,position:"sticky",top:0,zIndex:40}}>
      ...
      <input type="date" value={bulkDue} onChange={function(e){setBulkDue(e.target.value);}} style={{padding:"4px 8px",fontSize:11,borderRadius:6,border:"none",width:"auto"}}/>
...
function MaterialsView({tenders,packages,people,saveTenders,onNavTender,memory,setMemory}){
  var mem=memory||{};
  const [fPkg,setFPkg]=useState(mem.fPkg||"all");
  const [fTender,setFTender]=useState(mem.fTender||"all");
  const [fStatus,setFStatus]=useState(mem.fStatus||"all");
        ...
        <table className="tbl" style={{width:"100%",borderCollapse:"collapse"}}>
          <thead>
            <tr>
              ...
              <th>MSS Status</th><th>MSS Target</th>
              <th>MAR Status</th><th>MAR Target</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(function(r,idx){
              ...
              return <tr key={r.td.id+"_"+r.mi}>
                ...
                <td><input type="date" value={r.mat.mssTarget||""} onChange={function(e){updMat(r.td,r.mi,"mssTarget",e.target.value);}} style={{fontSize:10,padding:"2px 5px",border:"1px solid #e8e6df",borderRadius:4}}/></td>
                <td>
                ...
                <td><input type="date" value={r.mat.marTarget||""} onChange={function(e){updMat(r.td,r.mi,"marTarget",e.target.value);}} style={{fontSize:10,padding:"2px 5px",border:"1px solid #e8e6df",borderRadius:4}}/></td>
              </tr>;
            })}
          </tbody>
        </table>
      ...
      {editMode
        ?<div style={{display:"flex",flexDirection:"column",gap:6}}>
          ...
          <div style={{display:"flex",gap:6}}>
            ...
            <input type="date" value={task.due||""} onChange={function(e){onUpdate({due:e.target.value});}} style={{flex:1,padding:"4px 6px",fontSize:11,borderRadius:5,border:"1px solid #ddd"}}/>
          </div>
          ...
          <div className="ac-meta">
            {task.due
              ?(function(){
                ...
                var isSoon=!isLate&&task.status!=="done"&&(function(){var d=new Date();d.setDate(d.getDate()+7);return task.due<=d.toISOString().slice(0,10);})();
...
function ScheduleView({curZone,schedules,saveSchedules,tasks,saveTasks,people,tags}){
  ...
  function shiftRowCascade(rows,rowId,deltaWeeks,wksList,visited){
    visited=visited||{};
    ...
    var out=rows.map(function(r){
      ...
      Object.keys(cells).forEach(function(wk){
        ...
        if(hasPlan){
          ...
          var tgt=wksList[i+deltaWeeks];
          if(tgt===undefined)return;   // pushed outside the visible window: dropped
    ...
    deps.forEach(function(d){out=shiftRowCascade(out,d.id,deltaWeeks,wksList,visited);});
    return out;
  }
  ...
  function rowActions(rowId){
    return (tasks||[]).filter(function(t){return t.scheduleRowRef===rowId&&t.status!=="done";});
  }
  ...
  function shiftRow(rowId,delta){
    ...
    upd(sc.id,{rows:shiftRowCascade((sc.rows||[]).slice(),rowId,delta,wksList,{})});
  }
      ...
      :<div>
        <div className="sched-noprint" style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end",marginBottom:10,padding:"10px 12px",background:"#fafaf8",borderRadius:8,border:"1px solid #e8e6df"}}>
          ...
          <div style={{flex:1,minWidth:130}}>
            ...
            <input type="date" value={sc.startDate||""} onChange={function(e){upd(sc.id,{startDate:e.target.value});}} style={{padding:"4px 8px",fontSize:12}}/>
          </div>
        ...
        <div className="sched-print-only">
          <div style={{fontSize:16,fontWeight:800}}>{sc.title}</div>
          <div style={{fontSize:11,color:"#555"}}>{curZone} · {fmtDate(wks[0]||sc.startDate)} → {fmtDate(wks[wks.length-1]||sc.startDate)} · printed {fmtDate(todayStr)}</div>
        </div>
        ...
        <div className="sched-noprint" style={{display:"flex",gap:10,alignItems:"center",marginBottom:8,flexWrap:"wrap"}}>
          ...
          <span style={{fontSize:10,color:"#888",paddingLeft:6,borderLeft:"1px solid #e0ddd6"}}>Use ◀ ▶ to shift a row — linked rows follow automatically (planned bars only)</span>
        ...
        <div className="sched-wrap" style={{overflowX:"auto",border:"1px solid #e8e6df",borderRadius:8,background:"#fff"}}>
          <table style={{borderCollapse:"collapse",width:"100%",fontSize:11}}>
            <thead>
              <tr>
                <th style={{position:"sticky",left:0,background:"#f5f4f0",zIndex:2,minWidth:180,textAlign:"left",padding:"6px 10px",borderBottom:"1.5px solid #e8e6df",fontSize:10,textTransform:"uppercase",color:"#888"}}>Task</th>
                <th className="sched-noprint" style={{background:"#f5f4f0",minWidth:150,textAlign:"left",padding:"6px 8px",borderBottom:"1.5px solid #e8e6df",fontSize:10,textTransform:"uppercase",color:"#888"}}>Starts after</th>
                {wks.map(function(wk){
                  var isCur=(function(){var d=new Date(wk);var e=new Date(wk);e.setDate(e.getDate()+6);return wk<=todayStr&&todayStr<=e.toISOString().slice(0,10);})();
                  return <th key={wk} style={{minWidth:38,padding:"4px 2px",borderBottom:"1.5px solid #e8e6df",borderLeft:"1px solid #f0ede6",background:isCur?"#fff8e1":"#f5f4f0",fontSize:8,color:isCur?"#f57f17":"#888",fontWeight:isCur?800:600}}>
                    {fmtDate(wk).slice(0,5)}
                  </th>;
                })}
                <th className="sched-noprint" style={{width:60,background:"#f5f4f0",borderBottom:"1.5px solid #e8e6df"}}></th>
              </tr>
            </thead>
            <tbody>
                ...
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  <td style={{position:"sticky",left:0,zIndex:1,background:isCat?"#f0ede6":"#fff",padding:"3px 10px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df"}}>
                    <div style={{display:"flex",alignItems:"center",gap:4}}>
                      ...
                      {warn&&<span
                        onMouseEnter={function(){setHoverRowId(row.id);}}
                        onMouseLeave={function(){setHoverRowId(null);}}
                        style={{position:"relative",cursor:"help",fontSize:13,lineHeight:1,color:warnColor,flexShrink:0}}>
                        {warn.severity==="blocking"?"🔴":"⚠️"}
                        {hoverRowId===row.id&&<span style={{position:"absolute",left:"50%",transform:"translateX(-50%)",top:"120%",zIndex:200,minWidth:230,maxWidth:340,background:"#1c1c1e",color:"#fff",padding:"8px 10px",borderRadius:8,boxShadow:"0 4px 16px rgba(0,0,0,.3)",fontSize:11,fontWeight:400,textAlign:"left",whiteSpace:"normal",display:"block"}}>
                          <span style={{display:"block",fontWeight:700,marginBottom:5,color:warnColor}}>{warn.acts.length} open action{warn.acts.length!==1?"s":""}{warn.blocking>0?" · "+warn.blocking+" blocking":""}{warn.late>0?" · "+warn.late+" late":""}</span>
                          {warn.acts.slice(0,6).map(function(a){
                            var isB=(a.tags||[]).includes("Blocking Point");
                            var isL=a.due&&a.due<today();
                            return <span key={a.id} style={{display:"block",padding:"3px 0",borderTop:"1px solid #444"}}>
                              <span style={{color:isB?"#ff8a80":isL?"#ffcc80":"#fff"}}>{isB?"🔴 ":""}{a.text}</span>
                              <span style={{display:"block",color:"#aaa",fontSize:10}}>{a.owner?a.owner.split(",")[0]:"no owner"}{a.due?" · due "+fmtDate(a.due)+(isL?" (late)":""):""}</span>
                            </span>;
                          })}
                          {warn.acts.length>6&&<span style={{display:"block",color:"#aaa",fontSize:10,paddingTop:4}}>+{warn.acts.length-6} more…</span>}
                        </span>}
                      </span>}
                      <button className="sched-noprint" onClick={function(){setLinkRowId(linkRowId===row.id?null:row.id);}}
                        title="Add or link actions to this row"
                        style={{background:"none",border:"1px solid #e8e6df",borderRadius:4,cursor:"pointer",color:linkRowId===row.id?"#1a73e8":"#ccc",fontSize:10,padding:"1px 5px",flexShrink:0}}>⚑</button>
                    </div>
                  </td>
                  <td className="sched-noprint" style={{padding:"3px 8px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df",background:isCat?"#f0ede6":"#fff",whiteSpace:"nowrap"}}>
                    ...
                    <button onClick={function(){shiftRow(row.id,-1);}} title="Shift 1 week earlier (cascades to linked rows)" style={{background:"none",border:"1px solid #e8e6df",borderRadius:4,cursor:"pointer",color:"#888",fontSize:10,padding:"1px 5px",marginLeft:3}}>◀</button>
                    <button onClick={function(){shiftRow(row.id,1);}} title="Shift 1 week later (cascades to linked rows)" style={{background:"none",border:"1px solid #e8e6df",borderRadius:4,cursor:"pointer",color:"#888",fontSize:10,padding:"1px 5px",marginLeft:2}}>▶</button>
                  </td>
                  {wks.map(function(wk){
                    var v=(row.cells||{})[wk]||"";
                    return <td key={wk} onClick={function(){if(!isCat)toggleCell(row,wk);}}
                      style={Object.assign({borderBottom:"1px solid #f5f4f0",borderLeft:"1px solid #f0ede6",cursor:isCat?"default":"pointer",height:22,padding:0},cellStyle(v))}></td>;
                  })}
                  <td className="sched-noprint" style={{textAlign:"center",borderBottom:"1px solid #f5f4f0",whiteSpace:"nowrap"}}>
                    <button onClick={function(){moveRow(row.id,-1);}} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:10,padding:"0 2px"}} title="Move up">▲</button>
                    <button onClick={function(){moveRow(row.id,1);}} style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:10,padding:"0 2px"}} title="Move down">▼</button>
                    <button onClick={function(){delRow(row.id);}} style={{background:"none",border:"none",cursor:"pointer",color:"#ddd",fontSize:11,padding:"0 2px"}} onMouseEnter={function(e){e.currentTarget.style.color="#c62828";}} onMouseLeave={function(e){e.currentTarget.style.color="#ddd";}}>🗑</button>
                  </td>
                </tr>;
              })}
            </tbody>
        ...
        {linkRowId&&(function(){
          ...
          return <div className="sched-noprint" style={{marginTop:10,padding:"12px 14px",background:"#f0f8ff",border:"1.5px solid #bbdefb",borderRadius:10}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
        ...
        <div className="sched-noprint" style={{display:"flex",gap:8,marginTop:10,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <input type="text" value={newRowLabel} onChange={function(e){setNewRowLabel(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")addRow();}} placeholder={newRowKind==="category"?"Category name, e.g. POOL":"Task name, e.g. Slab"} style={{flex:1,minWidth:180,padding:"5px 10px",fontSize:12}}/>
...
function ZoneView({tasks,saveTasks,rooms,saveRooms,zones,people,tags,memory,setMemory,peopleEmails,defaultCC,kpis,saveKpis,meetings,saveMeetings,zoneOwners,schedules,saveSchedules}){
  var mem=memory||{};
  ...
  function addTask(){
    ...
    var td={text:qText.trim(),due:qDue,owner:qOwner||((zoneOwners||{})[curZone]||""),tags:qTags,zone:curZone,importance:1,urgence:1};
    saveTasks([newTask(td),...(tasks||[])]);
  ...
  function addRoom(){
    ...
    saveRooms([...(rooms||[]),newRoom({name:newRoomName.trim(),zone:curZone})]);
    setNewRoomName("");
  }
  function delRoom(id){if(safeConfirm("Remove this room?"))saveRooms((rooms||[]).filter(function(r){return r.id!==id;}));}
  ...
  function buildZoneReport(){
    ...
    var lookaheadEnd=(function(){var d=new Date();d.setDate(d.getDate()+21);return d.toISOString().slice(0,10);})();
    ...
    var zoneResp=(zoneOwners||{})[curZone];
    var toNames=[...new Set([...ownerNames,...attendeeNames,...(zoneResp?[zoneResp]:[])])];
    var missing=[];
  ...
  return <div style={{padding:"16px 20px",overflowY:"auto",flex:1}}>
    <div className="page-hdr">
      <div>
        ...
        <div className="page-sub">Dedicated space for zone pilots — actions &amp; room-by-room progress{(zoneOwners||{})[curZone]?" · Responsible: "+((zoneOwners||{})[curZone]).split(",")[0]:""}</div>
      </div>
    ...
    {subTab==="actions"&&<div>
      <div className="card" style={{marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:currentMeeting?10:0,flexWrap:"wrap",gap:8}}>
          <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
            <div style={{fontWeight:700,fontSize:13}}>👥 Meeting attendance</div>
            {currentMeeting&&<input type="date" value={currentMeeting.date||""} onChange={function(e){setMeetingDate(e.target.value);}} style={{padding:"3px 7px",fontSize:11,border:"1px solid #e8e6df",borderRadius:5}}/>}
            {currentMeeting&&(function(){var l=attendanceLists(currentMeeting);var total=l.present.length+l.excused.length+l.absent.length;return <span style={{fontSize:11,color:"#888"}}>{l.present.length} present / {total} recorded</span>;})()}
          </div>
          <div style={{display:"flex",gap:6}}>
        ...
        {currentMeeting&&<div style={{display:"flex",flexDirection:"column",gap:4}}>
          {(people||[]).map(function(p){
            var st=(currentMeeting.attendance||{})[p]||"";
            var c=ownerColor(p);
            return <div key={p} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 8px",borderRadius:7,background:st?"#fafaf8":"transparent"}}>
              <span style={{flex:1,fontSize:12,fontWeight:st?600:400,color:st?c.accent:"#aaa"}}>{p.split(",")[0]}</span>
              {[{k:"present",label:"Present",col:"#2e7d32",bg:"#e8f5e9"},{k:"excused",label:"Excused",col:"#f57f17",bg:"#fff8e1"},{k:"absent",label:"Absent",col:"#c62828",bg:"#fce4ec"}].map(function(opt){
                var on=st===opt.k;
                return <button key={opt.k} onClick={function(){setAttendance(p,opt.k);}} style={{padding:"2px 10px",borderRadius:16,border:"1.5px solid "+(on?opt.col:"#ddd"),background:on?opt.bg:"#fff",color:on?opt.col:"#bbb",fontFamily:"inherit",fontSize:10,fontWeight:700,cursor:"pointer"}}>{opt.label}</button>;
              })}
            </div>;
          })}
        </div>}
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}}>
          <input type="date" value={qDue} onChange={function(e){setQDue(e.target.value);}} style={{padding:"4px 7px",fontSize:11,border:"1px solid #ddd",borderRadius:5}}/>
    ...
    {subTab==="rooms"&&<div>
      <div className="card" style={{marginBottom:14}}>
        <div style={{fontWeight:700,fontSize:13,marginBottom:8}}>＋ Add room to {curZone}</div>
        <div style={{display:"flex",gap:8}}>
        ...
        :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:10}}>
          {zoneRooms.map(function(r){
            ...
            return <div key={r.id} className="card" style={{marginBottom:0,padding:"12px 14px",borderColor:isBlocked?"#f48fb1":"#e8e6df",background:isBlocked?"#fff5f7":"#fff"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:isBlocked?8:0}}>
                ...
                <button onClick={function(){delRoom(r.id);}} style={{background:"none",border:"none",cursor:"pointer",color:"#ddd",fontSize:12}} onMouseEnter={function(e){e.currentTarget.style.color="#c62828";}} onMouseLeave={function(e){e.currentTarget.style.color="#ddd";}}>🗑</button>
              </div>
        ...
        {showKpiForm&&<div style={{marginTop:10}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
            <div style={{flex:1,minWidth:130}}>
              ...
              <input type="date" value={kpiStart} onChange={function(e){setKpiStart(e.target.value);}} style={{padding:"5px 8px",fontSize:12}}/>
            </div>
            <div style={{flex:1,minWidth:130}}>
              <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase"}}>End date</label>
              <input type="date" value={kpiEnd} onChange={function(e){setKpiEnd(e.target.value);}} style={{padding:"5px 8px",fontSize:12}}/>
            </div>
          </div>
            ...
            {isOpen&&<div style={{padding:"14px 16px",borderTop:"1.5px solid #f0ede6"}}>
              <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end",marginBottom:12,padding:"10px 12px",background:"#fafaf8",borderRadius:8,border:"1px solid #e8e6df"}}>
                ...
                <div style={{flex:1,minWidth:120}}>
                  ...
                  <input type="date" value={kpi.startDate||""} onChange={function(e){updateKpi(kpi.id,"startDate",e.target.value);}} style={{padding:"4px 8px",fontSize:12}}/>
                </div>
                <div style={{flex:1,minWidth:120}}>
                  <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase"}}>End</label>
                  <input type="date" value={kpi.endDate||""} onChange={function(e){updateKpi(kpi.id,"endDate",e.target.value);}} style={{padding:"4px 8px",fontSize:12}}/>
                </div>
              ...
              {(function(){
                ...
                if(remaining<=0){projFinishLabel="Target already reached";}
                  ...
                  projFinishLabel=fmtDate(projDate.toISOString().slice(0,10));
                }else{
                  projFinishLabel="No progress yet";
                }
                var isLateVsPlan=avgRate>0&&(function(){var d=new Date(latestDone.monday);d.setDate(d.getDate()+Math.ceil(remaining/avgRate)*7);return d.toISOString().slice(0,10)>kpi.endDate;})();
...
{subTab==="schedule"&&<ScheduleView curZone={curZone} schedules={schedules} saveSchedules={saveSchedules} tasks={tasks} saveTasks={saveTasks} people={people} tags={tags}/>}
...
function App(){
  ...
  useEffect(()=>{
    const load=async()=>{
      try{
        const [t,x,td,ct,p,pk,g,tr,pr,imp,corr,awn,pko,prefs,zn,rm,pkgsub,pemail,dcc,kp,pacc,mtg,zown,sched]=await Promise.all([
          ...
          cloudStore.get(KEYS.tags),cloudStore.get(KEYS.tagrules),cloudStore.get(KEYS.pkgrules),cloudStore.get(KEYS_IMP),cloudStore.get(KEYS_CORR),cloudStore.get(KEYS_AWN),cloudStore.get(KEYS.pkgowners),cloudStore.get(KEYS_PREFS),cloudStore.get(KEYS.zones),cloudStore.get(KEYS_ROOMS),cloudStore.get(KEYS.pkgsubcontractors),cloudStore.get(KEYS.peopleemails),cloudStore.get(KEYS.defaultcc),cloudStore.get(KEYS_KPIS),cloudStore.get(KEYS.peopleaccess),cloudStore.get(KEYS_MEETINGS),cloudStore.get(KEYS.zoneowners),cloudStore.get(KEYS_SCHEDULES)
        ]);
        if(t){setTasks(t);}
        else{
  ...
  if(!currentUser)return <UserLogin people={people} onLogin={function(name){setCurrentUser(name);window._currentUser={name:name};}}/>;  return <div className="layout">
    ...
    <div className="main-area">
      <div className="content">
        ...
        {view==="zone"&&<ZoneView tasks={tasks} saveTasks={saveT} rooms={rooms} saveRooms={saveRooms} zones={zones} people={people} tags={tags} memory={memoryFor("zone")} setMemory={function(p){setMemoryFor("zone",p);}} peopleEmails={peopleEmails} defaultCC={defaultCC} kpis={kpis} saveKpis={saveKpis} meetings={meetings} saveMeetings={saveMeetings} zoneOwners={zoneOwners} schedules={schedules} saveSchedules={saveSchedules}/>}
        {view==="materials"&&<MaterialsView tenders={tenders} packages={packages} people={people} saveTenders={saveTenders} onNavTender={navToTender} memory={memoryFor("materials")} setMemory={function(p){setMemoryFor("materials",p);}}/>}
