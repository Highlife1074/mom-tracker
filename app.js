      ...
      {effectiveNAV.map(function(n){
        ...
        return <React.Fragment key={n.id}>
          <button className={"navbtn"+(activeHere?" on":"")} title={kids?n.label+" — "+kids.map(function(k){return k.label;}).join(", "):n.label}
            onClick={function(){
              if(kids){setNavGroup(navGroup===n.id?"":n.id);if(!activeHere)setView(kids[0].id);}
              else setView(n.id);
            }}>
            <span style={{fontSize:20}}>{n.icon}</span>
            <span className="lbl">{n.label}</span>
            {kids&&<span style={{fontSize:7,lineHeight:1,marginTop:-1,opacity:.7}}>{(navGroup===n.id||activeHere)?"▾":"▸"}</span>}
          </button>
          {kids&&(navGroup===n.id||activeHere)&&kids.map(function(k){
