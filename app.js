...
function newSchedule(overrides){return Object.assign({id:uuid(),zone:"",title:"New schedule",startDate:today(),weeks:12,rows:[],holidayWeeks:[],createdAt:today(),updatedAt:today(),updatedBy:window._currentUser?window._currentUser.name:""},overrides||{});}
...
function ScheduleView({curZone,schedules,saveSchedules,tasks,saveTasks,people,tags}){
  ...
  function toggleHolidayWeek(wk){
    ...
    var i=hw.indexOf(wk);
    if(i>=0)hw.splice(i,1);else hw.push(wk);
    upd(sc.id,{holidayWeeks:hw});
  }
              ...
              <tr>
                ...
                {wks.map(function(wk){
                  ...
                  return <th key={wk} onClick={function(){toggleHolidayWeek(wk);}}
                    ...
                    style={{minWidth:38,padding:"4px 2px",borderBottom:"1.5px solid #e8e6df",borderLeft:"1px solid #f0ede6",cursor:"pointer",
              ...
              {(sc.rows||[]).map(function(row){
                ...
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  ...
                  {wks.map(function(wk){
                    var v=(row.cells||{})[wk]||"";
                    var hol=isHoliday(wk);
                    return <td key={wk} onClick={function(){if(!isCat&&!hol)toggleCell(row,wk);}}
                      title={hol?"Neutralised week":""}
                      style={Object.assign({borderBottom:"1px solid #f5f4f0",borderLeft:"1px solid #f0ede6",cursor:(isCat||hol)?"default":"pointer",height:22,padding:0},
                        hol?{background:"repeating-linear-gradient(45deg,#e8e4da,#e8e4da 3px,#dcd8ce 3px,#dcd8ce 6px)"}:cellStyle(v))}></td>;
                  })}
