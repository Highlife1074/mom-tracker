...
const TENDER_STEPS=[
  ...
  {key:"acc",label:"ACC/Aconex",opts:["—","N/A","Internal review ongoing","Pending client approval","Approved A","Approved B","Rejected","Not Approved"]},
  {key:"contract",label:"Contract",opts:["—","N/A","Request sent","In circulation","Signed"]},
  {key:"itp",label:"ITP",opts:["—","N/A","Not done","Pending Approval","Approved A","Approved B","Rejected","Not Approved"]},
  {key:"wms",label:"WMS",opts:["—","N/A","Not done","Pending Approval","Approved A","Approved B","Rejected","Not Approved"]}
];
...
const SEED_TAGS=["Contract","Design","HR","Letter","Procurement","Production","RFI","Top Management"];
...
function SubmissionSteps({td,TENDER_STEPS,updateStep,tenders,saveTenders,setSelTender}){
  ...
  return <div>
    ...
    {contractStep&&<div style={{background:"#fff",borderRadius:10,border:"1.5px solid #e8e6df",overflow:"hidden"}}>
      <div style={{padding:"8px 12px",background:"#f5f4f0",fontWeight:700,fontSize:13,borderBottom:"1.5px solid #e8e6df"}}>Contract</div>
      <table className="tbl" style={{fontSize:12}}>
        <thead style={{background:"#f5f4f0"}}><tr>
          <th>Status</th>
          <th style={{textAlign:"center",minWidth:160}}>Subcontract request target date</th>
          <th style={{textAlign:"center",minWidth:110}}>Date done</th>
          <th style={{textAlign:"center",minWidth:130}}>Date of signature</th>
          <th style={{minWidth:180}}>Comments</th>
        </tr></thead>
        <tbody><tr>
          <td>
            <select value={(td.steps||{}).contract||""} onChange={function(e){updateStep(td.id,"contract","status",e.target.value);}} style={{border:"none",background:"transparent",fontFamily:"inherit",fontSize:12,fontWeight:700,cursor:"pointer",outline:"none",color:(td.steps||{}).contract&&(td.steps||{}).contract!=="—"?"inherit":"#bbb"}}>
              {contractStep.opts.map(function(o){return <option key={o} value={o}>{o}</option>;})}
            </select>
          </td>
          <td style={{textAlign:"center"}}>
            <input type="date" value={((td.stepDates||{}).contract||{}).target||""} onChange={function(e){updateStep(td.id,"contract","target",e.target.value);}} style={{border:"1px solid #e8e6df",borderRadius:5,padding:"3px 6px",fontSize:11}}/>
          </td>
          <td style={{textAlign:"center"}}>
            <input type="date" value={((td.stepDates||{}).contract||{}).done||""} onChange={function(e){updateStep(td.id,"contract","done",e.target.value);}} style={{border:"1px solid #e8e6df",borderRadius:5,padding:"3px 6px",fontSize:11}}/>
          </td>
          <td style={{textAlign:"center"}}>
            <input type="date" value={((td.stepDates||{}).contract||{}).signature||""} onChange={function(e){var d=(tenders||[]).map(function(t){if(t.id!==td.id)return t;var sd=Object.assign({},t.stepDates||{});sd.contract=Object.assign({},sd.contract||{},{signature:e.target.value});return Object.assign({},t,{stepDates:sd});});saveTenders(d);setSelTender(d.find(function(t){return t.id===td.id;}));}} style={{border:"1px solid #e8e6df",borderRadius:5,padding:"3px 6px",fontSize:11}}/>
          </td>
          <td>
            <textarea value={(td.stepComments||{}).contract||""} onChange={function(e){updateStep(td.id,"contract","comment",e.target.value);}} placeholder="Add comment..." style={{border:"1px solid #e8e6df",borderRadius:5,padding:"3px 6px",fontSize:11,width:"100%",fontFamily:"inherit",resize:"vertical",minHeight:28,boxSizing:"border-box"}}/>
          </td>
        </tr></tbody>
      </table>
    </div>}
