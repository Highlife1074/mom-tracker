...
const SEED_TAGS=["Blocking Point","Warning","Contract","Design","FCR","HR","Letter","Procurement","Production","Quality External","Quality Podium","Quality Tower","RFI","Top Management"];
            ...
            {localTags.includes("Blocking Point")&&<div style={{padding:"8px",background:"#fff5f7",borderRadius:7,border:"1px solid #f48fb1"}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"flex-end",flexWrap:"wrap"}}>
                <div style={{flex:1,minWidth:130}}>
                  ...
                  <select value={localZone} onChange={function(e){var z=e.target.value;setLocalZone(z);if(onUpdate)onUpdate({zone:z,blockedRooms:[]},null,true);}} style={{padding:"3px 7px",fontSize:11,border:"1px solid #f48fb1",borderRadius:5,width:"100%"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
                ...
                {(function(){
                  ...
                  if(linked.length>0){
                    ...
                    var f=linked[0];
                    return <span title={"From the schedule: \""+(f.r.label||"task")+"\" in "+(f.sc.zone||"?")+" ("+f.sc.title+"). Change the start week there to move this date."}
                      style={{fontSize:11,fontWeight:800,color:"#00695c",background:"#e8f5e9",border:"1px solid #c8e6c9",borderRadius:5,padding:"2px 8px",cursor:"help"}}>
                      🗓 {fmtDate(td.startOnSite||f.r.startWeek)} <span style={{fontSize:9,fontWeight:600,color:"#4caf50"}}>· from schedule</span>
                    </span>;
                  }
...
function SettingsView({subList,saveSubList,contractors,tags,saveTags,people,savePeople,packages,savePackages,tagrules,saveTagrules,pkgrules,savePkgrules,apiKey,saveApiKey,improvements,saveImprovements,pkgOwners,savePkgOwners,pkgSubcontractors,savePkgSubcontractors,peopleEmails,savePeopleEmails,defaultCC,saveDefaultCC,peopleAccess,savePeopleAccess,durations,saveDurations,isAdmin,zones,saveZones,zoneOwners,saveZoneOwners,userPrefs,saveUserPrefs,allData,onImport}){
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
      {["tags","people","packages","zones","subcontractors","pkg-owners","cc-rules","report-cc","access","my-prefs","improvements","backup"].concat(isAdmin?["durations"]:[]).map(function(t){return <button key={t} className={"fchip"+(tab===t?" on":"")} onClick={function(){setTab(t);}} style={{textTransform:"capitalize"}}>{t==="cc-rules"?"CC Rules":t==="improvements"?"💡 Improvements":t==="pkg-owners"?"📦 Pkg Owners":t==="backup"?"💾 Backup":t==="my-prefs"?"👤 My Prefs":t==="subcontractors"?"👷 Subcontractors":t==="zones"?"🏢 Zones":t==="report-cc"?"📧 Report CC":t==="access"?"🔐 Access":t==="durations"?"⏱ Durations":t.charAt(0).toUpperCase()+t.slice(1)}</button>;})}
    </div>
    ...
    {tab==="subcontractors"&&<div className="card">
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
        {mergedSubs.map(function(v){
          ...
          return <div key={v} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 12px",borderRadius:20,background:fromCtr?"#e8f5e9":"#f0ede6",border:"1.5px solid "+(fromCtr?"#c8e6c9":"#e0ddd6")}}>
    ...
    <div style={{flex:1,minWidth:0}}>
      {editMode
        ?<div style={{display:"flex",flexDirection:"column",gap:6}}>
          ...
          <div>
            <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase",display:"block",marginBottom:2}}>🔗 Linked tender</label>
        ...
        :<div onClick={function(){setEditMode(true);}} style={{cursor:"pointer"}}>
          ...
          <div className="ac-meta">
            ...
            {task.tenderRef&&(function(){
              ...
              return td?<span style={{fontSize:10,fontWeight:700,color:"#1a73e8",background:"#e8f0fe",padding:"1px 7px",borderRadius:10}}>🔗 {td.title}</span>:null;
            })()}
          </div>
        </div>}
    </div>
    {!editMode&&<div style={{display:"flex",gap:4,flexShrink:0,alignItems:"flex-start"}}>
      {onAdopt&&<button className="btn btn-sm" onClick={onAdopt} title="Take this procurement action into the zone: it will then count in the zone reports and KPIs" style={{padding:"3px 7px",background:"#fff8e1",color:"#f57f17",border:"1px solid #ffe082",fontWeight:700}}>⬇ Adopt</button>}
...
function ScheduleView({curZone,schedules,saveSchedules,tasks,saveTasks,people,tags,zones,rooms,saveRooms,canEdit,roomBlockersOf,tenders,saveTenders,pkgOwners}){
  ...
  const [newActionSeverity,setNewActionSeverity]=useState("info");   // blocking | warning | info
  ...
  function procRisk(row){
    ...
    return{tender:td,delivery:proc.deliveryDate,startWeek:row.startWeek,late:late,tight:tight,weeksLate:weeksLate,feasible:feasible};
  }
  ...
  function sortCategories(){
    ...
    bs.blocks.sort(function(a,b){return (a.head.label||"").localeCompare(b.head.label||"",undefined,{numeric:true,sensitivity:"base"});});
    upd(sc.id,{rows:flattenBlocks(bs)});
  }
  ...
  function createLinkedAction(rowId,rowLabel,text,severity){
    if(!saveTasks||!text.trim())return;
    var tg=severity==="blocking"?["Blocking Point"]:severity==="warning"?["Warning"]:[];
    var sev=severity==="blocking"?3:severity==="warning"?2:1;
    saveTasks([newTask({text:text.trim(),zone:curZone,scheduleRowRef:rowId,tags:tg,importance:sev,urgence:sev,note:"Linked to schedule row: "+rowLabel}),...(tasks||[])]);
  }
  ...
  function planColor(row){
    ...
    if(row.group&&gc[row.group])return gc[row.group];
    return "#c9a84c";
  }
  ...
  var SCHED_PALETTE=["#c9a84c","#2e7d32","#7b1fa2","#00838f","#e65100","#5d4037","#c2185b","#455a64","#f9a825","#1565c0"];
  ...
  return <div>
    ...
    {!sc
      ...
      :<div>
        <div className="sched-noprint" style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end",marginBottom:10,padding:"10px 12px",background:"#fafaf8",borderRadius:8,border:"1px solid #e8e6df"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:4,paddingLeft:8,borderLeft:"1px solid #e0ddd6"}}>
            ...
            <input type="text" value={newGroup} onChange={function(e){setNewGroup(e.target.value);}}
              ...
              title="Quick add, kept on this schedule only. The shared list lives in Settings > Subcontractors."
              placeholder="+ one-off" style={{width:80,padding:"4px 6px",fontSize:10}}/>
          </div>
        ...
        <div className="sched-wrap" ref={wrapRef} onScroll={function(){syncScroll("wrap");}} style={{overflowX:"auto",overflowY:"auto",maxHeight:"62vh",border:"1px solid #e8e6df",borderRadius:"8px 8px 0 0",background:"#fff"}}>
            ...
            <tbody>
              ...
              {visibleRows.map(function(row){
                ...
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  <td style={{position:"sticky",left:0,zIndex:1,background:isSel?"#e8f0fe":isCat?"#f0ede6":"#fff",padding:"3px 10px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df"}}>
                    <div style={{display:"flex",alignItems:"center",gap:3}}>
                      ...
                      <input type="text" value={row.label||""} onChange={function(e){updRow(row.id,{label:e.target.value});}}
                        style={{flex:1,minWidth:0,border:"none",background:"transparent",fontSize:isCat?12:11,fontWeight:isCat?800:500,textTransform:isCat?"uppercase":"none",color:isCat?"#555":"#333",padding:"2px 0",paddingLeft:isCat?0:12,outline:"none",fontFamily:"inherit"}}/>
                      {warn&&<span
                      ...
                      {(function(){
                        ...
                        return <span style={{display:"inline-flex",alignItems:"center",gap:2,flexShrink:0}}>
                          <span title={(pr.late?"Procurement LATE: ":"Tight: ")+pr.tender.title+" — delivery "+fmtDate(pr.delivery)+" vs start week of "+fmtDate(pr.startWeek)}
                            style={{fontSize:11,cursor:"help"}}>{pr.late?"🚚":"⏳"}</span>
                          {pr.late&&pr.feasible&&<span
                            ...
                            style={{fontSize:9,fontWeight:800,padding:"1px 4px",borderRadius:8,background:"#fff3e0",color:"#e65100",border:"1px solid #ffcc80",cursor:canEdit?"pointer":"help",whiteSpace:"nowrap"}}>
                  ...
                  <td style={{padding:"3px 6px",borderBottom:"1px solid #f5f4f0",background:isCat?"#f0ede6":"#fff",whiteSpace:"nowrap"}}>
                    {isCat
                      ?<span style={{color:"#ddd",fontSize:10}}>—</span>
                      :canEdit
                        ?<div style={{display:"flex",alignItems:"center",gap:3}}>
                        <span className="sched-noprint" title={row.color?"Colour set on this task — click to cycle, right-click to clear":row.group?"Colour of "+row.group+" — click to cycle it for every task of this subcontractor, right-click to clear":"Pick a subcontractor first, or click to colour this single task"}
                          onClick={function(e){
                            var pal=SCHED_PALETTE;
                            if(e.shiftKey||!row.group){
                              var i=pal.indexOf(row.color||"");
                              updRow(row.id,{color:pal[(i+1)%pal.length]});
                            }else{
                              var gc=Object.assign({},(sc.groupColors)||{});
                              var j=pal.indexOf(gc[row.group]||"");
                              gc[row.group]=pal[(j+1)%pal.length];
                              upd(sc.id,{groupColors:gc});
                            }
                          }}
                          onContextMenu={function(e){
                            e.preventDefault();
                            if(row.color){updRow(row.id,{color:""});return;}
                            if(row.group){var gc=Object.assign({},(sc.groupColors)||{});delete gc[row.group];upd(sc.id,{groupColors:gc});}
                          }}
                        ...
                        <select value={row.group||""} onChange={function(e){updRow(row.id,{group:e.target.value});}}
                  ...
                  {wks.map(function(wk){
                    ...
                    var isFeasible=rowRisk&&rowRisk.late&&rowRisk.feasible===wk;
                    ...
                    return <td key={wk}
                      ...
                      style={Object.assign({borderBottom:"1px solid #f5f4f0",borderLeft:"1px solid #f0ede6",cursor:(isCat||hol)?"default":"pointer",height:22,padding:0,textAlign:"center",fontSize:8,fontWeight:800,color:"#fff",textShadow:"0 0 2px rgba(0,0,0,.5)"},
                        ...
                        isFeasible?{boxShadow:"inset 0 0 0 2px #e65100"}:null)}>
                      {hasWp?wpVal:isFeasible&&!v?<span style={{color:"#e65100",fontSize:9,textShadow:"none"}}>▶</span>:""}
                    </td>;
                  })}
                </tr>;
              })}
        ...
        <div className="sched-noprint sched-mirror" ref={mirrorRef} onScroll={function(){syncScroll("mirror");}}
          style={{display:mirrorBox.show?"block":"none",position:"fixed",left:mirrorBox.left,width:mirrorBox.width,
            bottom:12,zIndex:900,overflowX:"auto",overflowY:"hidden",height:18,
          ...
          return <div className="sched-noprint" onClick={function(e){e.stopPropagation();}}
            ...
            <div style={{marginBottom:10,padding:"8px 9px",border:"1.5px solid #e0ddd6",borderRadius:7,background:"#fafaf8"}}>
              ...
              {(row.prereqs||[]).map(function(p,pi){
                return <div key={pi} style={{display:"flex",gap:4,alignItems:"center",marginBottom:4}}>
                  <input type="text" value={p.label||""} placeholder="e.g. permit issued"
                    onChange={function(e){var l=(row.prereqs||[]).slice();l[pi]=Object.assign({},p,{label:e.target.value});updPrereqs(row.id,l);}}
                    style={{flex:1,minWidth:0,padding:"3px 6px",fontSize:10}}/>
                  <input type="date" min="1990-01-01" max="2200-12-31" value={p.date||""}
                    onChange={function(e){var l=(row.prereqs||[]).slice();l[pi]=Object.assign({},p,{date:e.target.value});updPrereqs(row.id,l);}}
                    style={{width:118,padding:"3px 4px",fontSize:10}}/>
                  <button onClick={function(){var l=(row.prereqs||[]).slice();l[pi]=Object.assign({},p,{confirmed:!p.confirmed});updPrereqs(row.id,l);}}
                    title={p.confirmed?"Date confirmed — click to put it back to TBC":"Date still to be confirmed — click to confirm it"}
                    style={{fontSize:9,fontWeight:800,padding:"3px 6px",borderRadius:10,cursor:"pointer",fontFamily:"inherit",flexShrink:0,
                      border:"1px solid "+(p.confirmed?"#c8e6c9":"#ffe082"),background:p.confirmed?"#e8f5e9":"#fff8e1",color:p.confirmed?"#2e7d32":"#f57f17"}}>
                    {p.confirmed?"✓ OK":"TBC"}</button>
                  <button onClick={function(){updPrereqs(row.id,(row.prereqs||[]).filter(function(_,k){return k!==pi;}));}}
                    style={{background:"none",border:"none",color:"#ccc",cursor:"pointer",fontSize:12,flexShrink:0}}>×</button>
                </div>;
              })}
              <button className="btn btn-sm" onClick={function(){updPrereqs(row.id,[...(row.prereqs||[]),{label:"",date:"",confirmed:false}]);}}
                style={{padding:"2px 8px",fontSize:10}}>＋ Add prerequisite</button>
              {(function(){
          ...
          return <div className="sched-noprint" onClick={function(e){e.stopPropagation();}}
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
              <input type="text" value={newActionText} onChange={function(e){setNewActionText(e.target.value);}}
                onKeyDown={function(e){if(e.key==="Enter"){createLinkedAction(row.id,row.label,newActionText,newActionSeverity);setNewActionText("");}}}
                placeholder="New action for this task…" style={{flex:1,minWidth:200,padding:"5px 10px",fontSize:12}}/>
              {[{k:"blocking",lbl:"🔴 Blocking",c:"#c62828",bg:"#fce4ec"},{k:"warning",lbl:"🟠 Warning",c:"#ef6c00",bg:"#fff3e0"},{k:"info",lbl:"ℹ️ Info",c:"#1565c0",bg:"#e8f0fe"}].map(function(o){
                var on=newActionSeverity===o.k;
                return <button key={o.k} onClick={function(){setNewActionSeverity(o.k);}}
                  title={o.k==="blocking"?"Stops the task — turns the row red":o.k==="warning"?"Needs attention but does not stop the work — turns the row orange":"Information only — no marker on the row"}
              ...
              <button className="btn btn-sm btn-gold" disabled={!newActionText.trim()}
                onClick={function(){createLinkedAction(row.id,row.label,newActionText,newActionSeverity);setNewActionText("");}}>＋ Create</button>
            </div>
  ...
  return <div style={{padding:"16px 20px",overflowY:"auto",flex:1}}>
    ...
    {subTab==="actions"&&<div>
      ...
      {incomingActions.length>0&&<div style={{marginBottom:18,border:"1.5px solid #ffe082",borderRadius:10,background:"#fffdf5",padding:"10px 12px"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
          <span style={{padding:"3px 12px",borderRadius:20,background:"#fff8e1",color:"#f57f17",fontWeight:800,fontSize:12}}>⬇ FROM PROCUREMENT</span>
    ...
    const load=async()=>{
      ...
      try{
        ...
        if(g){
          ...
          var gg=g.indexOf("Warning")>=0?g:["Warning"].concat(g);
          setTags(gg);
          if(gg!==g)sync(KEYS.tags,gg);
        }
        else{
