...
function ZoneView({tasks,saveTasks,rooms,saveRooms,zones,people,tags,memory,setMemory,peopleEmails,defaultCC}){
  ...
  function buildZoneReport(){
    ...
    var lines=[curZone.toUpperCase()+" ZONE — ACTION REPORT","="+("=".repeat(50)),dateStr,""];
    ...
    lines.push("Total: "+filteredActions.length+" action"+(filteredActions.length!==1?"s":"")+" — "+pending+" pending, "+done+" done");
    lines.push("");

    orderedCats.forEach(function(cat){
      var items=groups[cat];
      lines.push((cat==="Blocking Point"?"🚧 ":"")+cat.toUpperCase()+" ("+items.length+")");
      lines.push("-".repeat(40));
      items.forEach(function(t){
        var st=STATUS_ICONS[t.status]||"•";
        var owner=t.owner?" — "+t.owner.split(",")[0]:"";
        var due=t.due?" [due "+fmtDate(t.due)+"]":"";
        var rooms="";
        if((t.tags||[]).includes("Blocking Point")){
          if(t.blockedRooms==="all")rooms=" (ALL rooms)";
          else if(Array.isArray(t.blockedRooms)&&t.blockedRooms.length>0){
            var names=t.blockedRooms.map(function(rid){var rm=zoneRooms.find(function(x){return x.id===rid;});return rm?rm.name:"";}).filter(Boolean);
            if(names.length>0)rooms=" (rooms: "+names.join(", ")+")";
          }
        }
        lines.push("  "+st+" "+t.text+owner+due+rooms);
      });
      lines.push("");
    });
    ...
    if(zoneRooms.length>0){
      ...
      lines.push("ROOM STATUS ("+zoneRooms.length+" rooms, "+blockedRoomsList.length+" blocked)");
      lines.push("-".repeat(40));
      if(blockedRoomsList.length===0){
        ...
        blockedRoomsList.forEach(function(r){
          ...
          lines.push("  🔴 "+r.name+": "+blockers.map(function(b){return b.text;}).join("; "));
        });
      }
      lines.push("");
    }
