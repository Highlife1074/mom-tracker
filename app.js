...
function newSchedule(overrides){return Object.assign({id:uuid(),zone:"",title:"New schedule",startDate:today(),weeks:12,rows:[],holidayWeeks:[],groups:[],links:[],createdAt:today(),updatedAt:today(),updatedBy:window._currentUser?window._currentUser.name:""},overrides||{});}
function newScheduleRow(kind,label){return{id:uuid(),kind:kind||"task",label:label||"",cells:{},afterId:"",lagWeeks:0,roomId:"",progress:"",weekProgress:{},group:"",startWeek:"",endWeek:"",qty:"",unit:"",tenderRef:"",links:[]};}
...
function toISO(dt){
  try{
    ...
    // Was "return toISO(t)" — an infinite recursion whose stack overflow was swallowed
    // by the catch below, so every computed date in the app silently became "".
...
// Palette behind the s-* chip classes, so a <select> can carry the same colour
// as the chip shown in the tender list.
const STEP_PALETTE={
  "s-approved-a":{bg:"#e8f5e9",color:"#2e7d32",border:"#c8e6c9"},
  "s-pending":{bg:"#f3e5f5",color:"#7b1fa2",border:"#ce93d8"},
  "s-notdone":{bg:"#fce4ec",color:"#c62828",border:"#f48fb1"},
  "s-na":{bg:"#f5f5f5",color:"#aaa",border:"#ddd"},
  "s-default":{bg:"#f5f4f0",color:"#888",border:"#e0ddd6"}
};
function stepPalette(val){return STEP_PALETTE[tenderStepClass("",val)]||STEP_PALETTE["s-default"];}
...
function newTender(overrides){return Object.assign({id:uuid(),title:"",package:"",ownerPackage:"",ownerTender:"",createdAt:today(),targetDate:"",steps:{bidders:"",pkg:"",process:"",acc:"",contract:"",mar:"",itp:"",wms:""},stepDates:{bidders:{target:"",done:""},pkg:{target:"",done:""},process:{target:"",done:""},acc:{target:"",done:"",approval:""},contract:{target:"",done:""},mar:{target:"",done:"",approval:""},itp:{target:"",done:"",approval:""},wms:{target:"",done:"",approval:""}},stepComments:{bidders:"",pkg:"",process:"",acc:"",contract:"",mar:"",itp:"",wms:""},stepLinks:{bidders:[],pkg:[],process:[],acc:[],contract:[],mar:[],itp:[],wms:[]},description:"",budget:"",instructionAmount:"",currency:"EUR",nextStep:"",cancelled:false},overrides||{});}
...
function ActionItem({task,onStatusChange,onUpdate,onDelete,people,packages,tags,tenders,contractors,showCreated,onNavTender,zones,onOpenRooms}){
  ...
  // onUpdate accepts either (field,val) or a single object of fields. Two separate
  // calls in one handler both read the same pre-render task list, so the second one
  // silently discards the first — the zone of a blocking point used to vanish that way.
  function upd(field,val){if(onUpdate)onUpdate(field,val,true);}
  function updAll(fields){if(onUpdate)onUpdate(fields,null,true);}
      ...
      <button className="btn btn-sm" onClick={function(){upd("links",[...(task.links||[]),{label:"",url:""}]);}} style={{fontSize:10,padding:"2px 8px"}}>＋ Add link</button>
            ...
            {(localTags.includes("RFI")||localTags.includes("FCR"))&&<div style={{display:"flex",gap:8,padding:"8px",background:"#fff8f0",borderRadius:7,border:"1px solid #fed7aa"}}>
              ...
              <div style={{flex:1}}>
                ...
                <input type="date" min="1990-01-01" max="2200-12-31" value={localRfiDue} onChange={function(e){setLocalRfiDue(e.target.value);upd("rfiDue",e.target.value);}} style={{padding:"3px 7px",fontSize:11,border:"1px solid #fed7aa",borderRadius:5,width:"100%"}}/>
              </div>
            </div>}
            {localTags.includes("Blocking Point")&&<div style={{padding:"8px",background:"#fff5f7",borderRadius:7,border:"1px solid #f48fb1"}}>
              <div style={{fontSize:9,fontWeight:800,color:"#c62828",textTransform:"uppercase",letterSpacing:".4px",marginBottom:5}}>🚧 Blocking point — which rooms are affected?</div>
              <div style={{display:"flex",gap:8,alignItems:"flex-end",flexWrap:"wrap"}}>
                <div style={{flex:1,minWidth:130}}>
                  <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase",display:"block",marginBottom:2}}>Zone</label>
                  <select value={task.zone||""} onChange={function(e){updAll({zone:e.target.value,blockedRooms:[]});}} style={{padding:"3px 7px",fontSize:11,border:"1px solid #f48fb1",borderRadius:5,width:"100%"}}>
                    <option value="">— pick a zone —</option>
                    {(zones||[]).map(function(z){return <option key={z} value={z}>{z}</option>;})}
                  </select>
                </div>
                {task.zone&&<button className="btn btn-sm" onClick={function(){if(onOpenRooms)onOpenRooms(task);}}
              ...
              {!task.zone&&<div style={{fontSize:10,color:"#888",marginTop:4}}>Pick a zone first — the action will then show up in that zone's report and turn the selected rooms red.</div>}
            </div>}
      ...
      {mats.map(function(mat,mi){
        return <div key={mat.id||mi} style={{padding:"8px 10px",background:"#fafaf8",borderRadius:7,border:"1px solid #f0ede6",marginBottom:8}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <div style={{flex:1,minWidth:280}}><DocBlock mat={mat} mi={mi} kind="MSS" color="#1a73e8" bg="#e8f0fe"/></div>
            <div style={{flex:1,minWidth:280}}><DocBlock mat={mat} mi={mi} kind="MAR" color="#7b1fa2" bg="#f3e5f5"/></div>
          </div>
        </div>;
      })}
    </div>}
...
function SubmissionSteps({td,TENDER_STEPS,updateStep,tenders,saveTenders,setSelTender,tasks,saveTasks,pkgOwners}){
  ...
  return <div>
            ...
            {s.special==="bidders"
              ?<div style={{display:"flex",flexDirection:"column",gap:4}}>
                <select value={val} onChange={function(e){updateStep(td.id,s.key,"status",e.target.value);}}
                  title={"Status: "+(val||"not set")}
                  style={{fontFamily:"inherit",fontSize:12,fontWeight:700,cursor:"pointer",outline:"none",borderRadius:6,padding:"4px 8px",width:"100%",
                    background:stepPalette(val).bg,color:val&&val!=="—"?stepPalette(val).color:"#bbb",border:"1.5px solid "+stepPalette(val).border}}>
          ...
          <td style={{minWidth:200}}>
            {showApproval
              ?<div style={{display:"flex",flexDirection:"column",gap:4}}>
                ...
                <select value={getApprovalStatus(s.key)} onChange={function(e){updateStep(td.id,s.key,"approvalStatus",e.target.value);}}
                  title={"Approval: "+(getApprovalStatus(s.key)||"not set")}
                  style={{borderRadius:6,padding:"4px 7px",fontSize:11,fontFamily:"inherit",fontWeight:700,width:"100%",
                    background:stepPalette(getApprovalStatus(s.key)).bg,
                    color:getApprovalStatus(s.key)&&getApprovalStatus(s.key)!=="—"?stepPalette(getApprovalStatus(s.key)).color:"#bbb",
                    border:"1.5px solid "+stepPalette(getApprovalStatus(s.key)).border}}>
    ...
    {contractStep&&(function(){
      ...
      var _acc=(td.stepDates||{}).acc||{};
      var accApproval=_acc.approval||"";
      // The chain has to be able to run forward before anything has actually happened,
      // otherwise every target below the first stays empty until the step above is ticked
      // done. So each step falls back to the best forecast available, in order of
      // decreasing certainty: actual date, then target date. This is the same rule
      // calcProcurement already applies for the Procurement Timeline.
      var accApprBasis=accApproval||addWorkingDays(_acc.done||_acc.target||"",getDur("accApproval"));

      // Auto target dates (never overwrite existing manual values)
      var autoRequestTarget=accApprBasis?addWorkingDays(accApprBasis,getDur("accToRequest")):"";
      var requestTarget=ct.requestTarget||autoRequestTarget;

      var circulateBasis=ct.requestDone||requestTarget;
      var autoCirculateTarget=circulateBasis?addWorkingDays(circulateBasis,getDur("requestToCirculate")):"";
      var circulateTarget=ct.circulateTarget||autoCirculateTarget;

      var signedAllBasis=ct.circulateDone||circulateTarget;
      var autoSignedAllTarget=signedAllBasis?addWorkingDays(signedAllBasis,getDur("circulateToSign")):"";
      var signedAllTarget=ct.signedAllTarget||autoSignedAllTarget;

      var autoSignedTarget=accApprBasis?addWorkingDays(accApprBasis,getDur("accToSigned")):"";
          ...
          <tbody>
            <tr style={rowStyle}>
              ...
              <td style={{textAlign:"center"}}>
                ...
                {targetHint(autoRequestTarget,ct.requestTarget,accApproval?"Auto: ACC approval+3d":"Forecast: ACC approval target+3d")}
              </td>
              <td style={{textAlign:"center"}}>
            ...
            <tr style={rowStyle}>
              ...
              <td style={{textAlign:"center"}}>
                ...
                {targetHint(autoCirculateTarget,ct.circulateTarget,ct.requestDone?"Auto: request done+7d":"Forecast: request target+7d")}
              </td>
              <td style={{textAlign:"center"}}>
            ...
            <tr style={rowStyle}>
              ...
              <td style={{textAlign:"center"}}>
                <input type="date" min="1990-01-01" max="2200-12-31" value={signedAllTarget}
                  onChange={function(e){setCtField("signedAllTarget",e.target.value);}}
                  style={targetStyle(autoSignedAllTarget,ct.signedAllTarget)}/>
                {targetHint(autoSignedAllTarget,ct.signedAllTarget,ct.circulateDone?"Auto: circulate done+14d":"Forecast: circulate target+14d")}
              </td>
              <td style={{textAlign:"center"}}>
                <input type="date" min="1990-01-01" max="2200-12-31" value={ct.signedAllDone||""}
                  onChange={function(e){
                    setCtField("signedAllDone",e.target.value);
                    if(e.target.value){
                      setCtField("signedDone",e.target.value);
                      closeAction("Contract to be signed internally and by the sub — "+td.title);
                    }else{
                      setCtField("signedDone","");
                      reopenAction("Contract to be signed internally and by the sub — "+td.title);
                      if(td.hasSD)removeAction("SD to be submitted: "+td.title);
                    }
                  }}
...
function TendersView({tenders,saveTenders,packages,people,tasks,saveTasks,contractors,pkgOwners,jumpTender,clearJumpTender,jumpFrom,onBack}){
  ...
  function updateStep(tdId,step,field,val){
    ...
    if(updatedTd&&step==="acc"&&field==="approvalStatus"&&val&&val!=="—"){
      closeActionForTender(updatedTd,apprText);
    }
  ...
  if(selTender){
    ...
    // Several fields at once. Calling updTd twice in one handler loses the first write:
    // both map over the same pre-render `tenders`, so the second result overwrites it.
    function updTdMany(fields){var d=(tenders||[]).map(function(t){return t.id!==td.id?t:Object.assign({},t,fields);});saveTenders(d);setSelTender(d.find(function(t){return t.id===td.id;}));}
    ...
    return <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{position:"sticky",top:0,zIndex:30,background:"#f4f3f0",display:"flex",gap:6,paddingBottom:6,paddingTop:2,marginBottom:6}}>
          {onBack&&<button className="btn btn-sm" onClick={onBack} style={{flexShrink:0,color:"#3949ab",border:"1px solid #3949ab",boxShadow:"0 1px 4px rgba(0,0,0,.08)"}}>← Back to {jumpFrom}</button>}
          <button className="btn btn-sm" onClick={function(){setSelTender(null);onBack?null:null;}} style={{flexShrink:0,boxShadow:"0 1px 4px rgba(0,0,0,.08)"}}>← Tenders</button>
        </div>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:6,alignItems:"center"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
                ...
                <input type="date" min="1990-01-01" max="2200-12-31" value={td.startOnSite||""} onChange={function(e){updTd("startOnSite",e.target.value);}} style={{fontSize:11,border:"1px solid #e8e6df",borderRadius:5,padding:"2px 6px"}}/>
                {(function(){
              ...
              <label style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer",textTransform:"none",letterSpacing:"normal",fontWeight:500,fontSize:11}}>
                <input type="checkbox" checked={isReleased} onChange={function(e){updTd("released",e.target.checked);}} style={{width:13,height:13}} disabled={!!td.cancelled}/>
                <span style={{color:isReleased?"#2e7d32":"#888"}}>✅ Budget Release</span>
              </label>
              <label title="Keep the tender on record but take its budget and cost out of every total"
                style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer",textTransform:"none",letterSpacing:"normal",fontWeight:700,fontSize:11,
                  padding:"2px 9px",borderRadius:12,border:"1.5px solid "+(td.cancelled?"#e65100":"#ddd"),background:td.cancelled?"#fff3e0":"#fff"}}>
                <input type="checkbox" checked={!!td.cancelled} onChange={function(e){updTd("cancelled",e.target.checked);}} style={{width:13,height:13}}/>
                <span style={{color:td.cancelled?"#e65100":"#888"}}>🚫 Cancelled</span>
              </label>
            </div>
          </div>
        ...
        {td.cancelled&&<div style={{padding:"8px 12px",background:"#fff3e0",border:"1.5px solid #ffb74d",borderRadius:8,marginBottom:10,fontSize:12,color:"#e65100",fontWeight:700}}>
          🚫 Cancelled tender — kept on record, but its budget and cost are excluded from every total.
        </div>}

        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12,opacity:td.cancelled?0.55:1}}>
        ...
        {(function(){
          ...
          function setSdStatus(val){
            var appr=val==="approved"?"approved":val==="pending approval"?"pending approval":"";
            updTdMany({sdStatus:val,sdApprovalStatus:appr});
            sdStepAction(val);
          }
      ...
      <div className="card">
        ...
        {(function(){
          ...
          var todayStr=today();
          return <div className="card" style={{marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:procOpen?10:0,cursor:"pointer"}} onClick={function(){setProcOpen(!procOpen);}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <span style={{fontSize:12,color:"#aaa"}}>{procOpen?"▾":"▸"}</span>
                <div style={{fontWeight:700,fontSize:13}}>📊 Procurement Timeline</div>
                {!procOpen&&proc.deliveryDate&&<span style={{fontSize:11,color:"#1a73e8",fontWeight:600}}>→ delivery {fmtDate(proc.deliveryDate)}{proc.margin!==null&&proc.margin!==undefined&&<span style={{marginLeft:6,color:proc.margin<0?"#c62828":proc.margin<7?"#f57f17":"#2e7d32",fontWeight:700}}>{proc.margin>0?"+":""}{proc.margin}d margin</span>}</span>}
              </div>
              {procOpen&&<div style={{display:"flex",gap:8,alignItems:"center"}} onClick={function(e){e.stopPropagation();}}>
                {(function(){
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
              <div style={{flex:1,minWidth:110,padding:"7px 10px",borderRadius:8,background:"#f8f7f4",border:"1px solid #ede9e3"}}>
                <div style={{fontSize:9,color:"#888",fontWeight:700,marginBottom:2}}>PROCUREMENT START{proc.theoretical&&<span title="Worked back from the site start date — no ACC date entered yet" style={{marginLeft:4,padding:"0 4px",borderRadius:4,background:"#fff8e1",color:"#b45309"}}>THEO</span>}</div>
                  ...
                  <td style={{padding:"6px 8px",textAlign:"center"}}>
                    {(s.manual||s.key==="accSub")
                      ...
                      :<div style={{display:"flex",alignItems:"center",gap:3,justifyContent:"center"}}>
                        <input type="date" min="1990-01-01" max="2200-12-31" value={s.date||""} onChange={function(e){
                          var d=(tenders||[]).map(function(t){
                            if(t.id!==td.id)return t;
                            var ov2=Object.assign({},t.procOverrides||{});
                            ov2[s.key]=e.target.value;
                            return Object.assign({},t,{procOverrides:ov2});
                          });
                          saveTenders(d);setSelTender(d.find(function(t){return t.id===td.id;}));
                        }} style={{fontSize:10,padding:"2px 4px",border:"1px solid "+(s.overridden?"#ffe082":isOverdue?"#f48fb1":"#e8e6df"),borderRadius:4,background:s.overridden?"#fffde7":"#fff",color:isOverdue?"#c62828":"#333"}}/>
                        {s.overridden&&<span title="Modified — click to reset to auto date" onClick={function(){
                          var d=(tenders||[]).map(function(t){
                            if(t.id!==td.id)return t;
                            var ov2=Object.assign({},t.procOverrides||{});
                            delete ov2[s.key];
                          ...
                          saveTenders(d);setSelTender(d.find(function(t){return t.id===td.id;}));
                        }} style={{cursor:"pointer",fontSize:13,lineHeight:1}}>⚠️</span>}
                        {s.fromSdPanel&&<span style={{fontSize:9,color:"#00695c",lineHeight:1,fontWeight:700}} title="Taken from the Shop Drawing panel's Target date">SD</span>}
                        {!s.overridden&&!s.fromSdPanel&&s.autoDate&&<span style={{fontSize:9,color:"#c9a84c",lineHeight:1}} title="Auto-calculated">⚙️</span>}
                      </div>}
                  </td>
  ...
  setTimeout(function(){
    ...
    {(function(){
      // Cancelled tenders stay in the list but never count towards money.
      var liveTenders=filtered.filter(function(t){return !t.cancelled;});
      var cancelledCount=filtered.length-liveTenders.length;
      var releasedTenders=liveTenders.filter(function(t){return t.released;});
      ...
      var totAllBudget=liveTenders.reduce(function(s,t){return s+Number(t.budget||0);},0);
      ...
      return <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14,alignItems:"stretch"}}>
        <div className="card" style={{flex:1,minWidth:100,marginBottom:0,padding:"10px 12px"}}>
          ...
          <div style={{fontSize:9,color:"#aaa"}}>{liveTenders.length} tenders{cancelledCount>0?" · "+cancelledCount+" cancelled excluded":""}</div>
        </div>
    ...
    {filtered.length===0
      ...
      :<table className="tbl">
        ...
        <tbody>{filtered.map(function(td){
          var steps=td.steps||{};
          return <tr key={td.id} style={Object.assign({cursor:"pointer"},td.cancelled?{background:"#fff3e0",color:"#b45309"}:{})} onClick={function(){setSelTender(td);}}>
            <td style={{fontWeight:600}}>
              <div style={{display:"flex",alignItems:"center",gap:5}}>
                {td.released&&!td.cancelled&&<span title="Released — included in totals" style={{width:7,height:7,borderRadius:"50%",background:"#2e7d32",flexShrink:0,display:"inline-block"}}/>}
                {td.cancelled&&<span title="Cancelled — excluded from all totals" style={{fontSize:9,fontWeight:800,padding:"1px 6px",borderRadius:7,background:"#ffe0b2",color:"#e65100",flexShrink:0}}>🚫 CANCELLED</span>}
                <span style={td.cancelled?{textDecoration:"line-through",opacity:.75}:null}>{td.title}</span>
              </div>
            </td>
        ...
        <tfoot>
          <tr style={{background:"#fafaf8",borderTop:"2px solid #e8e6df"}}>
            <td colSpan={3+TENDER_STEPS.filter(function(s){return s.key!=="process"&&s.key!=="bidders";}).length+4} style={{padding:"8px 12px"}}>
              {(function(){
                var live=filtered.filter(function(t){return !t.cancelled;});
                var nCancelled=filtered.length-live.length;
                var totBudget=live.reduce(function(s,t){return s+Number(t.budget||0);},0);
                var totInstructed=live.reduce(function(s,t){return s+Number(t.instructionAmount||0);},0);
                var totAccSub=live.reduce(function(s,t){return s+Number(t.accAmountSubcontract||0);},0);
                var totAccOther=live.reduce(function(s,t){return s+Number(t.accAmountOther||0);},0);
                ...
                return <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                  <span style={{fontSize:10,fontWeight:700,color:"#aaa",textTransform:"uppercase",letterSpacing:".5px",marginRight:4}}>TOTALS ({live.length})</span>
                  {nCancelled>0&&<span style={{fontSize:10,fontWeight:700,color:"#e65100",background:"#fff3e0",padding:"2px 8px",borderRadius:7}}>🚫 {nCancelled} cancelled excluded</span>}
...
function EmailModal({em,onClose}){
  ...
  // Printing the modal through the app's own stylesheet clips the text: the body is
  // inside a scrolling box, and the global print rules are tuned for the schedule.
  // A standalone document with its own page setup avoids all of it.
  function printReport(){
    var w=window.open("","_blank","width=900,height=1000");
    if(!w){safeAlert("The print window was blocked. Allow pop-ups for this site and try again.");return;}
    function esc(s){return String(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");}
    w.document.write(
      "<!doctype html><html><head><meta charset='utf-8'><title>"+esc(em.subject)+"</title><style>"+
      "@page{size:A4 portrait;margin:18mm 16mm;}"+
      "html,body{margin:0;padding:0;background:#fff;}"+
      "body{font-family:'IBM Plex Mono',ui-monospace,Menlo,Consolas,monospace;font-size:10.5px;line-height:1.55;color:#111;}"+
      "h1{font-family:ui-sans-serif,system-ui,sans-serif;font-size:14px;margin:0 0 10px;padding-bottom:6px;border-bottom:1.5px solid #111;}"+
      // pre-wrap keeps the report's alignment while still wrapping long lines, so
      // nothing is ever cut off at either edge of the sheet
      "pre{white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;margin:0;font:inherit;}"+
      "</style></head><body><h1>"+esc(em.subject)+"</h1><pre>"+esc(em.body)+"</pre></body></html>"
    );
    w.document.close();
    w.focus();
...
function SettingsView({tags,saveTags:_saveTags,people,savePeople:_savePeople,packages,savePackages:_savePackages,tagrules,saveTagrules:_saveTagrules,pkgrules,savePkgrules:_savePkgrules,apiKey,saveApiKey:_saveApiKey,improvements,saveImprovements,pkgOwners,savePkgOwners:_savePkgOwners,pkgSubcontractors,savePkgSubcontractors:_savePkgSubcontractors,peopleEmails,savePeopleEmails:_savePeopleEmails,defaultCC,saveDefaultCC:_saveDefaultCC,peopleAccess,savePeopleAccess:_savePeopleAccess,durations,saveDurations:_saveDurations,isAdmin,zones,saveZones:_saveZones,zoneOwners,saveZoneOwners:_saveZoneOwners,userPrefs,saveUserPrefs,allData,onImport:_onImport}){
  // Every shared setting is administrator-only. The guard sits on the save
  // functions rather than on the buttons, so no control can slip through by
  // being missed in the markup. Personal preferences and the improvement
  // suggestions box stay writable by everyone.
  function adminOnly(fn,what){
    return function(){
      if(!isAdmin){safeAlert("Only the app administrator can change "+what+".\n\nAsk "+APP_ADMIN+" to make this change.");return;}
      return fn.apply(null,arguments);
    };
  }
  const saveTags=adminOnly(_saveTags,"the tag list");
  const savePeople=adminOnly(_savePeople,"the people list");
  const savePackages=adminOnly(_savePackages,"the package list");
  const saveTagrules=adminOnly(_saveTagrules,"the CC rules");
  const savePkgrules=adminOnly(_savePkgrules,"the package rules");
  const saveApiKey=adminOnly(_saveApiKey,"the API key");
  const savePkgOwners=adminOnly(_savePkgOwners,"package ownership");
  const savePkgSubcontractors=adminOnly(_savePkgSubcontractors,"package subcontractors");
  const savePeopleEmails=adminOnly(_savePeopleEmails,"email addresses");
  const saveDefaultCC=adminOnly(_saveDefaultCC,"the report CC list");
  const savePeopleAccess=adminOnly(_savePeopleAccess,"access rights");
  const saveDurations=adminOnly(_saveDurations,"the lead times");
  const saveZones=adminOnly(_saveZones,"the zone list");
  const saveZoneOwners=adminOnly(_saveZoneOwners,"zone ownership");
  const onImport=adminOnly(_onImport,"the app data by import");
  ...
  // Rebuilds the People list from every other place a person's name is stored.
  // Used when the list itself has been lost or truncated: emails, access rights,
  // package/zone ownership and action owners all survive independently of it.
  function collectKnownNames(){
    ...
    function add(v){
      ...
      var n=v.trim();
      // "System" is the marker for auto-created actions, not a person.
      if(!n||n==="System")return;
      out[n]=true;
    }
    Object.keys(peopleEmails||{}).forEach(add);
    Object.keys(peopleAccess||{}).forEach(add);
    add(defaultCC);
    Object.keys(pkgOwners||{}).forEach(function(k){add((pkgOwners||{})[k]);});
    Object.keys(zoneOwners||{}).forEach(function(k){add((zoneOwners||{})[k]);});
    var d=allData||{};
    (d.tasks||[]).forEach(function(t){add(t.owner);add(t.addedBy);add(t.lastModifiedBy);});
    (d.tenders||[]).forEach(function(t){add(t.ownerPackage);add(t.ownerTender);});
    (d.contractors||[]).forEach(function(ct){
      add(ct.owner);
      (ct.contracts||[]).forEach(function(k){add(k.owner);});
    });
    (d.trackers||[]).forEach(function(x){(x.actions||[]).forEach(function(a){add(a.owner);});});
    return Object.keys(out).sort();
  }

  function recoverPeople(){
    var known=collectKnownNames();
    var missing=known.filter(function(n){return people.indexOf(n)<0;});
    if(!missing.length){safeAlert("Nothing to recover — every name found elsewhere in the app is already in the People list.");return;}
    var preview=missing.slice(0,40).join("\n")+(missing.length>40?"\n… and "+(missing.length-40)+" more":"");
    if(!safeConfirm("Add "+missing.length+" name(s) back to the People list?\n\n"+preview))return;
    savePeople(people.concat(missing).sort());
  }
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
      {["tags","people","packages","zones","pkg-owners","cc-rules","report-cc","access","my-prefs","improvements","backup"].concat(isAdmin?["durations"]:[]).map(function(t){return <button key={t} className={"fchip"+(tab===t?" on":"")} onClick={function(){setTab(t);}} style={{textTransform:"capitalize"}}>{t==="cc-rules"?"CC Rules":t==="improvements"?"💡 Improvements":t==="pkg-owners"?"📦 Pkg Owners":t==="backup"?"💾 Backup":t==="my-prefs"?"👤 My Prefs":t==="zones"?"🏢 Zones":t==="report-cc"?"📧 Report CC":t==="access"?"🔐 Access":t==="durations"?"⏱ Durations":t.charAt(0).toUpperCase()+t.slice(1)}</button>;})}
    </div>

    {!isAdmin&&<div style={{margin:"10px 0 14px",padding:"9px 12px",borderRadius:8,background:"#fff8e1",border:"1.5px solid #ffca28",fontSize:12,color:"#8d6e00",fontWeight:600}}>
      🔒 Read-only — these settings are shared by the whole project and only {APP_ADMIN} can change them. My Prefs and Improvements remain open to you.
    </div>}
    ...
    {tab==="people"&&<div className="card">
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"8px 10px",borderRadius:8,background:"#f7f5f0",border:"1px solid #e0ddd6"}}>
        <div style={{flex:1,fontSize:11,color:"#666"}}>Names missing from this list can be rebuilt from emails, access rights, package and zone ownership, and action owners.</div>
        <button className="btn" onClick={recoverPeople}>♻ Recover missing people</button>
      </div>
      <div style={{display:"flex",gap:8,marginBottom:16}}>
...
function calcProcurement(td){
  ...
  // Backward pass. With no ACC date the forward chain has no anchor and every step
  // stays blank — even when the site start date is known and the durations are fixed.
  // In that case the whole chain is derived backwards from the site start date, which
  // is exactly the question being asked: when must each step happen to hit that date.
  var isTheoretical=false;
  var siteStart=td.startOnSite||"";
  if(!accSubmittal&&isValidDate(siteStart)){
    var chainDays=getDur("accApproval")+getDur("contractSigning")+LEAD
      +(hasSd?getDur("sdAfterContract")+getDur("sdApproval")+sdResubCount*28:0);
    var anchor=new Date(siteStart);
    anchor.setDate(anchor.getDate()-chainDays);
    accSubmittal=toISO(anchor);
    isTheoretical=true;
  }

  steps.push({key:"accSub", label:"ACC Submittal", date:accSubmittal, done:accDoneActual, duration:null, manual:true, theoretical:isTheoretical, note:accDoneActual?"From 'Date done' of ACC step":isTheoretical?"Theoretical — worked back from the site start date":"Provisional — from ACC target date (not yet submitted)"});

  var accApprTargetAuto = addWorkDays(accSubmittal, getDur("accApproval"));
  var accApprTarget = ov.accApp||accApprTargetAuto;
  steps.push({key:"accApp", label:"ACC Approval", date:accApprTarget, done:accApproval, duration:getDur("accApproval"), manual:false, autoDate:accApprTargetAuto, overridden:!!(ov.accApp&&ov.accApp!==accApprTargetAuto)});
  ...
  steps.push({key:"contract", label:"Contract Signing", date:contractTarget, done:contractDone, duration:getDur("contractSigning"), manual:false, autoDate:contractTargetAuto, overridden:!!(ov.contract&&ov.contract!==contractTargetAuto)});
  ...
  if(hasSd){
    ...
    steps.push({key:"sdSub", label:"SD Submission", date:sdSubTarget, done:td.sdDone||"", duration:getDur("sdAfterContract"), manual:false, sd:true, autoDate:sdSubTargetAuto,
    ...
    steps.push({key:"sdApp", label:"SD Approval", date:sdAppTarget, done:td.sdApprovalDone||"", duration:getDur("sdApproval"), manual:false, sd:true,
  ...
  var margin = "";
  var procStart = "";
  // The delivery step already carries LEAD as its duration, so it must not be added
  // a second time here — that pushed PROCUREMENT START a full lead time too early.
  var totalDays = steps.filter(function(s){return s.duration;}).reduce(function(a,s){return a+(s.duration||0);},0);
  if(startOnSite){
  ...
  return {steps:steps, deliveryDate:deliveryDate, procStart:procStart, margin:margin, totalDays:totalDays, LEAD:LEAD, theoretical:isTheoretical};
}
...
function GlobalView({tasks,trackers,tenders,contractors,people,packages,tags,saveTasks,saveTrackers,tagrules,pkgrules,jumpOwner,clearJump,onNavTender,memory,setMemory,peopleEmails,defaultCC,zones,actionsOnly,currentUser}){
  ...
  function buildEmail(){
    ...
    var NL=String.fromCharCode(10);
    var allReal=filtered.filter(function(a){return !a.isInfo;});
    var doneCount=allReal.filter(function(a){return a.status==="done";}).length;
    // Same rule as the zone MoM: closed actions are counted, not listed.
    var realActions=allReal.filter(function(a){return a.status!=="done";});
    var pendingCount=realActions.length;

    var lines=[];
    lines.push("ACTIONS REPORT");
    lines.push(now.toLocaleDateString("en-GB",{weekday:"long",day:"2-digit",month:"long",year:"numeric"}));
    lines.push("");
    lines.push(pendingCount+" open action"+(pendingCount!==1?"s":"")+(doneCount>0?" - "+doneCount+" completed (not listed)":""));
    lines.push("");

    var CAT_ORDER=["Blocking Point","Top Management",...(tags||[]).filter(function(t){return t!=="Blocking Point"&&t!=="Top Management";})];
    var groups2={};
    realActions.forEach(function(a){
      var cat=CAT_ORDER.find(function(c){return(a.tags||[]).includes(c);})||"General";
...
function PackagesView({tasks,tenders,contractors,packages,people,pkgOwners,pkgSubcontractors,saveTasks,tags,onNavTender,memory,setMemory}){
  ...
  var rows=(packages||[]).map(function(pkg){
    ...
    var liveTenders=pkgTenders.filter(function(t){return !t.cancelled;});
    var budgetTotal=liveTenders.reduce(function(s,t){return s+(Number(t.budget)||0);},0);
    var budgetTreated=liveTenders.reduce(function(s,t){return s+(Number(t.accAmountTreated)||0);},0);
    var costTreated=0;
    liveTenders.forEach(function(t){
      (contractors||[]).forEach(function(ctr){
        (ctr.contracts||[]).forEach(function(ct){
  ...
  return <div style={{padding:"16px 20px",overflowY:"auto",flex:1}}>
    ...
    {filtered.length===0
      ...
      :<div>
        ...
        <div className="sched-print-only" style={{marginTop:12}}>
          {(function(){
            // rooms / curZone / roomBlockersOf belong to ZoneView and are not in scope here.
            // Guard with typeof so a bare reference cannot throw a ReferenceError.
            var zr=(typeof rooms!=="undefined"&&rooms?rooms:[]).filter(function(rm){return typeof curZone!=="undefined"&&rm.zone===curZone;});
            var blocked=zr.map(function(rm){return{room:rm,acts:(typeof roomBlockersOf==="function")?roomBlockersOf(rm.id):[]};}).filter(function(x){return x.acts.length>0;});
            if(blocked.length===0)return <div style={{fontSize:11,color:"#2e7d32",fontWeight:700}}>No blocking point recorded on any room.</div>;
            return <div>
              <div style={{fontSize:13,fontWeight:800,marginBottom:6,borderTop:"2px solid #c62828",paddingTop:8}}>BLOCKING POINTS BY ROOM — {blocked.length} room{blocked.length!==1?"s":""} affected</div>
              {blocked.map(function(x){
                return <div key={x.room.id} style={{marginBottom:6}}>
                  <div style={{fontSize:11,fontWeight:800}}>{x.room.name}</div>
                  {x.acts.map(function(a){
                    return <div key={a.id} style={{fontSize:10,paddingLeft:12}}>
                      • {a.text}{a.owner?" — "+a.owner.split(",")[0]:""}{a.due?" — due "+fmtDate(a.due):""}
                    </div>;
                  })}
                </div>;
              })}
...
function ClientSubmissionsView({tasks,tenders,contractors,packages,people,saveTasks,onNavTender,memory,setMemory}){
  ...
  var allDocs=buildTrackedDocs(tasks,tenders,contractors);
  var withClient=allDocs.filter(function(d){return d.withClient;});
...
function ZoneActionRow({task,rooms,onUpdate,onDelete,onOpenRoomModal,people,tags,context}){
  const [editMode,setEditMode]=useState(false);
  ...
  return <div className="ac-item" style={{background:editMode?"#f8f9ff":task.status==="done"?"#fafaf8":"#fff",borderColor:isBlocking?"#f48fb1":editMode?"#3949ab":"#e8e6df"}}>
    ...
    <div style={{flex:1,minWidth:0}}>
      {editMode
        ?<div style={{display:"flex",flexDirection:"column",gap:6}}>
          ...
          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
            {(tags||[]).map(function(tg){var on=(task.tags||[]).includes(tg);var tc=tagColor(tg);return <button key={tg} onClick={function(){var cur=task.tags||[];var nt=on?cur.filter(function(x){return x!==tg;}):[...cur,tg];onUpdate({tags:nt});if(tg==="Blocking Point"&&!on)onOpenRoomModal();}} style={{padding:"2px 9px",borderRadius:12,border:"1.5px solid "+(on?tc.color:"#ddd"),background:on?tc.bg:"#fff",color:on?tc.color:"#bbb",fontFamily:"inherit",fontSize:11,fontWeight:700,cursor:"pointer"}}>{tg}</button>;})}
          </div>
        ...
        :<div onClick={function(){setEditMode(true);}} style={{cursor:"pointer"}}>
          <div className={"ac-text"+(task.status==="done"?" done":"")} style={{fontWeight:500}}>
            {context&&<span style={{display:"inline-block",fontSize:10,fontWeight:800,padding:"1px 7px",borderRadius:8,background:"#f3e5f5",color:"#7b1fa2",marginRight:6,verticalAlign:"middle",letterSpacing:".3px"}}>🚪 {context.toUpperCase()}</span>}
            {task.text||<span style={{color:"#ccc",fontStyle:"italic"}}>No text</span>}
          </div>
          <div className="ac-meta">
            {task.due
              ?(function(){
                ...
                return <span style={{fontSize:11,fontWeight:700,padding:"1px 8px",borderRadius:10,background:isLate?"#fce4ec":isSoon?"#fff8e1":"#f0ede6",color:isLate?"#c62828":isSoon?"#f57f17":"#666"}}>
                  📅 {fmtDate(task.due)}{isLate?" · "+workingDaysDiff(task.due,today())+"d late":""}
                </span>;
              })()
              :<span style={{fontSize:11,padding:"1px 8px",borderRadius:10,background:"#f5f4f0",color:"#bbb",fontStyle:"italic"}}>no target date</span>}
            {task.owner&&<OwnerChip owner={task.owner}/>}
            {(task.tags||[]).map(function(tg){return <TagChip key={tg} tag={tg}/>;})}
    ...
    {!editMode&&<div style={{display:"flex",gap:4,flexShrink:0}}>
      <select className="btn btn-sm" value={task.status||"pending"} onChange={function(e){onUpdate({status:e.target.value,completedAt:e.target.value==="done"?today():""});}} style={{width:"auto",padding:"3px 6px",fontSize:10,border:"1px solid #ddd"}}>
        {STATUS_OPTS.map(function(s){return <option key={s} value={s}>{STATUS_ICONS[s]} {s}</option>;})}
      </select>
      <button className="btn btn-sm btn-danger" onClick={onDelete} style={{padding:"3px 7px"}}>🗑</button>
    </div>}
  </div>;
}
...
// Defined at module level on purpose: a component declared inside ScheduleView would be
// a new function identity on every render, so React would remount the inputs and the
// text field would lose focus after each keystroke.
// One document editor, reused for the schedule, a room and a task, so the three
// levels behave identically. `links` is always an array of {label,url}.
function DocsEditor(props){
  var links=props.links||[];
  function save(ls){props.onChange(ls);}
  return <div>
    {links.length===0&&<div style={{fontSize:11,color:"#bbb",marginBottom:6}}>No document yet. Paste a SharePoint or Aconex link.</div>}
    {links.map(function(lk,li){return <div key={li} style={{display:"flex",gap:4,marginBottom:4,alignItems:"center"}}>
      <input type="text" value={lk.label||""} placeholder="Label"
        onChange={function(e){save(links.map(function(x,j){return j!==li?x:Object.assign({},x,{label:e.target.value});}));}}
        style={{width:90,padding:"3px 6px",fontSize:11,border:"1px solid #e0ddd8",borderRadius:5}}/>
      <input type="url" value={lk.url||""} placeholder="https://…"
        onChange={function(e){save(links.map(function(x,j){return j!==li?x:Object.assign({},x,{url:e.target.value});}));}}
        style={{flex:1,minWidth:0,padding:"3px 6px",fontSize:11,border:"1px solid #e0ddd8",borderRadius:5}}/>
      {lk.url&&<a href={lk.url} target="_blank" rel="noopener noreferrer" title="Open in a new tab" style={{fontSize:13,textDecoration:"none",flexShrink:0}}>↗</a>}
      <button onClick={function(){save(links.filter(function(_,j){return j!==li;}));}}
        style={{background:"none",border:"none",cursor:"pointer",color:"#ccc",fontSize:12,flexShrink:0}}>✕</button>
    </div>;})}
    <button className="btn btn-sm" style={{fontSize:10,padding:"2px 8px",marginTop:4}}
      onClick={function(){save([...links,{label:"",url:""}]);}}>＋ Add document</button>
  </div>;
}
function docCount(o){return ((o&&o.links)||[]).filter(function(l){return l&&l.url;}).length;}
function ScheduleView({curZone,schedules,saveSchedules,tasks,saveTasks,people,tags,zones,rooms,saveRooms,canEdit,roomBlockersOf,tenders,saveTenders,pkgOwners,contractors,pkgSubcontractors}){
  ...
  // Documents attach at three levels: the schedule, a room (category row), a task (row).
  const [docsRowId,setDocsRowId]=useState(null);
  const [docsPos,setDocsPos]=useState({x:0,y:0});
  const [schedDocsOpen,setSchedDocsOpen]=useState(false);
  const [newGroup,setNewGroup]=useState("");
  const [hoverInfo,setHoverInfo]=useState(null);
  const [newActionText,setNewActionText]=useState("");
  const [linkSearch,setLinkSearch]=useState("");
  const [newActionBlocking,setNewActionBlocking]=useState(false);
  ...
  const [copyCatId,setCopyCatId]=useState(null);
  const [copyPos,setCopyPos]=useState({x:0,y:0});
  // The week grid needs ~40px per week plus 650px of frozen columns, so it is unreadable
  // on a phone. Below 820px we switch to a stacked card list; "auto" can be overridden.
  const [viewMode,setViewMode]=useState("auto");
  const [vw,setVw]=useState(typeof window!=="undefined"?window.innerWidth:1200);
  const [only3w,setOnly3w]=useState(false);
  useEffect(function(){
    function onResize(){setVw(window.innerWidth);}
    window.addEventListener("resize",onResize);
    return function(){window.removeEventListener("resize",onResize);};
  },[]);
  var cards=viewMode==="auto"?vw<820:viewMode==="cards";
  ...
  function delSchedule(id){
    if(!safeConfirm("Delete this schedule and all its rows?"))return;
  ...
  // The planned bars are the source of truth for a row's start and end week.
  // Clicking cells in the grid must therefore recompute them, otherwise every
  // downstream calculation (procurement risk, tender start on site, theoretical
  // dates) keeps reading stale or empty values.
  function spanFromCells(cells){
    var wl=sc?scheduleWeeks(sc):[];
    var painted=wl.filter(function(w){var v=(cells||{})[w];return v==="plan"||v==="both";});
    if(!painted.length)return{startWeek:"",endWeek:""};
    return{startWeek:painted[0],endWeek:painted[painted.length-1]};
  }
  function updRow(rowId,fields){
    if(!sc)return;
    var row=(sc.rows||[]).find(function(r){return r.id===rowId;});
    var patch=fields;
    // Only derive when the caller changed the cells without stating the span itself.
    if(fields&&fields.cells&&fields.startWeek===undefined&&fields.endWeek===undefined){
      patch=Object.assign({},fields,spanFromCells(fields.cells));
    }
    var nextRows=(sc.rows||[]).map(function(r){return r.id!==rowId?r:Object.assign({},r,patch);});
    upd(sc.id,{rows:nextRows});
    // A changed span moves the tender's start on site, which drives its theoretical dates.
    var newRef=(fields&&fields.tenderRef!==undefined)?fields.tenderRef:(row?row.tenderRef:"");
    if(newRef&&(patch.startWeek!==undefined||fields.tenderRef!==undefined)){
      pushStartOnSiteFromSchedule(newRef,nextRows);
    }
  ...
  function delRow(rowId){
    if(!sc)return;
    var row=(sc.rows||[]).find(function(r){return r.id===rowId;});
    if(row&&row.kind==="category"&&row.roomId){
      if(!safeConfirm("Delete this category? Its linked room will be removed too."))return;
      if(saveRooms)saveRooms((rooms||[]).filter(function(rm){return rm.id!==row.roomId;}));
    }
    upd(sc.id,{rows:(sc.rows||[]).filter(function(r){return r.id!==rowId;})});
  }
  ...
  function procRisk(row){
    if(!row.tenderRef||!row.startWeek)return null;
    ...
    var late=proc.deliveryDate>wkEnd;
    var weeksLate=late?Math.ceil((new Date(proc.deliveryDate)-new Date(wkEnd))/(1000*60*60*24*7)):0;
    var tight=!late&&proc.deliveryDate>=row.startWeek;
    return{tender:td,delivery:proc.deliveryDate,startWeek:row.startWeek,late:late,tight:tight,weeksLate:weeksLate};
  }
  ...
  // rowsOverride carries the rows as they were just patched: React state has not
  // re-rendered yet at this point, so reading sc.rows would use the previous span.
  function pushStartOnSiteFromSchedule(tenderRef,rowsOverride){
    if(!tenderRef||!saveTenders)return;
    var srcRows=rowsOverride||(sc?sc.rows:[])||[];
    var linked=srcRows.filter(function(r){return r.tenderRef===tenderRef&&r.startWeek;});
  ...
  useEffect(function(){
    ...
    // open one action per late row
    (sc.rows||[]).forEach(function(r){
      var pr=procRisk(r);
      if(!pr||!pr.late)return;
      var text=procRiskText(r,pr.tender);
      if(next.some(function(t){return t.scheduleRowRef===r.id&&t.text===text&&t.status!=="done";}))return;
      next=[newTask({
        text:text,
    ...
    // close the ones whose conflict is gone
    next=next.map(function(t){
      if(t.addedBy!=="System"||t.status==="done"||!t.scheduleRowRef)return t;
      if((t.text||"").indexOf("Procurement late for: ")!==0)return t;
  ...
  function rowSpan(row){
    if(!row.startWeek||!row.endWeek||!sc)return null;
    ...
    return{weeks:span,working:working,totalWeeks:span.length,workingWeeks:working.length,rate:rate};
  }
  ...
  function applySpan(row,patch){
    ...
    if(sp)sp.forEach(function(w){cells[w]=cells[w]==="actual"?"both":"plan";});
    // Both ends are written so updRow does not re-derive and undo a half-set span
    // (the user picks a start before an end, and vice versa).
    updRow(row.id,Object.assign({startWeek:merged.startWeek||"",endWeek:merged.endWeek||""},patch,{cells:cells}));
  }
  ...
  function rowWarning(rowId){
    ...
    var blocking=acts.filter(function(t){return(t.tags||[]).includes("Blocking Point");});
    var late=acts.filter(function(t){return t.due&&t.due<today();});
    return{acts:acts,blocking:blocking.length,late:late.length,severity:blocking.length>0?"blocking":late.length>0?"late":"info"};
  }
  ...
  function createLinkedAction(rowId,rowLabel,text,isBlocking){
    if(!saveTasks||!text.trim())return;
    var tg=isBlocking?["Blocking Point"]:[];
    var srcRow=(sc&&(sc.rows||[]).find(function(r){return r.id===rowId;}))||null;
    // The action lands in the shared task list, so it must carry enough context to be
    // found there: the zone, the trade as owner hint, and the row's planned start as
    // the target date (an action with no date sinks to the bottom of every view).
    saveTasks([newTask({
      text:text.trim(),
      zone:curZone,
      scheduleRowRef:rowId,
      due:srcRow&&srcRow.startWeek?srcRow.startWeek:"",
      tags:tg,
      importance:isBlocking?3:2,
      urgence:isBlocking?3:2,
      note:"Linked to schedule row: "+rowLabel
    }),...(tasks||[])]);
  }
  ...
  function toggleCell(row,wk){
    ...
    updRow(row.id,{cells:cells});
  }
  function cellStyle(v){
    if(v==="plan")return{background:"#c9a84c"};
    if(v==="actual")return{background:"#1a73e8"};
    if(v==="both")return{background:"linear-gradient(180deg,#c9a84c 50%,#1a73e8 50%)"};
    return{};
  }
  ...
  var allGroups=sc?[...new Set([...(sc.groups||[]),...(sc.rows||[]).map(function(r){return r.group;}).filter(Boolean)])].sort():[];
  // Names already registered elsewhere in the app, offered as suggestions when adding a
  // subcontractor to the schedule so the same company is not spelled three different ways.
  var subSuggestions=[...new Set([
    ...(contractors||[]).map(function(c){return c.name;}),
    ...Object.keys(pkgSubcontractors||{}).map(function(k){return (pkgSubcontractors||{})[k];})
  ])].filter(Boolean).filter(function(n){return allGroups.indexOf(n)<0;}).sort();

  // Adding a subcontractor and assigning it to a row must be ONE write: two calls in the same
  // tick would both read the pre-render `schedules` and the second would drop the first.
  function addSubcontractor(name,rowId){
    var n=(name||"").trim();
    if(!n||!sc)return;
    if(canEdit===false){safeAlert("Only the zone leaders can modify this schedule.");return;}
    saveSchedules((schedules||[]).map(function(s){
      ...
      return Object.assign({},s,{
        groups:[...new Set([...(s.groups||[]),n])],
        rows:rowId?(s.rows||[]).map(function(r){return r.id!==rowId?r:Object.assign({},r,{group:n});}):(s.rows||[]),
        updatedAt:today(),updatedBy:window._currentUser?window._currentUser.name:""
      });
    }));
  }
  function promptSubcontractor(rowId){
    var n=window.prompt("Subcontractor / trade name:","");
    if(n===null)return;
    addSubcontractor(n,rowId);
  }
  ...
  if(sc){
    if(!fGroup){
      ...
      for(var i=0;i<all.length;i++){
        if(all[i].kind!=="category")continue;
        var end=i+1;var kept=[];
        while(end<all.length&&all[end].kind!=="category"){
          if(all[end].group===fGroup)kept.push(all[end]);
          end++;
        }
        if(kept.length>0)groupRows.push(all[i],...kept);
        i=end-1;
      }
    }
  }

  // "Next 3 weeks": tasks already running plus those starting within 21 days.
  var lookaheadEndWk=(function(){var d=new Date();d.setDate(d.getDate()+21);return toISO(d);})();
  function inNext3Weeks(r){
    if(r.kind==="category")return true;
    if(!r.startWeek)return false;
    return r.startWeek<=lookaheadEndWk&&(r.endWeek||r.startWeek)>=todayStr;
  }
  // Applying the lookahead drops the rooms left without a single matching task, so the
  // list is not padded with empty category headers.
  var scopedRows=groupRows;
  if(only3w){
    var kept3=[];
    for(var i3=0;i3<groupRows.length;i3++){
      var r3=groupRows[i3];
      if(r3.kind!=="category"){if(inNext3Weeks(r3))kept3.push(r3);continue;}
      var any3=false;
      for(var j3=i3+1;j3<groupRows.length&&groupRows[j3].kind!=="category";j3++){
        if(inNext3Weeks(groupRows[j3])){any3=true;break;}
      }
      if(any3)kept3.push(r3);
    }
    scopedRows=kept3;
  }
  ...
  if(sc){
    ...
    scopedRows.forEach(function(r){
      if(r.kind==="category"){hiding=!!collapsedCats[r.id];visibleRows.push(r);return;}
      if(!hiding)visibleRows.push(r);
    });
  }
  // Card list: each category carries its own tasks instead of relying on row order.
  function cardBlocks(){
    var blocks=[];var cur=null;
    visibleRows.forEach(function(r){
      if(r.kind==="category"){cur={cat:r,tasks:[]};blocks.push(cur);return;}
      if(!cur){cur={cat:null,tasks:[]};blocks.push(cur);}
      cur.tasks.push(r);
    });
    return blocks;
  }
  ...
  return <div>
    <div className="sched-noprint" style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginBottom:12}}>
    ...
    {!sc
      ...
      :<div>
        <div className="sched-noprint" style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end",marginBottom:10,padding:"10px 12px",background:"#fafaf8",borderRadius:8,border:"1px solid #e8e6df"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:4,paddingLeft:8,borderLeft:"1px solid #e0ddd6"}}>
            <span style={{fontSize:9,fontWeight:800,color:"#aaa",textTransform:"uppercase"}}>Trade</span>
            <select value={fGroup} onChange={function(e){setFGroup(e.target.value);}} title="Show only the tasks of one trade — rooms with no matching task are hidden"
            ...
            <input type="text" list="sched-sub-list" value={newGroup} onChange={function(e){setNewGroup(e.target.value);}}
              onKeyDown={function(e){if(e.key==="Enter"&&newGroup.trim()){addSubcontractor(newGroup,"");setNewGroup("");}}}
              placeholder="New subcontractor" title="Type a name then press Enter or ＋"
              style={{width:130,padding:"4px 6px",fontSize:10}}/>
            <datalist id="sched-sub-list">
              {subSuggestions.map(function(g){return <option key={g} value={g}/>;})}
            </datalist>
            <button className="btn btn-sm btn-gold" disabled={!newGroup.trim()}
              onClick={function(){addSubcontractor(newGroup,"");setNewGroup("");}}
              title="Add this subcontractor to the schedule" style={{padding:"3px 9px",fontSize:11}}>＋</button>
          </div>
          <button className="btn btn-sm" onClick={function(){setSchedDocsOpen(!schedDocsOpen);}}
            title="Documents for the whole schedule — the master programme, the zone drawings…"
            style={docCount(sc)>0?{color:"#3949ab",borderColor:"#c5cae9"}:null}>
            📎 Documents{docCount(sc)>0?" ("+docCount(sc)+")":""}
          </button>
          <button className="btn btn-sm" onClick={printSchedule}>🖨 Print / PDF</button>
          <button className="btn btn-sm btn-danger" onClick={function(){delSchedule(sc.id);}}>🗑</button>
        </div>
        ...
        {schedDocsOpen&&<div className="sched-noprint" style={{marginBottom:10,padding:"12px 14px",background:"#f5f6ff",border:"1.5px solid #3949ab",borderRadius:10}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
            <div style={{fontWeight:700,fontSize:13,color:"#283593"}}>📎 Schedule documents — {sc.title||"(untitled)"}</div>
            <button className="btn btn-sm" onClick={function(){setSchedDocsOpen(false);}}>✕</button>
          </div>
          <div style={{fontSize:11,color:"#888",marginBottom:8}}>Attached to the whole schedule: master programme, zone drawings, method statements.</div>
          <DocsEditor links={sc.links} onChange={function(ls){upd(sc.id,{links:ls});}}/>
        </div>}

        <div className="sched-print-only">
          <div style={{fontSize:16,fontWeight:800}}>{sc.title}{fGroup?" — "+fGroup:""}</div>
          <div style={{fontSize:11,color:"#555"}}>{curZone} · {fmtDate(wks[0]||sc.startDate)} → {fmtDate(wks[wks.length-1]||sc.startDate)} · printed {fmtDate(todayStr)}{fGroup?" · filtered for "+fGroup:""}</div>
          {docCount(sc)>0&&<div style={{fontSize:10,color:"#3949ab",marginTop:2}}>
            📎 {((sc.links)||[]).filter(function(l){return l&&l.url;}).map(function(l){return l.label||l.url;}).join(" · ")}
          </div>}
        </div>
        ...
        <div className="sched-noprint" style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap",alignItems:"center",padding:"8px 10px",background:"#fffdf0",border:"1px solid #f0e2b8",borderRadius:8}}>
          ...
          <input type="text" value={newRowLabel} onChange={function(e){setNewRowLabel(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")addRow();}} placeholder={newRowKind==="category"?"Room name, e.g. POOL":"Task name, e.g. Slab"} style={{flex:1,minWidth:180,padding:"5px 10px",fontSize:12}}/>
        ...
        <div className="sched-noprint" style={{display:"flex",gap:10,alignItems:"center",marginBottom:8,flexWrap:"wrap"}}>
          ...
          <button className={"fchip"+(only3w?" on":"")} onClick={function(){setOnly3w(!only3w);}}
            title="Show only the tasks running now or starting within 3 weeks"
            style={Object.assign({padding:"3px 10px",fontSize:11},only3w?{background:"#00695c",borderColor:"#00695c",color:"#fff"}:{})}>⏱ Next 3 weeks</button>
          <button className="btn btn-sm" onClick={function(){setViewMode(cards?"grid":"cards");}}
            title={cards?"Switch to the week grid":"Switch to the mobile card list"}
            style={{padding:"2px 8px",fontSize:11}}>{cards?"▦ Grid":"📱 Cards"}</button>
          {!cards&&<span style={{display:"flex",gap:3,alignItems:"center",paddingRight:6,borderRight:"1px solid #e0ddd6"}}>
            ...
            <button className="btn btn-sm" onClick={function(){
              ...
              if(idx>=0)el.scrollLeft=Math.max(0,380+150+140+150+86+idx*40-el.clientWidth/2);
        ...
        {!cards&&<div className="sched-wrap" style={{overflow:"auto",maxHeight:"62vh",border:"1px solid #e8e6df",borderRadius:8,background:"#fff"}}>
          <table style={{borderCollapse:"collapse",width:"max-content",minWidth:"100%",fontSize:11}}>
            <thead>
              <tr>
                <th style={{position:"sticky",left:0,top:0,background:"#f5f4f0",zIndex:4,width:380,minWidth:380,maxWidth:380,textAlign:"left",padding:"6px 10px",borderBottom:"1.5px solid #e8e6df",borderRight:"1.5px solid #e8e6df",fontSize:10,textTransform:"uppercase",color:"#888"}}>Task</th>
                <th className="sched-noprint" style={{position:"sticky",top:0,background:"#f5f4f0",zIndex:3,width:150,minWidth:150,maxWidth:150,textAlign:"left",padding:"6px 8px",borderBottom:"1.5px solid #e8e6df",fontSize:10,textTransform:"uppercase",color:"#888"}}>Subcontractor</th>
                <th className="sched-noprint" style={{position:"sticky",top:0,background:"#f5f4f0",zIndex:3,width:140,minWidth:140,maxWidth:140,textAlign:"left",padding:"6px 8px",borderBottom:"1.5px solid #e8e6df",fontSize:10,textTransform:"uppercase",color:"#888"}}>Room</th>
                <th className="sched-noprint" style={{position:"sticky",top:0,background:"#f5f4f0",zIndex:3,width:150,minWidth:150,maxWidth:150,textAlign:"left",padding:"6px 8px",borderBottom:"1.5px solid #e8e6df",fontSize:10,textTransform:"uppercase",color:"#888"}}>Starts after</th>
                <th style={{position:"sticky",top:0,background:"#f5f4f0",zIndex:3,width:86,minWidth:86,maxWidth:86,textAlign:"right",padding:"6px 8px",borderBottom:"1.5px solid #e8e6df",borderRight:"1.5px solid #e8e6df",fontSize:10,textTransform:"uppercase",color:"#888"}}>Rate</th>
                {wks.map(function(wk){
            ...
            <tbody>
              {(sc.rows||[]).length===0&&<tr><td colSpan={wks.length+5} style={{padding:"14px",textAlign:"center",color:"#bbb",fontSize:12}}>No rows yet. Add a category or a task below.</td></tr>}
              {visibleRows.map(function(row){
                ...
                var warnColor=warn?(warn.severity==="blocking"?"#c62828":warn.severity==="late"?"#e65100":"#f57f17"):"";
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  <td style={{position:"sticky",left:0,zIndex:1,width:380,minWidth:380,maxWidth:380,background:isCat?"#f0ede6":"#fff",padding:"3px 10px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df"}}>
                    <div style={{display:"flex",alignItems:"center",gap:3}}>
                      ...
                      <input type="text" value={row.label||""} onChange={function(e){updRow(row.id,{label:e.target.value});}}
                        title={row.label||""}
                        style={{flex:1,minWidth:110,border:"none",background:"transparent",fontSize:isCat?12:11,fontWeight:isCat?800:500,textTransform:isCat?"uppercase":"none",color:isCat?"#555":"#333",padding:"2px 0",paddingLeft:isCat?0:12,outline:"none",fontFamily:"inherit",textOverflow:"ellipsis"}}/>
                      {warn&&<span
                      ...
                      {(function(){
                        var pr=procRisk(row);
                        if(!pr||(!pr.late&&!pr.tight))return null;
                        return <span title={(pr.late?"Procurement LATE: ":"Tight: ")+pr.tender.title+" — delivery "+fmtDate(pr.delivery)+" vs start week of "+fmtDate(pr.startWeek)}
                          style={{fontSize:11,flexShrink:0,cursor:"help"}}>{pr.late?"🚚":"⏳"}</span>;
                      })()}
                      <button className="sched-noprint" onClick={function(e){
                        if(docsRowId===row.id){setDocsRowId(null);return;}
                        var b=e.currentTarget.getBoundingClientRect();
                        setDocsPos({x:b.right+8,y:b.top});
                        setDocsRowId(row.id);
                      }} title={isCat?"Documents for this room":"Documents for this task"}
                        style={{background:docsRowId===row.id?"#eef0ff":docCount(row)>0?"#f0f0ff":"none",
                          border:"1px solid "+(docsRowId===row.id?"#3949ab":docCount(row)>0?"#c5cae9":"#e8e6df"),
                          borderRadius:4,cursor:"pointer",color:docCount(row)>0?"#3949ab":"#888",
                          fontSize:10,padding:"1px 5px",flexShrink:0,fontWeight:700}}>
                        📎{docCount(row)>0?docCount(row):""}
                      </button>
                      {docCount(row)>0&&<span className="sched-print-only" style={{fontSize:8,color:"#3949ab",fontWeight:700}}>📎{docCount(row)}</span>}
                  ...
                  <td className="sched-noprint" style={{padding:"3px 8px",borderBottom:"1px solid #f5f4f0",background:isCat?"#f0ede6":"#fff",whiteSpace:"nowrap"}}>
                    {isCat
                      ?<span style={{fontSize:10,color:"#ccc"}}>—</span>
                      :<select value={row.group||""} onChange={function(e){
                        if(e.target.value==="__new"){promptSubcontractor(row.id);return;}
                        updRow(row.id,{group:e.target.value});
                      }}
                        title="Subcontractor / trade for this task"
                        style={{width:136,padding:"2px 4px",fontSize:10,border:"1px solid #e8e6df",borderRadius:4,fontFamily:"inherit",color:row.group?"#00695c":"#bbb",fontWeight:row.group?700:400}}>
                        <option value="">👷 —</option>
                        {allGroups.map(function(g){return <option key={g} value={g}>{g}</option>;})}
                        {subSuggestions.length>0&&<optgroup label="Registered subcontractors">
                          {subSuggestions.map(function(g){return <option key={g} value={g}>{g}</option>;})}
                        </optgroup>}
                        <option value="__new">＋ New subcontractor…</option>
                      </select>}
                  </td>
                  <td className="sched-noprint" style={{padding:"3px 8px",borderBottom:"1px solid #f5f4f0",background:isCat?"#f0ede6":"#fff",whiteSpace:"nowrap"}}>
                    {isCat
                      ?<div style={{display:"flex",alignItems:"center",gap:4}}>
                        {/* A category IS a room — the app keeps them 1:1, so there is nothing to pick here. */}
                        <span title="This category is the room" style={{fontSize:10,fontWeight:700,color:"#7b1fa2",overflow:"hidden",textOverflow:"ellipsis",maxWidth:96}}>
                          🚪 {(function(){var rm=(rooms||[]).find(function(x){return x.id===row.roomId;});return rm?rm.name:(row.label||"—");})()}
                        </span>
                        <button onClick={function(e){
                          if(copyCatId===row.id){setCopyCatId(null);return;}
                          var b=e.currentTarget.getBoundingClientRect();
                          setCopyPos({x:b.left,y:b.bottom+6});
                          setCopyCatId(row.id);
                        }} title="Copy this whole sequence onto another room"
                          style={{background:copyCatId===row.id?"#e8f0fe":"none",border:"1px solid "+(copyCatId===row.id?"#1a73e8":"#e8e6df"),borderRadius:4,cursor:"pointer",color:"#1a73e8",fontSize:10,padding:"1px 5px",flexShrink:0}}>⧉</button>
                      </div>
                      :<select value={row.roomId||""} onChange={function(e){updRow(row.id,{roomId:e.target.value});}}
                        title="Room this task belongs to"
                        style={{width:126,padding:"2px 4px",fontSize:10,border:"1px solid #e8e6df",borderRadius:4,fontFamily:"inherit",color:row.roomId?"#7b1fa2":"#bbb"}}>
                        <option value="">🚪 —</option>
                  ...
                  <td className="sched-noprint" style={{padding:"3px 8px",borderBottom:"1px solid #f5f4f0",background:isCat?"#f0ede6":"#fff",whiteSpace:"nowrap"}}>
                    {isCat
                      ...
                      :<select value={row.afterId||""} onChange={function(e){updRow(row.id,{afterId:e.target.value});}}
                        title="This task starts after…"
                        style={{width:136,padding:"2px 4px",fontSize:10,border:"1px solid #e8e6df",borderRadius:4,fontFamily:"inherit",color:row.afterId?"#1a73e8":"#bbb"}}>
                        <option value="">— none —</option>
                  ...
                  <td style={{padding:"3px 8px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df",background:isCat?"#f0ede6":"#fff",whiteSpace:"nowrap",textAlign:"right"}}>
                    {(function(){
                      var sp=rowSpan(row);
                      if(isCat||!sp||sp.rate===null)return <span style={{color:"#ddd",fontSize:10}}>—</span>;
                      return <span title={"Quantity "+row.qty+" "+(row.unit||"")+" over "+sp.workingWeeks+" working week"+(sp.workingWeeks!==1?"s":"")}
                        style={{fontSize:10,fontWeight:800,color:"#b45309",cursor:"help"}}>
                  ...
                  {wks.map(function(wk){
                    ...
                    return <td key={wk}
                      ...
                      style={Object.assign({borderBottom:"1px solid #f5f4f0",borderLeft:"1px solid #f0ede6",cursor:(isCat||hol)?"default":"pointer",height:22,padding:0,textAlign:"center",fontSize:8,fontWeight:800,color:"#fff",textShadow:"0 0 2px rgba(0,0,0,.5)"},
                        hol?{background:"repeating-linear-gradient(45deg,#e8e4da,#e8e4da 3px,#dcd8ce 3px,#dcd8ce 6px)"}:cellStyle(v))}>
                      {hasWp?wpVal:""}
                    </td>;
                  })}
                </tr>;
        ...
        {cards&&<div className="sched-cards" style={{display:"flex",flexDirection:"column",gap:10}}>
          {(sc.rows||[]).length===0&&<div style={{padding:"14px",textAlign:"center",color:"#bbb",fontSize:12,border:"1px solid #e8e6df",borderRadius:10,background:"#fff"}}>No rows yet. Add a category or a task below.</div>}
          {cardBlocks().map(function(blk){
            var cat=blk.cat;
            var catCollapsed=cat&&!!collapsedCats[cat.id];
            var catCount=(function(){
              if(!cat)return blk.tasks.length;
              var all=scopedRows;var i=all.findIndex(function(r){return r.id===cat.id;});
              var n=0;for(var j=i+1;j<all.length&&all[j].kind!=="category";j++)n++;
              return n;
            })();
            return <div key={cat?cat.id:"__nocat"} style={{border:"1px solid #e8e6df",borderRadius:10,overflow:"hidden",background:"#fff"}}>
              {cat&&<div onClick={function(){toggleCat(cat.id);}}
                style={{display:"flex",alignItems:"center",gap:8,padding:"11px 12px",background:"#f0ede6",cursor:"pointer"}}>
                <span style={{fontSize:11,color:"#888",fontWeight:800}}>{catCollapsed?"▸":"▾"}</span>
                <span style={{flex:1,fontWeight:800,fontSize:13,textTransform:"uppercase",color:"#555",letterSpacing:".3px"}}>🚪 {cat.label||"(untitled)"}</span>
                {(function(){var pg=rowProgress(cat);return pg===null?null:<span style={{fontSize:10,fontWeight:800,padding:"2px 7px",borderRadius:9,background:pg>=100?"#e8f5e9":"#fff",color:pg>=100?"#2e7d32":"#888"}}>{pg}%</span>;})()}
                <span style={{fontSize:10,color:"#888"}}>{catCount} task{catCount!==1?"s":""}</span>
              </div>}
              {blk.tasks.map(function(row){
                var warn=rowWarning(row.id);
                var warnColor=warn?(warn.severity==="blocking"?"#c62828":warn.severity==="late"?"#e65100":"#f57f17"):"";
                var pg=rowProgress(row);
                var sp=rowSpan(row);
                var pr=procRisk(row);
                return <div key={row.id} style={{padding:"11px 12px",borderTop:"1px solid #f0ede6"}}>
                  <div style={{display:"flex",alignItems:"flex-start",gap:6}}>
                    <input type="text" value={row.label||""} onChange={function(e){updRow(row.id,{label:e.target.value});}}
                      style={{flex:1,minWidth:0,border:"none",background:"transparent",fontSize:14,fontWeight:600,color:"#333",padding:0,outline:"none",fontFamily:"inherit"}}/>
                    {warn&&<span title={warn.acts.length+" open action"+(warn.acts.length!==1?"s":"")+(warn.blocking>0?" — "+warn.blocking+" blocking":"")+(warn.late>0?" — "+warn.late+" overdue":"")} style={{fontSize:14,lineHeight:1,color:warnColor,flexShrink:0}}>{warn.severity==="blocking"?"🔴":"⚠️"}</span>}
                    {pr&&(pr.late||pr.tight)&&<span title={(pr.late?"Procurement LATE: ":"Tight: ")+pr.tender.title} style={{fontSize:13,flexShrink:0}}>{pr.late?"🚚":"⏳"}</span>}
                    {pg!==null&&<span style={{fontSize:11,fontWeight:800,padding:"2px 8px",borderRadius:10,flexShrink:0,
                      background:pg>=100?"#e8f5e9":pg>0?"#e3f2fd":"#f5f4f0",color:pg>=100?"#2e7d32":pg>0?"#1565c0":"#bbb"}}>{pg}%</span>}
                  </div>

                  <div style={{display:"flex",flexWrap:"wrap",gap:6,alignItems:"center",marginTop:8}}>
                    {cat&&<span style={{fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:9,background:"#f3e5f5",color:"#7b1fa2",letterSpacing:".3px"}}>🚪 {(cat.label||"").toUpperCase()}</span>}
                    <select value={row.group||""} onChange={function(e){
                      if(e.target.value==="__new"){promptSubcontractor(row.id);return;}
                      updRow(row.id,{group:e.target.value});
                    }} style={{width:"auto",maxWidth:150,padding:"3px 6px",fontSize:11,border:"1px solid #e8e6df",borderRadius:6,fontFamily:"inherit",color:row.group?"#00695c":"#bbb",fontWeight:row.group?700:400}}>
                      <option value="">👷 —</option>
                      {allGroups.map(function(g){return <option key={g} value={g}>{g}</option>;})}
                      {subSuggestions.length>0&&<optgroup label="Registered subcontractors">
                        {subSuggestions.map(function(g){return <option key={g} value={g}>{g}</option>;})}
                    ...
                    <span style={{fontSize:11,color:row.startWeek?"#666":"#ccc",fontWeight:600}}>
                      📅 {row.startWeek?fmtDate(row.startWeek):"—"} → {row.endWeek?fmtDate(row.endWeek):"—"}
                    </span>
                    {sp&&sp.rate!==null&&<span style={{fontSize:11,fontWeight:800,color:"#b45309"}}>{sp.rate.toLocaleString(undefined,{maximumFractionDigits:1})} {row.unit||""}/wk</span>}
                  </div>

                  <div style={{display:"flex",gap:1,marginTop:8,height:18,borderRadius:5,overflow:"hidden",border:"1px solid #f0ede6",background:"#fafaf8"}}>
                    {wks.map(function(wk){
                      var v=(row.cells||{})[wk];
                      var hol=isHoliday(wk);
                      var isCur=(function(){var e=new Date(wk);e.setDate(e.getDate()+6);return wk<=todayStr&&todayStr<=toISO(e);})();
                      var wp=(row.weekProgress||{})[wk];
                      return <div key={wk} onClick={function(){
                          if(hol)return;
                          if(paintMode==="progress"){
                            var input=window.prompt("Progress reached at week of "+fmtDate(wk)+" (%)\nLeave empty to clear:",wp!==undefined&&wp!==""?String(wp):"");
                            if(input===null)return;
                            setWeekProgress(row,wk,input.trim());
                          }else{
                            toggleCell(row,wk);
                        ...
                        title={fmtDate(wk)+(hol?" — neutralised":"")+(wp!==undefined&&wp!==""?" — "+wp+"%":"")}
                        style={Object.assign({flex:1,minWidth:0,cursor:hol?"default":"pointer",
                          boxShadow:isCur?"inset 0 0 0 1.5px #f57f17":"none",
                          fontSize:7,fontWeight:800,color:"#fff",textAlign:"center",lineHeight:"18px",overflow:"hidden"},
                          hol?{background:"repeating-linear-gradient(45deg,#e8e4da,#e8e4da 3px,#dcd8ce 3px,#dcd8ce 6px)"}:cellStyle(v))}>
                        {wp!==undefined&&wp!==""?wp:""}
                      </div>;
                    })}
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:8,color:"#bbb",marginTop:2}}>
                    <span>{wks.length?fmtDate(wks[0]):""}</span>
                    <span>{wks.length?fmtDate(wks[wks.length-1]):""}</span>
                  </div>

                  <div className="sched-noprint" style={{display:"flex",gap:6,marginTop:9,flexWrap:"wrap"}}>
                    <button className="btn btn-sm" onClick={function(e){
                      if(spanRowId===row.id){setSpanRowId(null);return;}
                      var b=e.currentTarget.getBoundingClientRect();
                      setSpanPos({x:Math.max(8,b.left-140),y:b.bottom+6});
                      setSpanRowId(row.id);
                    }} style={{padding:"4px 10px",fontSize:11}}>📐 Dates</button>
                    <button className="btn btn-sm" onClick={function(e){
                      if(linkRowId===row.id){setLinkRowId(null);return;}
                      var b=e.currentTarget.getBoundingClientRect();
                      setLinkPos({x:Math.max(8,b.left-140),y:b.bottom+6});
                      setLinkRowId(row.id);
                    }} style={{padding:"4px 10px",fontSize:11}}>⚑ Actions</button>
                    <button className="btn btn-sm" onClick={function(e){
                      if(docsRowId===row.id){setDocsRowId(null);return;}
                      var b=e.currentTarget.getBoundingClientRect();
                      setDocsPos({x:Math.max(8,b.left-140),y:b.bottom+6});
                      setDocsRowId(row.id);
                    }} style={docCount(row)>0?{padding:"4px 10px",fontSize:11,color:"#3949ab",borderColor:"#c5cae9"}:{padding:"4px 10px",fontSize:11}}>📎{docCount(row)>0?" "+docCount(row):""}</button>
                    <button className="btn btn-sm" onClick={function(){shiftRow(row.id,-1);}} title="Shift 1 week earlier" style={{padding:"4px 10px",fontSize:11}}>◀</button>
                    <button className="btn btn-sm" onClick={function(){shiftRow(row.id,1);}} title="Shift 1 week later" style={{padding:"4px 10px",fontSize:11}}>▶</button>
                    <button className="btn btn-sm btn-danger" onClick={function(){delRow(row.id);}} style={{padding:"4px 10px",fontSize:11,marginLeft:"auto"}}>🗑</button>
                  </div>
                </div>;
              })}
              {blk.tasks.length===0&&!catCollapsed&&<div style={{padding:"10px 12px",fontSize:11,color:"#bbb"}}>No task in this room{only3w?" within 3 weeks":""}.</div>}
            </div>;
          })}
        </div>}

        {copyCatId&&<div className="sched-noprint" onClick={function(){setCopyCatId(null);}} style={{position:"fixed",inset:0,zIndex:790}}></div>}
        {copyCatId&&(function(){
          var row=(sc.rows||[]).find(function(r){return r.id===copyCatId;});
          if(!row)return null;
          var targets=(rooms||[]).filter(function(rm){return rm.zone===curZone&&rm.id!==row.roomId;});
          return <div className="sched-noprint" onClick={function(e){e.stopPropagation();}}
            style={{position:"fixed",zIndex:800,left:Math.min(copyPos.x,window.innerWidth-300),top:Math.min(copyPos.y,window.innerHeight-260),
              width:280,maxHeight:240,overflowY:"auto",padding:"12px 14px",background:"#f0f8ff",border:"1.5px solid #64b5f6",borderRadius:10,boxShadow:"0 8px 28px rgba(0,0,0,.25)"}}>
            <div style={{fontWeight:700,fontSize:12,color:"#1565c0",marginBottom:8}}>⧉ Copy “{row.label||"(untitled)"}” to…</div>
            <div style={{fontSize:10,color:"#888",marginBottom:8}}>Copies every task of this sequence onto another room, keeping the links and clearing the dates.</div>
            {targets.length===0&&<div style={{fontSize:11,color:"#bbb"}}>No other room in {curZone}.</div>}
            {targets.map(function(rm){
              return <button key={rm.id} className="btn btn-sm" style={{display:"block",width:"100%",textAlign:"left",marginBottom:4,fontSize:11}}
                onClick={function(){copySequenceToRoom(row.id,rm.id);setCopyCatId(null);}}>🚪 {rm.name}</button>;
            })}
          </div>;
        })()}
        ...
        {spanRowId&&(function(){
          ...
          return <div className="sched-noprint" onClick={function(e){e.stopPropagation();}}
            style={{position:"fixed",zIndex:800,
              left:Math.min(spanPos.x,window.innerWidth-330),
              top:Math.min(spanPos.y,window.innerHeight-300),
              width:310,padding:"12px 14px",background:"#fffdf0",border:"1.5px solid #c9a84c",borderRadius:10,
            ...
            <div style={{marginBottom:8}}>
              <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase"}}>Subcontractor / trade</label>
              <select value={row.group||""} onChange={function(e){
                if(e.target.value==="__new"){promptSubcontractor(row.id);return;}
                updRow(row.id,{group:e.target.value});
            ...
            <div style={{marginBottom:8}}>
              ...
              <select value={row.tenderRef||""} onChange={function(e){updRow(row.id,{tenderRef:e.target.value});if(e.target.value)pushStartOnSiteFromSchedule(e.target.value);}} style={{padding:"4px 6px",fontSize:11}}>
                <option value="">— none —</option>
        ...
        {docsRowId&&<div className="sched-noprint" onClick={function(){setDocsRowId(null);}} style={{position:"fixed",inset:0,zIndex:790}}></div>}
        {docsRowId&&(function(){
          var row=(sc.rows||[]).find(function(r){return r.id===docsRowId;});
          if(!row)return null;
          return <div className="sched-noprint" onClick={function(e){e.stopPropagation();}}
            style={{position:"fixed",zIndex:800,
              left:Math.min(docsPos.x,window.innerWidth-390),
              top:Math.min(docsPos.y,window.innerHeight-280),
              width:370,maxHeight:300,overflowY:"auto",
              padding:"12px 14px",background:"#f5f6ff",border:"1.5px solid #3949ab",borderRadius:10,
              boxShadow:"0 8px 28px rgba(0,0,0,.25)"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
              <div style={{fontWeight:700,fontSize:13,color:"#283593"}}>
                📎 {row.kind==="category"?"Room":"Task"} documents — {row.label||"(untitled)"}
              </div>
              <button className="btn btn-sm" onClick={function(){setDocsRowId(null);}}>✕</button>
            </div>
            <DocsEditor links={row.links} onChange={function(ls){updRow(row.id,{links:ls});}}/>
          </div>;
        })()}
          ...
          return <div className="sched-noprint" onClick={function(e){e.stopPropagation();}}
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
              <input type="text" value={newActionText} onChange={function(e){setNewActionText(e.target.value);}}
                onKeyDown={function(e){if(e.key==="Enter"){createLinkedAction(row.id,row.label,newActionText,newActionBlocking);setNewActionText("");}}}
                placeholder="New action for this task…" style={{flex:1,minWidth:200,padding:"5px 10px",fontSize:12}}/>
              <label style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer",textTransform:"none",letterSpacing:"normal",fontSize:11,fontWeight:700,padding:"4px 10px",borderRadius:16,border:"1.5px solid "+(newActionBlocking?"#c62828":"#ddd"),background:newActionBlocking?"#fce4ec":"#fff",color:newActionBlocking?"#c62828":"#888"}}>
                <input type="checkbox" checked={newActionBlocking} onChange={function(e){setNewActionBlocking(e.target.checked);}} style={{width:13,height:13}}/>
                Blocking Point
              </label>
              <button className="btn btn-sm btn-gold" disabled={!newActionText.trim()}
                onClick={function(){createLinkedAction(row.id,row.label,newActionText,newActionBlocking);setNewActionText("");}}>＋ Create</button>
            </div>

            {candidates.length>0&&<div style={{marginBottom:10}}>
              <div style={{fontSize:10,fontWeight:800,color:"#888",textTransform:"uppercase",marginBottom:4}}>Or link an existing {curZone} action ({candidates.length})</div>
              <input type="text" value={linkSearch} onChange={function(e){setLinkSearch(e.target.value);}}
                placeholder="🔍 Search an action by text or owner…" style={{width:"100%",padding:"5px 9px",fontSize:12,marginBottom:5}}/>
              {(function(){
                var q=linkSearch.trim().toLowerCase();
                var hits=q?candidates.filter(function(t){
                  return (t.text||"").toLowerCase().indexOf(q)>=0||(t.owner||"").toLowerCase().indexOf(q)>=0;
                }):candidates;
                if(hits.length===0)return <div style={{fontSize:11,color:"#bbb",padding:"4px 2px"}}>No matching action.</div>;
                return <div style={{maxHeight:150,overflowY:"auto",border:"1px solid #d6e6fa",borderRadius:7,background:"#fff"}}>
                  {hits.slice(0,50).map(function(t){
                    var isB=(t.tags||[]).includes("Blocking Point");
                    return <div key={t.id} onClick={function(){linkTaskToRow(t.id,row.id);setLinkSearch("");}}
                      style={{display:"flex",alignItems:"center",gap:6,padding:"5px 9px",borderBottom:"1px solid #f5f4f0",cursor:"pointer",fontSize:11}}
                      onMouseEnter={function(e){e.currentTarget.style.background="#eaf3ff";}}
                      onMouseLeave={function(e){e.currentTarget.style.background="transparent";}}>
                      <span style={{flex:1,minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{isB&&"🔴 "}{t.text}</span>
                      {t.owner&&<span style={{fontSize:10,color:"#888",flexShrink:0}}>{t.owner.split(",")[0]}</span>}
                      {t.due&&<span style={{fontSize:10,color:"#888",flexShrink:0}}>{fmtDate(t.due)}</span>}
                    </div>;
                  })}
                  {hits.length>50&&<div style={{padding:"4px 9px",fontSize:10,color:"#bbb"}}>+{hits.length-50} more — refine the search.</div>}
                </div>;
              })()}
            </div>}
...
function ZoneView({tasks,saveTasks,rooms,saveRooms,zones,people,tags,memory,setMemory,peopleEmails,defaultCC,kpis,saveKpis,meetings,saveMeetings,zoneOwners,schedules,saveSchedules,tenders,saveTenders,pkgOwners,contractors,pkgSubcontractors}){
  var mem=memory||{};
  const [curZone,setCurZone]=useState(mem.curZone||(zones||[])[0]||"");
  const [subTab,setSubTab]=useState(mem.subTab||"actions");
  const [fTags,setFTags]=useState(mem.fTags||[]);
  ...
  function syncRoomsFromCategories(){
    ...
    if(created>0)saveRooms(allRooms);
    saveSchedules(updatedScheds);
    safeAlert(created>0?created+" room"+(created!==1?"s":"")+" created from schedule categories.":"All categories are already linked to a room.");
  }
  ...
  // The same task label ("Blockwork", "Screed") is repeated under several category/rooms of the
  // schedule, so an action on its own does not say WHERE it happens. Resolve the owning
  // category (= room) of the schedule row the action is linked to, and fall back to the row's
  // roomId, then to a single blocked room.
  function actionContext(t){
    if(!t)return "";
    if(t.scheduleRowRef){
      var scheds=(schedules||[]).filter(function(s){return s.zone===curZone;});
      for(var i=0;i<scheds.length;i++){
        var rws=scheds[i].rows||[];
        var idx=-1;
        for(var k=0;k<rws.length;k++){if(rws[k].id===t.scheduleRowRef){idx=k;break;}}
        if(idx<0)continue;
        var row=rws[idx];
        if(row.kind==="category")return row.label||"";
        for(var j=idx;j>=0;j--){if(rws[j].kind==="category")return rws[j].label||"";}
        if(row.roomId){
          var rm=(rooms||[]).find(function(x){return x.id===row.roomId;});
          if(rm)return rm.name||"";
        }
        return "";
      }
    }
    if(Array.isArray(t.blockedRooms)&&t.blockedRooms.length===1){
      var rm2=(rooms||[]).find(function(x){return x.id===t.blockedRooms[0];});
      if(rm2)return rm2.name||"";
    }
    return "";
  }
  ...
  var CATEGORY_ORDER=["Blocking Point","Top Management",...( tags||[]).filter(function(t){return t!=="Blocking Point"&&t!=="Top Management";})];
  var groups={};
  filteredActions.forEach(function(t){
    var cat=CATEGORY_ORDER.find(function(catName){return (t.tags||[]).includes(catName);})||"General";
  ...
  function buildZoneReport(){
    ...
    // Completed actions are counted but not listed: the MoM is a working document
    // about what is still open, and listing closed items buries the rest.
    lookaheadActions=lookaheadActions.filter(function(t){return t.status!=="done";});
    var pending=lookaheadActions.length;
    ...
    lines.push(pending+" open action"+(pending!==1?"s":"")+" due within 3 weeks"+(done>0?" - "+done+" completed (not listed)":"")+relStr);
    lines.push("");
    ...
    lookaheadCats.forEach(function(cat){
      ...
      items.forEach(function(t){
        ...
        var ctx=actionContext(t);
        lines.push(mark+(ctx?"["+ctx.toUpperCase()+"] ":"")+t.text+owner+due+rooms);
      });
      lines.push("");
    });
  ...
  return <div style={{padding:"16px 20px",overflowY:"auto",flex:1}}>
    <div className="page-hdr">
      <div>
        <div className="page-title">🏢 Zone Workspace</div>
        <div className="page-sub">Dedicated space for zone pilots — actions &amp; room-by-room progress{zoneLeadersOf(zoneOwners,curZone).length>0?" · Leaders: "+zoneLeadersOf(zoneOwners,curZone).map(function(p){return p.split(",")[0];}).join(", "):""}</div>
      </div>
    ...
    {subTab==="actions"&&<div>
      <div className="card" style={{marginBottom:14}}>
        ...
        {currentMeeting&&showAttendance&&<div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
            ...
            <input type="date" min="1990-01-01" max="2200-12-31" value={currentMeeting.date||""} onChange={function(e){setMeetingDate(e.target.value);}} style={{padding:"3px 7px",fontSize:11,border:"1px solid #e8e6df",borderRadius:5}}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:4}}>
            {(people||[]).map(function(p){
              var st=(currentMeeting.attendance||{})[p]||"";
              var c=ownerColor(p);
              return <div key={p} style={{display:"flex",alignItems:"center",gap:5,padding:"3px 6px",borderRadius:6,background:st?"#fafaf8":"transparent"}}>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:10}}>
          {(tags||[]).map(function(tg){var on=qTags.includes(tg);var tc=tagColor(tg);return <button key={tg} onClick={function(){toggleQTag(tg);}} style={{padding:"2px 9px",borderRadius:12,border:"1.5px solid "+(on?tc.color:"#ddd"),background:on?tc.bg:"#fff",color:on?tc.color:"#bbb",fontFamily:"inherit",fontSize:11,fontWeight:700,cursor:"pointer"}}>{tg}</button>;})}
        </div>
      ...
      {orderedCats.map(function(cat){
        ...
        var tc=cat==="Blocking Point"?{bg:"#fce4ec",color:"#c62828"}:cat==="General"?{bg:"#f5f4f0",color:"#888"}:tagColor(cat);
        return <div key={cat} style={{marginBottom:18}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,paddingBottom:6,borderBottom:"2px solid "+tc.color}}>
            <span style={{padding:"3px 12px",borderRadius:20,background:tc.bg,color:tc.color,fontWeight:800,fontSize:12,letterSpacing:".3px"}}>{cat==="Blocking Point"?"🚧 ":""}{cat.toUpperCase()}</span>
          ...
          {items.map(function(t){
            return <ZoneActionRow key={t.id} task={t} rooms={zoneRooms} context={actionContext(t)}
    ...
    {subTab==="rooms"&&<div>
      <div className="card sched-noprint" style={{marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <button className="btn btn-sm" onClick={syncRoomsFromCategories} title="Create a room for every schedule category that doesn't have one yet">🔄 Sync from schedule</button>
          ...
          {zoneRooms.map(function(r){
            ...
            return <div key={r.id} className="card" style={{marginBottom:0,padding:"12px 14px",borderColor:isBlocked?"#f48fb1":"#e8e6df",background:isBlocked?"#fff5f7":"#fff"}}>
              ...
              {prog!==null&&<div style={{marginBottom:8}}>
                ...
                <div style={{fontSize:9,color:"#aaa",marginTop:2}}>{prog.doneRows}/{prog.totalRows} task{prog.totalRows!==1?"s":""} complete</div>
              </div>}
              ...
              {isBlocked&&<div style={{display:"flex",flexDirection:"column",gap:3}}>
                {blockers.map(function(b){return <div key={b.id} style={{fontSize:11,color:"#c62828",background:"#fce4ec",padding:"3px 8px",borderRadius:5,fontWeight:600}}>🚧 {b.text}</div>;})}
              </div>}
          ...
          <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:8,marginBottom:16}}>
            {weeklyPPI.slice(0,12).map(function(w){
              ...
              return <div key={w.monday} onClick={function(){setExpandedWeek(isOpen?null:w.monday);}} style={{flex:"0 0 100px",cursor:"pointer",padding:"10px 8px",borderRadius:10,border:"1.5px solid "+(isOpen?ppiColor(w.ppi):"#e8e6df"),background:w.isCurrent?"#fff8e1":isOpen?"#fafaf8":"#fff",textAlign:"center"}}>
                <div style={{fontSize:9,color:"#aaa",fontWeight:700}}>{fmtDate(w.monday)}</div>
                <div style={{fontSize:20,fontWeight:900,color:w.isCurrent?"#f57f17":ppiColor(w.ppi),marginTop:4}}>{w.ppi!==null?w.ppi+"%":"—"}</div>
                <div style={{fontSize:9,color:"#888",marginTop:2}}>{w.kept}/{w.promised}</div>
                {w.isCurrent&&<div style={{fontSize:8,fontWeight:800,color:"#f57f17",marginTop:2}}>IN PROGRESS</div>}
              </div>;
            })}
          </div>
        ...
        {showKpiForm&&<div style={{marginTop:10}}>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:8}}>
            <div style={{flex:2,minWidth:160}}>
              <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase"}}>KPI name</label>
            ...
            <div style={{flex:1,minWidth:100}}>
              <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase"}}>Total target</label>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
            <div style={{flex:1,minWidth:130}}>
              <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase"}}>Start date</label>
            ...
            <div style={{flex:1,minWidth:130}}>
              <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase"}}>End date</label>
          ...
          return <div key={kpi.id} className="card" style={{marginBottom:10,padding:0,overflow:"hidden"}}>
            <div onClick={function(){setExpandedKpi(isOpen?null:kpi.id);}} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",cursor:"pointer",background:isOpen?"#fafaf8":"#fff",flexWrap:"wrap"}}>
              ...
              <div style={{textAlign:"right"}}>
                ...
                <div style={{fontSize:9,color:behindSchedule?"#c62828":"#888",fontWeight:behindSchedule?700:400}}>{behindSchedule?"behind schedule":"on/ahead of schedule"}</div>
              </div>
            </div>
            ...
            {isOpen&&<div style={{padding:"14px 16px",borderTop:"1.5px solid #f0ede6"}}>
              <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end",marginBottom:12,padding:"10px 12px",background:"#fafaf8",borderRadius:8,border:"1px solid #e8e6df"}}>
                ...
                <div style={{flex:1,minWidth:120}}>
                  <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase"}}>Start</label>
                ...
                <div style={{flex:1,minWidth:120}}>
                  <label style={{fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase"}}>End</label>
              ...
              {(function(){
                ...
                return <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
                  <div className="card" style={{flex:1,minWidth:150,marginBottom:0,padding:"8px 12px",background:"#f8f7f4"}}>
                    <div style={{fontSize:9,color:"#888",textTransform:"uppercase",fontWeight:700}}>Current avg. pace</div>
                  ...
                  <div className="card" style={{flex:1,minWidth:170,marginBottom:0,padding:"8px 12px",background:weeksLeftInPlan<=0&&remaining>0?"#fce4ec":"#e3f2fd"}}>
                    <div style={{fontSize:9,color:"#1565c0",textTransform:"uppercase",fontWeight:700}}>Rate needed to finish on time</div>
                  ...
                  <div className="card" style={{flex:1,minWidth:170,marginBottom:0,padding:"8px 12px",background:isLateVsPlan?"#fce4ec":"#f0fff4"}}>
                    <div style={{fontSize:9,color:isLateVsPlan?"#c62828":"#2e7d32",textTransform:"uppercase",fontWeight:700}}>Projected finish (current pace)</div>
                    ...
                    {weeks.map(function(w){
                      ...
                      return <tr key={w.monday} style={{background:w.isCurrent?"#fff8e1":"#fff"}}>
                        ...
                        <td style={{textAlign:"right"}}>
                          <input type="number" value={(kpi.weeklyActuals||{})[w.monday]||""} onChange={function(e){setKpiWeekActual(kpi.id,w.monday,e.target.value);}} placeholder="0" style={{width:80,padding:"2px 5px",fontSize:11,textAlign:"right"}}/>
                        </td>
...
{subTab==="schedule"&&<ScheduleView contractors={contractors} pkgSubcontractors={pkgSubcontractors} curZone={curZone} schedules={schedules} saveSchedules={saveSchedules} tasks={tasks} saveTasks={saveTasks} people={people} tags={tags} zones={zones} rooms={rooms} saveRooms={saveRooms} canEdit={canEditZoneSchedule(zoneOwners,curZone,window._currentUser?window._currentUser.name:"")} roomBlockersOf={roomBlockers} tenders={tenders} saveTenders={saveTenders} pkgOwners={pkgOwners}/>}
...
function App(){
  ...
  // Mirrors `people` so guards can compare against the last known good list
  // without re-creating savePeople on every render.
  const peopleRef=useRef([]);
  useEffect(function(){peopleRef.current=people;},[people]);
  // Set to true once the initial Firebase load has completed.
  // Until then no user-triggered write is allowed to reach the database.
  ...
  // Writes performed by the initial load itself (migrations, first-run seeding).
  // They bypass the loaded guard because by definition they run before it is set.
  const bootSync=useCallback(async(key,val)=>{
    try{await cloudStore.set(key,val);}catch(e){console.error("Boot sync failed",key,e);}
  },[]);

  const sync=useCallback(async(key,val)=>{
    // A write issued before the initial load has completed would push the local
    // placeholder state over whatever the database already holds. Refuse it.
    if(!loadedRef.current){console.warn("sync blocked before initial load: "+key);return;}
  ...
  const savePeople=d=>{
    var next=Array.isArray(d)?d.slice():[];
    // The admin profile must never be able to disappear from the list.
    if(!next.some(function(n){return isAppAdmin(n);}))next.unshift(APP_ADMIN);
    var prev=peopleRef.current||[];
    // Removing people one at a time from Settings is legitimate. A save that drops
    // several at once is almost always a stale client overwriting the shared list.
    if(prev.length>1&&next.length<prev.length-1){
      console.warn("savePeople blocked: refusing to drop "+(prev.length-next.length)+" people in one write");
      safeAlert("This change would remove "+(prev.length-next.length)+" people at once and has been blocked.\n\nReload the page and try again, removing them one at a time.");
      return;
    }
    setPeople(next);sync(KEYS.people,next);
  };
  ...
  useEffect(()=>{
    const load=async()=>{
      ...
      try{
        ...
        if(t){setTasks(t);}
          ...
          if(oldTasks&&oldTasks.length){
            ...
            setTasks(migrated);bootSync(KEYS.tasks,migrated);
          }
        }
        if(x){setTrackers(x);}
        else{
          ...
          if(migratedTrackers.length){setTrackers(migratedTrackers);bootSync(KEYS.trackers,migratedTrackers);}
        }
        if(td)setTenders(td);
        ...
        if(p&&p.length){setPeople(p);}
          ...
          const finalP=(op&&op.length)?op:SEED_PEOPLE;
          // Shown locally only. A null read can mean "node absent" or "read failed",
          // and the two are indistinguishable here — writing seed data back would
          // destroy the real list in the second case. The first explicit edit in
          // Settings persists whatever is on screen.
          setPeople(finalP);
        }
        ...
        if(zn&&zn.length&&zn.indexOf("External Works")===-1){
          ...
          setZones(zn2);bootSync(KEYS.zones,zn2);
        ...
        if(tr){setTagrules(tr);}
          ...
          if(otr){setTagrules(otr);bootSync(KEYS.tagrules,otr);}
        }
        if(pr){setPkgrules(pr);}
        else{
          const opr=await cloudStore.get("pkgrules");
          if(opr){setPkgrules(opr);bootSync(KEYS.pkgrules,opr);}
        }
        // Everything above succeeded, so local state now reflects the database.
        // Only from this point are user-triggered writes allowed through.
        loadedRef.current=true;
      }catch(e){
        console.error("Load error",e);
        // loadedRef stays false: the app runs read-only rather than risk
        // overwriting good cloud data with an incomplete local state.
        setSyncStatus("err");
        safeAlert("Could not load data from the database.\n\nThe app is running in read-only mode to protect the shared data. Reload the page to try again.");
      }
      ...
      // One-off backfill: rows painted before start/end week were derived from the
      // grid still have an empty span, which silenced procurement risk and the
      // tender start on site. Rebuild the span from the planned bars.
      try{
        var spanScheds=loadedScheds;
        if(spanScheds.length>0){
          var spanChanged=false;
          var fixedScheds=spanScheds.map(function(s){
            var wl=scheduleWeeks(s);
            if(!wl.length)return s;
            var rowChanged=false;
            var rows=(s.rows||[]).map(function(r){
              if(r.kind==="category")return r;
              if(r.startWeek&&r.endWeek)return r;
              var painted=wl.filter(function(w){var v=(r.cells||{})[w];return v==="plan"||v==="both";});
              if(!painted.length)return r;
              rowChanged=true;
              return Object.assign({},r,{startWeek:painted[0],endWeek:painted[painted.length-1]});
            });
            if(rowChanged){spanChanged=true;return Object.assign({},s,{rows:rows});}
            return s;
          });
          if(spanChanged){setSchedules(fixedScheds);sync(KEYS_SCHEDULES,fixedScheds);}
        }
      }catch(e){console.error("Schedule span backfill error",e);}
  ...
  const NAV=[
    ...
    {id:"submissions",icon:"📬",label:"Follow-up"},
  ...
  if(!hasFullAccess){
    ...
    var allowedViews=effectiveNAV.map(function(n){return n.id;});
  ...
  if(!currentUser)return <UserLogin people={people} onLogin={function(name){setCurrentUser(name);window._currentUser={name:name};}}/>;  return <div className="layout">
    ...
    <nav className="leftnav">
      ...
      {effectiveNAV.map(n=><button key={n.id} className={"navbtn"+(view===n.id?" on":"")} onClick={()=>setView(n.id)} title={n.label}>
