...
function newScheduleRow(kind,label){return{id:uuid(),kind:kind||"task",label:label||"",cells:{}};}
...
function ScheduleView({curZone,schedules,saveSchedules}){
  ...
  function addRow(){
    ...
    upd(sc.id,{rows:[...(sc.rows||[]),newScheduleRow(newRowKind,newRowLabel.trim())]});
    setNewRowLabel("");
  }
                ...
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  <td style={{position:"sticky",left:0,zIndex:1,background:isCat?"#f0ede6":"#fff",padding:"3px 10px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df"}}>
                    <input type="text" value={row.label||""} onChange={function(e){updRow(row.id,{label:e.target.value});}}
                      style={{width:"100%",border:"none",background:"transparent",fontSize:isCat?12:11,fontWeight:isCat?800:500,textTransform:isCat?"uppercase":"none",color:isCat?"#555":"#333",padding:"2px 0",paddingLeft:isCat?0:12,outline:"none",fontFamily:"inherit"}}/>
                  </td>
                  {wks.map(function(wk){
                    var v=(row.cells||{})[wk]||"";
...
{subTab==="schedule"&&<ScheduleView curZone={curZone} schedules={schedules} saveSchedules={saveSchedules}/>}
