...
function ZoneActionRow({task,rooms,onUpdate,onDelete,onOpenRoomModal,people,tags}){
  const [editMode,setEditMode]=useState(false);
  ...
  return <div>
    ...
    {!sc
      ...
      :<div>
        <div className="sched-noprint" style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end",marginBottom:10,padding:"10px 12px",background:"#fafaf8",borderRadius:8,border:"1px solid #e8e6df"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:4,paddingLeft:8,borderLeft:"1px solid #e0ddd6"}}>
            ...
            <input type="text" value={newGroup} onChange={function(e){setNewGroup(e.target.value);}}
              onKeyDown={function(e){if(e.key==="Enter"&&newGroup.trim()){upd(sc.id,{groups:[...new Set([...(sc.groups||[]),newGroup.trim()])]});setNewGroup("");}}}
              placeholder="+ new trade" style={{width:90,padding:"4px 6px",fontSize:10}}/>
          </div>
        ...
        <div className="sched-wrap" style={{overflow:"auto",maxHeight:"62vh",border:"1px solid #e8e6df",borderRadius:8,background:"#fff"}}>
              ...
              {visibleRows.map(function(row){
                ...
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  ...
                  <td className="sched-noprint" style={{padding:"3px 8px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df",background:isCat?"#f0ede6":"#fff",whiteSpace:"nowrap"}}>
                    {!isCat&&<select value={row.group||""} onChange={function(e){updRow(row.id,{group:e.target.value});}}
                      title="Trade / subcontractor group — used to export a filtered schedule"
                      style={{width:95,padding:"2px 4px",fontSize:10,border:"1px solid #e8e6df",borderRadius:4,fontFamily:"inherit",color:row.group?"#00695c":"#bbb",marginRight:3,fontWeight:row.group?700:400}}>
                      <option value="">👷 —</option>
                      {allGroups.map(function(g){return <option key={g} value={g}>{g}</option>;})}
                    </select>}
      ...
      {orderedCats.map(function(cat){
        ...
        return <div key={cat} style={{marginBottom:18}}>
          ...
          {items.map(function(t){
            return <ZoneActionRow key={t.id} task={t} rooms={zoneRooms}
