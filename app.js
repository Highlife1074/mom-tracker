...
function newScheduleRow(kind,label){return{id:uuid(),kind:kind||"task",label:label||"",cells:{},afterId:"",lagWeeks:0,roomId:""};}
                ...
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  <td style={{position:"sticky",left:0,zIndex:1,background:isCat?"#f0ede6":"#fff",padding:"3px 10px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df"}}>
                    <div style={{display:"flex",alignItems:"center",gap:3}}>
                      ...
                      {(function(){var pg=rowProgress(row);return pg===null?null:<span style={{fontSize:9,fontWeight:800,padding:"1px 5px",borderRadius:8,flexShrink:0,background:pg>=100?"#e8f5e9":pg>0?"#e3f2fd":"#f5f4f0",color:pg>=100?"#2e7d32":pg>0?"#1565c0":"#bbb"}}>{pg}%</span>;})()}
...
function ZoneView({tasks,saveTasks,rooms,saveRooms,zones,people,tags,memory,setMemory,peopleEmails,defaultCC,kpis,saveKpis,meetings,saveMeetings,zoneOwners,schedules,saveSchedules}){
  ...
  function roomProgress(roomId){
    ...
    rows.forEach(function(r){
      ...
      if(plan>0&&act>=plan)done++;
      else if(plan===0&&act>0)done++;
    });
    return{pct:Math.round(done/rows.length*100),doneRows:done,totalRows:rows.length};
  }
