...
const KEYS_BUDGET_PERMS="pp_budget_perms";
function canViewBudget(perms){
  var name=window._currentUser?window._currentUser.name:"";
  if(!name)return false;
  var p=(perms||window._ppBudgetPerms||{})[name]||{};
  return !!(p.view||p.edit);
}
function canEditBudget(perms){
  var name=window._currentUser?window._currentUser.name:"";
  if(!name)return false;
  var p=(perms||window._ppBudgetPerms||{})[name]||{};
  return !!p.edit;
...
const SEED_TAGS=["Contract","Design","HR","Letter","Procurement","Production","Quality External","Quality Podium","Quality Tower","RFI","Top Management"];
...
function MaterialsPanel({td,updTd,saveT,tasks,tenders,saveTenders,setSelTender}){
  ...
  function autoTask(mat,kind,targetDate){
    ...
    var taskText=kind+" to be submitted: "+mat.name+" — "+td.title;
    var exists=(tasks||[]).some(function(t){
      return t.tenderRef===td.id&&t.text===taskText&&t.status!=="done";
    });
    if(exists)return;
    var task=newTask({
      text:taskText,
          ...
          <div style={{marginBottom:4}}>
            ...
            {mssOpen&&<div>
              ...
              <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center",padding:"6px 8px",background:"#e8f0fe22",borderRadius:"6px 6px 0 0",border:"1px solid #e8e6df",borderBottom:"none",marginTop:4}}>
                ...
                <div style={{display:"flex",gap:3,alignItems:"center"}}><span style={{fontSize:9,color:"#555",fontWeight:600}}>Target date</span><input type="date" value={mat.mssTarget||""} onChange={function(e){updMat(mi,"mssTarget",e.target.value);if(e.target.value&&mat.mssStatus==="under preparation")autoTask(Object.assign({},mat,{mssTarget:e.target.value}),"MSS",e.target.value);}} style={{fontSize:10,padding:"2px 5px",border:"1px solid #bbdefb",borderRadius:4}}/></div>
          ...
          <div>
            ...
            {marOpen&&<div>
              ...
              <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center",padding:"6px 8px",background:"#f3e5f522",borderRadius:"6px 6px 0 0",border:"1px solid #ce93d8",borderBottom:"none",marginTop:4}}>
                ...
                <div style={{display:"flex",gap:3,alignItems:"center"}}><span style={{fontSize:9,color:"#555",fontWeight:600}}>Target date</span><input type="date" value={mat.marTarget||""} onChange={function(e){updMat(mi,"marTarget",e.target.value);if(e.target.value&&mat.marStatus==="under preparation")autoTask(Object.assign({},mat,{marTarget:e.target.value}),"MAR",e.target.value);}} style={{fontSize:10,padding:"2px 5px",border:"1px solid #ce93d8",borderRadius:4}}/></div>
          ...
          <td style={{minWidth:180}}>
            {s.special==="bidders"
              ...
              :<div style={{display:"flex",flexDirection:"column",gap:3}}>
                ...
                {s.key==="acc"&&canViewBudget()&&<div style={{display:"flex",gap:4}}>
                  <div style={{flex:1}}>
                    <div style={{fontSize:8,fontWeight:800,color:"#888",marginBottom:1}}>SUBCONT.</div>
                    <div style={{position:"relative",display:"flex",alignItems:"center"}}>
                      <input type="number" value={td.accAmountSubcontract||""} disabled={!canEditBudget()} onChange={function(e){var d=(tenders||[]).map(function(t){return t.id!==td.id?t:Object.assign({},t,{accAmountSubcontract:e.target.value});});saveTenders(d);setSelTender(d.find(function(t){return t.id===td.id;}));}} placeholder="0" style={{width:"100%",padding:"2px 24px 2px 5px",fontSize:10,border:"1px solid #e8e6df",borderRadius:4,background:canEditBudget()?"#fff":"#f5f5f5"}}/>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{position:"relative",display:"flex",alignItems:"center"}}>
                      <input type="number" value={td.accAmountOther||""} disabled={!canEditBudget()} onChange={function(e){var d=(tenders||[]).map(function(t){return t.id!==td.id?t:Object.assign({},t,{accAmountOther:e.target.value});});saveTenders(d);setSelTender(d.find(function(t){return t.id===td.id;}));}} placeholder="0" style={{width:"100%",padding:"2px 24px 2px 5px",fontSize:10,border:"1px solid #e8e6df",borderRadius:4,background:canEditBudget()?"#fff":"#f5f5f5"}}/>
                      <span style={{position:"absolute",right:3,fontSize:8,color:"#aaa",pointerEvents:"none"}}>{td.currency||"EUR"}</span>
                    </div>
                  </div>
                </div>}
...
function TendersView({tenders,saveTenders,packages,people,tasks,saveTasks,contractors,pkgOwners,jumpTender,clearJumpTender,jumpFrom,onBack}){
  var canViewBudg=canViewBudget();
  var canEditBudg=canEditBudget();
  ...
  if(selTender){
    ...
    return <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        {!canViewBudg
          ?<div className="card" style={{padding:"10px 14px",marginBottom:12,background:"#f5f5f5",color:"#aaa",fontSize:12,display:"flex",alignItems:"center",gap:6}}>🔒 Budget figures are restricted for your profile.</div>
          ...
          <div className="card" style={{flex:1,minWidth:120,marginBottom:0,padding:"10px 14px",background:"#f8f7f4"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              {canEditBudg
                ...
                :<div style={{fontSize:16,fontWeight:800}}>{td.budget?Number(td.budget).toLocaleString():"—"}</div>}
          ...
          <div className="card" style={{flex:1,minWidth:120,marginBottom:0,padding:"10px 14px",background:"#f0fff4"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              {canEditBudg
                ...
                :<div style={{fontSize:16,fontWeight:800,color:"#2e7d32"}}>{td.instructionAmount?Number(td.instructionAmount).toLocaleString():"—"}</div>}
              <span style={{fontSize:10,color:"#aaa"}}>{cur}</span>
            </div>
          </div>
  ...
  setTimeout(function(){
    ...
    {(function(){
      ...
      if(!canViewBudg)return <div className="card" style={{padding:"10px 14px",marginBottom:14,background:"#f5f5f5",color:"#aaa",fontSize:12,display:"flex",alignItems:"center",gap:6}}>🔒 Budget figures are restricted for your profile.</div>;
        ...
        <tbody>{filtered.map(function(td){
          ...
          return <tr key={td.id} style={{cursor:"pointer"}} onClick={function(){setSelTender(td);}}>
            ...
            <td style={{textAlign:"right",whiteSpace:"nowrap"}}>
              {!canViewBudg?<span style={{color:"#bbb",fontSize:11}}>🔒</span>:td.budget?<span style={{fontSize:11,fontWeight:600}}>{Number(td.budget).toLocaleString()}</span>:<span style={{color:"#ddd"}}>—</span>}
            </td>
            <td style={{textAlign:"right",whiteSpace:"nowrap"}}>
              {!canViewBudg?<span style={{color:"#bbb",fontSize:11}}>🔒</span>:(Number(td.accAmountSubcontract||0)+Number(td.accAmountOther||0))>0
            ...
            <td style={{textAlign:"right",whiteSpace:"nowrap"}}>
              {!canViewBudg?<span style={{color:"#bbb",fontSize:11}}>🔒</span>:td.instructionAmount?<span style={{fontSize:11,fontWeight:600,color:"#2e7d32"}}>{Number(td.instructionAmount).toLocaleString()}</span>:<span style={{color:"#ddd"}}>—</span>}
            </td>
            <td style={{textAlign:"right",whiteSpace:"nowrap"}}>
              {!canViewBudg?<span style={{color:"#bbb",fontSize:11}}>🔒</span>:(function(){
                var budget=Number(td.budget||0);
...
function SettingsView({tags,saveTags,people,savePeople,packages,savePackages,tagrules,saveTagrules,pkgrules,savePkgrules,apiKey,saveApiKey,improvements,saveImprovements,pkgOwners,savePkgOwners,userPrefs,saveUserPrefs,budgetPerms,saveBudgetPerms,allData,onImport}){
  ...
  return <div>
    ...
    {tab==="budget-access"&&<div className="card">
      <div style={{fontWeight:700,fontSize:14,marginBottom:4}}>💰 Budget Access</div>
      <div style={{fontSize:12,color:"#888",marginBottom:16}}>Control who can view and edit Budget figures (Budget, ACC proposed, Instructed, Variance, totals) across all tenders. Users without "View" will see 🔒 instead of amounts.</div>
      <table className="tbl">
        <thead><tr><th>Person</th><th style={{textAlign:"center"}}>View Budget</th><th style={{textAlign:"center"}}>Edit Budget</th></tr></thead>
        <tbody>{(people||[]).map(function(p){
          var cur=(budgetPerms||{})[p]||{};
          function toggle(field){
            var u=Object.assign({},budgetPerms||{});
            var pu=Object.assign({},cur);
            pu[field]=!pu[field];
            if(field==="edit"&&pu.edit)pu.view=true;
            if(field==="view"&&!pu.view)pu.edit=false;
            u[p]=pu;
            saveBudgetPerms(u);
          }
          return <tr key={p}>
            <td style={{fontWeight:600}}>{p.split(",")[0]}</td>
            <td style={{textAlign:"center"}}><input type="checkbox" checked={!!cur.view} onChange={function(){toggle("view");}} style={{width:16,height:16,cursor:"pointer"}}/></td>
            <td style={{textAlign:"center"}}><input type="checkbox" checked={!!cur.edit} onChange={function(){toggle("edit");}} style={{width:16,height:16,cursor:"pointer"}}/></td>
          </tr>;
        })}</tbody>
      </table>
    </div>}
  ...
  (tenders||[]).forEach(function(td){
    (td.materials||[]).forEach(function(mat){
      [{type:"mss",label:"MSS"},{type:"mar",label:"MAR"}].forEach(function(m){
        ...
        if(status!=="submitted"||!done)return;
...
function App(){
  ...
  const [budgetPerms,setBudgetPerms]=useState({});
  function saveBudgetPerms(d){setBudgetPerms(d);window._ppBudgetPerms=d;sync(KEYS_BUDGET_PERMS,d);}
  ...
  useEffect(()=>{
    const load=async()=>{
      try{
        const [t,x,td,ct,p,pk,g,tr,pr,imp,corr,awn,pko,prefs,bperms]=await Promise.all([
          ...
          cloudStore.get(KEYS.tags),cloudStore.get(KEYS.tagrules),cloudStore.get(KEYS.pkgrules),cloudStore.get(KEYS_IMP),cloudStore.get(KEYS_CORR),cloudStore.get(KEYS_AWN),cloudStore.get(KEYS.pkgowners),cloudStore.get(KEYS_PREFS),cloudStore.get(KEYS_BUDGET_PERMS)
        ]);
        if(t){setTasks(t);}
        else{
        ...
        if(bperms){setBudgetPerms(bperms);window._ppBudgetPerms=bperms;}
  ...
  window._ppBudgetPerms=budgetPerms;
  window._ppTags=tags;
  const NAV=[
