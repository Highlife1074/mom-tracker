...
function contractFinancials(ct){
  ...
  if(ct.startDate&&certified>0&&remaining>0){
    ...
    if(burn>0){var fd=new Date(now.getTime()+(remaining/burn)*30*24*60*60*1000);forecast=ftoISO(d);}
  }
    ...
    {filtered.length===0
      ...
      :<div>
        ...
        <div className="sched-print-only" style={{marginTop:12}}>
          {(function(){
            var zr=(rooms||[]).filter(function(rm){return rm.zone===curZone;});
            var blocked=zr.map(function(rm){return{room:rm,acts:roomBlockersOf?roomBlockersOf(rm.id):[]};}).filter(function(x){return x.acts.length>0;});
