...
function ClientSubmissionsView({tasks,tenders,contractors,packages,people,saveTasks,onNavTender}){
  const [fTender,setFTender]=useState("all");
  const [fPkg,setFPkg]=useState("all");
  const [fOwner,setFOwner]=useState("all");
  const [fStage,setFStage]=useState("all");
...
function MaterialsView({tenders,packages,people,saveTenders,onNavTender}){
  const [fPkg,setFPkg]=useState("all");
  const [fTender,setFTender]=useState("all");
  const [fStatus,setFStatus]=useState("all");
  const [q,setQ]=useState("");
...
function ZoneView({tasks,saveTasks,rooms,saveRooms,zones,people,tags}){
  const [curZone,setCurZone]=useState((zones||[])[0]||"");
  ...
  if(!currentUser)return <UserLogin people={people} onLogin={function(name){setCurrentUser(name);window._currentUser={name:name};}}/>;  return <div className="layout">
    ...
    <div className="main-area">
      <div className="content">
        ...
        {view==="zone"&&<ZoneView tasks={tasks} saveTasks={saveT} rooms={rooms} saveRooms={saveRooms} zones={zones} people={people} tags={tags}/>}
        {view==="materials"&&<MaterialsView tenders={tenders} packages={packages} people={people} saveTenders={saveTenders} onNavTender={navToTender}/>}
        {view==="submissions"&&<ClientSubmissionsView tasks={tasks} tenders={tenders} contractors={contractors} packages={packages} people={people} saveTasks={saveT} onNavTender={navToTender}/>}
