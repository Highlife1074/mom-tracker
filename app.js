            ...
            {localTags.includes("Blocking Point")&&<div style={{padding:"8px",background:"#fff5f7",borderRadius:7,border:"1px solid #f48fb1"}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"flex-end",flexWrap:"wrap"}}>
                <div style={{flex:1,minWidth:130}}>
                  ...
                  <select value={task.zone||""} onChange={function(e){upd("zone",e.target.value);upd("blockedRooms",[]);}} style={{padding:"3px 7px",fontSize:11,border:"1px solid #f48fb1",borderRadius:5,width:"100%"}}>
...
function ScheduleView({curZone,schedules,saveSchedules,tasks,saveTasks,people,tags,zones,rooms,saveRooms,canEdit,roomBlockersOf,tenders,saveTenders,pkgOwners,contractors,pkgSubcontractors}){
  ...
  function createLinkedAction(rowId,rowLabel,text,isBlocking){
    ...
    saveTasks([newTask({text:text.trim(),zone:curZone,scheduleRowRef:rowId,tags:tg,importance:isBlocking?3:2,urgence:isBlocking?3:2,note:"Linked to schedule row: "+rowLabel}),...(tasks||[])]);
  }
      ...
      :<div>
        ...
        {!cards&&<div className="sched-wrap" style={{overflow:"auto",maxHeight:"62vh",border:"1px solid #e8e6df",borderRadius:8,background:"#fff"}}>
          <table style={{borderCollapse:"collapse",width:"max-content",minWidth:"100%",fontSize:11}}>
            <thead>
              <tr>
                <th style={{position:"sticky",left:0,top:0,background:"#f5f4f0",zIndex:4,width:250,minWidth:250,maxWidth:250,textAlign:"left",padding:"6px 10px",borderBottom:"1.5px solid #e8e6df",fontSize:10,textTransform:"uppercase",color:"#888"}}>Task</th>
                ...
                <th className="sched-noprint" style={{position:"sticky",top:0,background:"#f5f4f0",zIndex:3,width:320,minWidth:320,maxWidth:320,textAlign:"left",padding:"6px 8px",borderBottom:"1.5px solid #e8e6df",fontSize:10,textTransform:"uppercase",color:"#888"}}>Starts after</th>
                {wks.map(function(wk){
            ...
            <tbody>
              {(sc.rows||[]).length===0&&<tr><td colSpan={wks.length+3} style={{padding:"14px",textAlign:"center",color:"#bbb",fontSize:12}}>No rows yet. Add a category or a task below.</td></tr>}
              {visibleRows.map(function(row){
                ...
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  <td style={{position:"sticky",left:0,zIndex:1,background:isCat?"#f0ede6":"#fff",padding:"3px 10px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df"}}>
                    <div style={{display:"flex",alignItems:"center",gap:3}}>
                      ...
                      <input type="text" value={row.label||""} onChange={function(e){updRow(row.id,{label:e.target.value});}}
                        style={{flex:1,border:"none",background:"transparent",fontSize:isCat?12:11,fontWeight:isCat?800:500,textTransform:isCat?"uppercase":"none",color:isCat?"#555":"#333",padding:"2px 0",paddingLeft:isCat?0:12,outline:"none",fontFamily:"inherit"}}/>
                      {warn&&<span
                  ...
                  <td style={{padding:"3px 8px",borderBottom:"1px solid #f5f4f0",background:isCat?"#f0ede6":"#fff",whiteSpace:"nowrap",textAlign:"right"}}>
                    {(function(){
                      var sp=rowSpan(row);
                  ...
                  <td className="sched-noprint" style={{padding:"3px 8px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df",background:isCat?"#f0ede6":"#fff",whiteSpace:"nowrap"}}>
                    {!isCat&&<select value={row.group||""} onChange={function(e){
                      if(e.target.value==="__new"){promptSubcontractor(row.id);return;}
                      updRow(row.id,{group:e.target.value});
                    }}
                      title="Subcontractor / trade for this task — pick one, or ＋ to create a new one"
                      style={{width:110,padding:"2px 4px",fontSize:10,border:"1px solid #e8e6df",borderRadius:4,fontFamily:"inherit",color:row.group?"#00695c":"#bbb",marginRight:3,fontWeight:row.group?700:400}}>
                      <option value="">👷 —</option>
                      {allGroups.map(function(g){return <option key={g} value={g}>{g}</option>;})}
