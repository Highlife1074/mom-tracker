...
function EmailModal({em,onClose}){
  ...
  return <div className="overlay"><div className="modal" style={{maxWidth:640}}>
    ...
    <div className="modal-body">
      {(toList.length>0||ccList.length>0)&&<div style={{marginBottom:12,padding:"10px 12px",background:"#f0f8ff",borderRadius:8,border:"1px solid #bbdefb"}}>
...
function GlobalView({tasks,trackers,tenders,contractors,people,packages,tags,saveTasks,saveTrackers,tagrules,pkgrules,jumpOwner,clearJump,onNavTender,memory,setMemory,peopleEmails,defaultCC,zones}){
  ...
  var GLOBAL_CATEGORY_ICONS={"Blocking Point":"🔴","Top Management":"👔","Design":"🏗️","Contract":"📄","Procurement":"🚚","Production":"🏭","Quality Tower":"✅","Quality Podium":"✅","Quality External":"✅","RFI":"❓","FCR":"❓","Letter":"✉️","HR":"👥"};
  function buildEmail(){
    var now=new Date();
    ...
    orderedCats2.forEach(function(cat){
      ...
      var icon=GLOBAL_CATEGORY_ICONS[cat]||"📌";
      lines.push(icon+" "+cat.toUpperCase()+" — "+items.length+" item"+(items.length!==1?"s":""));
      lines.push("─".repeat(40));
      items.forEach(function(a){
        var st=STATUS_ICONS[a.status]||"•";
        var isBlocking=(a.tags||[]).includes("Blocking Point");
        var mark=isBlocking?"🔴 ":"    "+st+" ";
        var owner=a.owner?" — "+a.owner.split(",")[0]:"";
        var due="";
        if(a.due){
          var isLate=a.due<today()&&a.status!=="done";
          due=" — due "+fmtDate(a.due)+(isLate?" (⚠️ late)":"");
        }
        var ccs=getAllCCs(a.tags||[],a.package||"",a.owner||"",tagrules||{},pkgrules||{});
        var ccStr=ccs.length>0?" — cc "+ccs.map(function(p){return p.split(",")[0];}).join(", "):"";
...
function ZoneView({tasks,saveTasks,rooms,saveRooms,zones,people,tags,memory,setMemory,peopleEmails,defaultCC}){
  var mem=memory||{};
  ...
  var CATEGORY_ICONS={"Blocking Point":"🔴","Top Management":"👔","Design":"🏗️","Contract":"📄","Procurement":"🚚","Production":"🏭","Quality Tower":"✅","Quality Podium":"✅","Quality External":"✅","RFI":"❓","FCR":"❓","Letter":"✉️","HR":"👥"};
  function buildZoneReport(){
    var NL=String.fromCharCode(10);
    var now=new Date();
    var dateStr=now.toLocaleDateString("en-GB",{day:"2-digit",month:"long",year:"numeric"});
    var subject=curZone+" — Zone Report — "+dateStr;
    var lines=[];

    lines.push("🏗️ "+curZone.toUpperCase()+" ZONE — WEEKLY REPORT");
    lines.push(now.toLocaleDateString("en-GB",{weekday:"long",day:"2-digit",month:"long",year:"numeric"}));
    lines.push("");

    var pending=filteredActions.filter(function(t){return t.status!=="done";}).length;
    var done=filteredActions.filter(function(t){return t.status==="done";}).length;
    var lastWeek=pastWeeks.length>0?pastWeeks[0]:null;
    var relStr="";
    if(lastWeek){
      var relEmoji=lastWeek.ppi>=80?"🟢":lastWeek.ppi>=60?"🟠":"🔴";
      var rangeStr=fmtDate(lastWeek.monday).slice(0,5)+"-"+fmtDate(lastWeek.sunday);
      relStr=" · Last week's reliability: "+lastWeek.ppi+"% "+relEmoji+" ("+rangeStr+")";
    }
    lines.push("👋 "+filteredActions.length+" action"+(filteredActions.length!==1?"s":"")+" tracked · "+done+" done · "+pending+" pending"+relStr);
    lines.push("");
    ...
    orderedCats.forEach(function(cat){
      ...
      items.forEach(function(t){
        ...
        if(t.due){
          ...
          due=" — due "+fmtDate(t.due)+(isLate?" (⚠️ "+workingDaysDiff(t.due,today())+"d late)":"");
        }
        var rooms="";
        if(isBlocking){
    ...
    if(zoneRooms.length>0){
      ...
      if(blockedRoomsList.length===0){
        ...
        blockedRoomsList.forEach(function(r){
          ...
          lines.push("🔴 "+r.name+" — blocked by: "+blockers.map(function(b){return b.text;}).join("; "));
        });
      }
      lines.push("");
    }
...
function App(){
  ...
  useEffect(()=>{
    const load=async()=>{
      try{
        const [t,x,td,ct,p,pk,g,tr,pr,imp,corr,awn,pko,prefs,zn,rm,pkgsub,pemail,dcc]=await Promise.all([
          ...
          cloudStore.get(KEYS.tags),cloudStore.get(KEYS.tagrules),cloudStore.get(KEYS.pkgrules),cloudStore.get(KEYS_IMP),cloudStore.get(KEYS_CORR),cloudStore.get(KEYS_AWN),cloudStore.get(KEYS.pkgowners),cloudStore.get(KEYS_PREFS),cloudStore.get(KEYS.zones),cloudStore.get(KEYS_ROOMS),cloudStore.get(KEYS.pkgsubcontractors),cloudStore.get(KEYS.peopleemails),cloudStore.get(KEYS.defaultcc)
        ]);
        if(t){setTasks(t);}
        else{
  ...
  if(!currentUser)return <UserLogin people={people} onLogin={function(name){setCurrentUser(name);window._currentUser={name:name};}}/>;  return <div className="layout">
    ...
    <div className="main-area">
      <div className="content">
        ...
        {view==="zone"&&<ZoneView tasks={tasks} saveTasks={saveT} rooms={rooms} saveRooms={saveRooms} zones={zones} people={people} tags={tags} memory={memoryFor("zone")} setMemory={function(p){setMemoryFor("zone",p);}} peopleEmails={peopleEmails} defaultCC={defaultCC}/>}
