...
function buildMomHtml(m){
  var rows=(m.participants||[]).map(function(p){
    return '<tr><td>'+esc(p.name)+'</td><td>'+esc(p.initials)+'</td><td>'+esc(p.company)+'</td><td>'+esc(p.position)+'</td></tr>';
  }).join("");
  var body="";
  (m.sections||[]).forEach(function(sec){
    var items=(sec.items||[]).filter(function(it){return (it.notes||"").trim()||(it.actionWith||"").trim();});
    body+='<tr class="sec"><td>'+esc(sec.index)+'</td><td colspan="4"><b>'+esc(sec.title)+'</b></td></tr>';
    if(items.length===0){body+='<tr><td></td><td colspan="4" class="none">—</td></tr>';return;}
    items.forEach(function(it,k){
      var late=it.dueBy&&it.dueBy<today()&&it.status!=="closed";
      body+='<tr><td>'+esc(sec.index+"."+(k+1))+'</td>'+
        '<td>'+esc(it.notes).replace(/\n/g,"<br/>")+'</td>'+
        '<td>'+esc(it.actionWith||"—")+'</td>'+
        '<td'+(late?' style="color:#b3302a;font-weight:700"':'')+'>'+(it.dueBy?esc(fmtDate(it.dueBy)):"—")+'</td>'+
        '<td>'+esc(it.status||"open")+'</td></tr>';
    });
  });
  return '<h1>Minutes of Meeting</h1>'+
    '<div class="sub">'+esc(m.number||"no number")+' · '+esc(m.subcontractor||"no subcontractor")+' · '+esc(fmtDate(m.date))+'</div>'+
    '<table class="hdr"><tr><th>Date</th><td>'+esc(fmtDate(m.date))+'</td></tr>'+
    '<tr><th>MoM Number</th><td>'+esc(m.number)+'</td></tr>'+
    '<tr><th>Project name</th><td>'+esc(m.projectName)+'</td></tr>'+
    '<tr><th>Subcontractor</th><td>'+esc(m.subcontractor)+'</td></tr>'+
    '<tr><th>Package</th><td>'+esc(m.package||"—")+'</td></tr>'+
    '<tr><th>Prepared by</th><td>'+esc(m.preparedBy)+'</td></tr>'+
    '<tr><th>Location</th><td>'+esc(m.location)+'</td></tr>'+
    '<tr><th>Copies to</th><td>'+esc(m.copiesTo)+'</td></tr></table>'+
    '<h2>Participants</h2>'+
    '<table><tr><th>Name</th><th>Initials</th><th>Company</th><th>Position</th></tr>'+
    (rows||'<tr><td colspan="4" class="none">No participant recorded</td></tr>')+'</table>'+
    '<h2>Notes &amp; actions</h2>'+
    '<table><tr><th style="width:52px">No.</th><th>Notes</th><th style="width:110px">Action with</th><th style="width:88px">Due by</th><th style="width:70px">Status</th></tr>'+
    body+'</table>'+
    '<div class="foot">'+esc(momFileName(m))+' · generated '+esc(fmtDate(today()))+'</div>';
}
...
function MomView({moms,saveMoms,tenders,packages,people,contractors,tasks,saveTasks,onNavTender,memory,setMemory}){
  ...
  // ---- one MoM ----
  var ctrNames=[...new Set([].concat((contractors||[]).map(function(c){return c.name;}),(window._ppSubList||[])).filter(Boolean))].sort();
  ...
  function pushToActions(si,ii){
    ...
    var t=newTask({
      text:it.notes.trim(),owner:it.actionWith||"",due:it.dueBy||"",
    ...
    <div className="page-hdr">
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <button className="btn btn-danger" onClick={function(){
          if(!safeConfirm("Delete "+(sel.number||"this MoM")+"?\n\nActions already pushed to the action list are kept."))return;
    ...
    <div className="titleblock" style={{display:"flex",alignItems:"stretch",overflow:"hidden",
      ...
      <div className="tb-main" style={{flex:1,minWidth:0,padding:"12px 16px"}}>
        ...
        <div className="tb-sub" style={{fontSize:12,color:"var(--ink-3,#6f6b62)",marginTop:3}}>
          File name: <span style={{fontFamily:"var(--font-mono)"}}>{momFileName(sel)}</span></div>
      </div>
    </div>
          ...
          <tbody>{(sec.items||[]).map(function(it,ii){
            ...
            return <tr key={it.id}>
              ...
              <td><select value={it.actionWith||""} onChange={function(e){updItem(si,ii,{actionWith:e.target.value});}} style={{fontSize:11,padding:"4px 6px"}}>
                ...
                {ctrNames.map(function(p){return <option key={"c"+p} value={p}>{p}</option>;})}
              </select></td>
