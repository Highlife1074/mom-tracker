...
function SettingsView({tags,saveTags,people,savePeople,packages,savePackages,tagrules,saveTagrules,pkgrules,savePkgrules,apiKey,saveApiKey,improvements,saveImprovements,pkgOwners,savePkgOwners,pkgSubcontractors,savePkgSubcontractors,zones,saveZones,userPrefs,saveUserPrefs,allData,onImport}){
...
function GlobalView({tasks,trackers,tenders,contractors,people,packages,tags,saveTasks,saveTrackers,tagrules,pkgrules,jumpOwner,clearJump,onNavTender,memory,setMemory}){
  var mem=memory||{};
...
function ZoneView({tasks,saveTasks,rooms,saveRooms,zones,people,tags,memory,setMemory}){
  var mem=memory||{};
  ...
  function buildZoneReport(){
    ...
    lines.push("=".repeat(50));
    lines.push("Generated "+now.toLocaleDateString("en-GB")+" at "+now.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"}));
    return{subject:subject,body:lines.join(NL)};
  }
...
function App(){
  ...
  useEffect(()=>{
    const load=async()=>{
      try{
        const [t,x,td,ct,p,pk,g,tr,pr,imp,corr,awn,pko,prefs,zn,rm,pkgsub]=await Promise.all([
          ...
          cloudStore.get(KEYS.tags),cloudStore.get(KEYS.tagrules),cloudStore.get(KEYS.pkgrules),cloudStore.get(KEYS_IMP),cloudStore.get(KEYS_CORR),cloudStore.get(KEYS_AWN),cloudStore.get(KEYS.pkgowners),cloudStore.get(KEYS_PREFS),cloudStore.get(KEYS.zones),cloudStore.get(KEYS_ROOMS),cloudStore.get(KEYS.pkgsubcontractors)
        ]);
        if(t){setTasks(t);}
        else{
  ...
  if(!currentUser)return <UserLogin people={people} onLogin={function(name){setCurrentUser(name);window._currentUser={name:name};}}/>;  return <div className="layout">
    ...
    <div className="main-area">
      <div className="content">
        ...
        {view==="zone"&&<ZoneView tasks={tasks} saveTasks={saveT} rooms={rooms} saveRooms={saveRooms} zones={zones} people={people} tags={tags} memory={memoryFor("zone")} setMemory={function(p){setMemoryFor("zone",p);}}/>}
    ...
    {view==="global"&&<GlobalView tasks={tasks} trackers={trackers} tenders={tenders} contractors={contractors} people={people} packages={packages} tags={tags} saveTasks={saveT} saveTrackers={saveX} tagrules={tagrules} pkgrules={pkgrules} jumpOwner={jumpOwner} clearJump={function(){setJumpOwner(null);}} onNavTender={navToTender} memory={memoryFor("global")} setMemory={function(p){setMemoryFor("global",p);}}/>}
        {view==="settings"&&<SettingsView tags={tags} saveTags={saveTags} people={people} savePeople={savePeople} packages={packages} savePackages={savePackages} tagrules={tagrules} saveTagrules={saveTagrules} pkgrules={pkgrules} savePkgrules={savePkgrules} apiKey={apiKey} saveApiKey={saveApiKey} improvements={improvements} saveImprovements={saveImprovements} pkgOwners={pkgOwners} savePkgOwners={savePkgOwners} pkgSubcontractors={pkgSubcontractors} savePkgSubcontractors={savePkgSubcontractors} zones={zones} saveZones={saveZones} userPrefs={userPrefs} saveUserPrefs={saveUserPrefs} allData={{tasks,trackers,tenders,contractors,people,packages,tags,tagrules,pkgrules,improvements,correspondences,awns,pkgOwners}} onImport={function(d){if(d.tasks)saveT(d.tasks);if(d.trackers)saveX(d.trackers);if(d.tenders)saveTenders(d.tenders);if(d.contractors)saveContractors(d.contractors);if(d.people)savePeople(d.people);if(d.packages)savePackages(d.packages);if(d.tags)saveTags(d.tags);}}/>}
      </div>
