  ...
  return <div className="overlay"><div className="modal" style={{maxWidth:460}}>
    ...
    <div className="modal-body">
      ...
      <div className="fg">
        ...
        {matched&&<div style={{marginTop:5,padding:"5px 8px",background:"#e8f5e9",borderRadius:6,fontSize:11,color:"#2e7d32",fontWeight:600}}>✅ {matched.ctr.name} · {matched.ct.number||"(no contract number)"}</div>}
        {cert.sap&&!matched&&<div style={{marginTop:5,fontSize:11,color:"#f57f17"}}>⚠️ No contract found for this SAP number</div>}
      </div>
      <div style={{display:"flex",gap:10}}>
        <div className="fg" style={{flex:1}}><label>Valuation #</label><input type="text" value={cert.number} onChange={function(e){set("number",e.target.value);}} placeholder="003"/></div>
        <div className="fg" style={{flex:1}}><label>Period (MM/YY)</label><input type="month" value={cert.date||""} onChange={function(e){set("date",e.target.value);}}/></div>
      </div>
      <div className="fg"><label>Amount incl. tax (this period)</label><input type="number" value={cert.amount} onChange={function(e){set("amount",e.target.value);}} placeholder="34249.84"/></div>
    </div>
...
function ImprovementBox({improvements,saveImprovements,currentPage}){
  ...
  return <div className="imp-btn-wrap" style={{position:"fixed",bottom:16,right:276,zIndex:500}}>
    {open&&<div style={{position:"absolute",bottom:44,right:0,width:280,background:"#fff",borderRadius:12,boxShadow:"0 8px 30px rgba(0,0,0,.18)",border:"1.5px solid #e8e6df",padding:14}}>
      ...
      <textarea ref={ref} autoFocus value={text} onChange={function(e){setText(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();submit();}}} placeholder="Describe the improvement..." style={{width:"100%",minHeight:80,padding:"6px 8px",fontSize:12,border:"1.5px solid #e8e6df",borderRadius:7,fontFamily:"inherit",resize:"vertical",outline:"none",boxSizing:"border-box"}}/>
      <div style={{display:"flex",gap:6,marginTop:8}}><button className="btn" style={{flex:1}} onClick={function(){setOpen(false);setText("");}}>Cancel</button><button className="btn btn-gold" style={{flex:1}} onClick={submit}>✓ Add</button></div>
    </div>}
...
function QuickAdd({people,packages,tenders,contractors,trackers,tags,onAdd,improvements,saveImprovements,currentPage}){
  ...
  return <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
    <div style={{padding:"14px 14px 10px",borderBottom:"1.5px solid #e8e6df"}}><div style={{fontFamily:"'DM Serif Display',serif",fontSize:15,marginBottom:2}}>Quick Add</div><div style={{fontSize:11,color:"#bbb"}}>Enter to add · {today().split("-").reverse().join("/")}</div></div>
    <div style={{flex:1,overflowY:"auto",padding:12}} className="sform">
      ...
      <div className="fg"><label>Package</label><select value={pkg} onChange={function(e){setPkg(e.target.value);}} style={{fontFamily:"inherit"}}><option value="">— none —</option>{(packages||[]).map(function(p){return <option key={p} value={p}>{p}</option>;})}</select></div>
      ...
      <div style={{display:"flex",gap:6,marginTop:4}}><button className="btn" onClick={function(){setText("");setDue(today());setOwner("");setPkg("");setSelTags([]);setIsInfo(false);}} style={{flex:"0 0 auto"}}>✕ Cancel</button><button className="btn btn-gold" style={{flex:1,justifyContent:"center"}} onClick={submit}>＋ Add {isInfo?"Info":"Task"}</button></div>
    </div>
  </div>;
}
...
function QuickAddTask({prefill,onAdd,people,tags,label}){
  ...
  return <div style={{marginTop:8,padding:"12px",background:"#f8f9ff",borderRadius:10,border:"1.5px solid #3949ab"}}><textarea value={text} onChange={function(e){setText(e.target.value);}} placeholder="What needs to be done?" style={{width:"100%",fontSize:12}}/><div style={{display:"flex",gap:6,marginTop:10}}><button className="btn btn-sm" onClick={function(){setOpen(false);}}>Cancel</button><button className="btn btn-sm btn-pri" onClick={submit}>Add</button></div></div>;
}
...
function parseLeadDays(str){if(!str)return 0;var s=String(str).toLowerCase().trim();var n=parseFloat(s)||0;if(!n)return 0;if(s.includes("week")||s.includes("wk"))return Math.round(n*6);if(s.includes("month"))return Math.round(n*26);return Math.round(n);}
...
function MaterialsPanel({td,updTd,saveT,tasks,tenders,saveTenders,setSelTender}){
  ...
  return <div className="card" style={{marginBottom:10}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer"}} onClick={function(){setOpen(!open);}}><div style={{fontWeight:700,fontSize:13}}>🏗️ Materials ({mats.length})</div><button className="btn btn-sm" onClick={function(e){e.stopPropagation();updTd("materials",[...mats,{id:uuid(),name:""}]);}}>＋ Material</button></div>{open&&<div style={{marginTop:10}}>{mats.map(function(m,i){return <div key={m.id} style={{marginBottom:5}}><input type="text" value={m.name} onChange={function(e){updMat(i,"name",e.target.value);}} placeholder="Material name" style={{fontSize:12}}/></div>;})}</div>}</div>;
}
...
function TendersView({tenders,saveTenders,packages,people,tasks,saveTasks,contractors,pkgOwners,jumpTender,clearJumpTender,jumpFrom,onBack}){
  ...
  return <div style={{height:"100%",overflowY:"auto",display:"flex",flexDirection:"column"}}>
    <div className="page-hdr" style={{padding:24,paddingBottom:8}}><div><div className="page-title">Tenders</div></div><button className="btn btn-gold" onClick={function(){saveTenders([newTender({title:"New Tender"}),...tenders]);}}>＋ New Tender</button></div>
    ...
    <div style={{flex:1,overflowX:"auto",padding:"0 24px"}}><table className="tbl"><thead><tr><th>Tender</th><th>Package</th><th>ACC/Aconex</th><th>Target Start</th></tr></thead><tbody>{filtered.map(function(td){return <tr key={td.id} style={{cursor:"pointer"}} onClick={function(){setSelTender(td);}}><td style={{fontWeight:600}}>{td.title}</td><td><span className="badge">{td.package}</span></td><td><span className="chip">{(td.steps||{}).acc||"—"}</span></td><td>{fmtDate(td.startOnSite)}</td></tr>;})}</tbody>
      <tfoot><tr style={{background:"#fafaf8",borderTop:"2px solid #e8e6df"}}><td colSpan={4} style={{padding:"8px 12px"}}>
        {(function(){
...
function AwnView({awns,saveAwns}){return <div style={{padding:24}}><div className="page-title">AWN</div></div>;}
...
function App(){
  ...
  const saveT=function(d){setTasks(d);cloudStore.set(KEYS.tasks,d);};
  const saveTenders=function(d){setTenders(d);cloudStore.set(KEYS.tenders,d);};
  const saveContractors=function(d){setContractors(d);cloudStore.set(KEYS.contractors,d);};
  useEffect(function(){
  ...
  return <div className="layout">
    ...
    <div className="main-area"><div className="content">
      ...
      {view==="contractors"&&<ContractorsView contractors={contractors} saveContractors={saveContractors} people={people} packages={packages} tenders={tenders} correspondences={correspondences} saveCorrespondences={function(d){setCorrespondences(d);cloudStore.set(KEYS_CORR,d);}} saveT={saveT}/>}
    </div><aside className="rsidebar open"><QuickAdd people={people} packages={packages} tenders={tenders} contractors={contractors} onAdd={function(t){saveT([t,...tasks]);}} improvements={[]} saveImprovements={function(){}} currentPage={view}/></aside></div>
  </div>;
}
