...
const KEYS={
  ...
  peopleemails:"pp_people_emails", defaultcc:"pp_default_cc"
};
...
function QuickAdd({people,packages,tenders,contractors,trackers,tags,zones,onAdd,improvements,saveImprovements,currentPage}){
  const [text,setText]=useState("");
...
function SettingsView({tags,saveTags,people,savePeople,packages,savePackages,tagrules,saveTagrules,pkgrules,savePkgrules,apiKey,saveApiKey,improvements,saveImprovements,pkgOwners,savePkgOwners,pkgSubcontractors,savePkgSubcontractors,peopleEmails,savePeopleEmails,defaultCC,saveDefaultCC,zones,saveZones,userPrefs,saveUserPrefs,allData,onImport}){
...
function ClientSubmissionsView({tasks,tenders,contractors,packages,people,saveTasks,onNavTender,memory,setMemory}){
  ...
  const [fOwner,setFOwner]=useState(mem.fOwner||"all");
  const [fStage,setFStage]=useState(mem.fStage||"all");
  const [fOverdueOnly,setFOverdueOnly]=useState(mem.fOverdueOnly||false);
  useEffect(function(){if(setMemory)setMemory({fTender:fTender,fPkg:fPkg,fOwner:fOwner,fStage:fStage,fOverdueOnly:fOverdueOnly});},[fTender,fPkg,fOwner,fStage,fOverdueOnly]);
  ...
  var filtered=withClient.filter(function(d){
    if(fTender!=="all"&&d.tenderRef!==fTender)return false;
    if(fPkg!=="all"&&d.package!==fPkg)return false;
    if(fOwner!=="all"&&d.owner!==fOwner)return false;
    if(fStage!=="all"&&d.stage!==fStage)return false;
    if(fOverdueOnly&&!d.overdue)return false;
    return true;
  }).sort(function(a,b){
    if(a.overdue&&!b.overdue)return -1;
    if(!a.overdue&&b.overdue)return 1;
  ...
  var overdueCount=withClient.filter(function(d){return d.overdue;}).length;
  var allOwners=[...new Set(withClient.map(function(d){return d.owner;}).filter(Boolean))].sort();
  var allTenders=[...new Set(withClient.map(function(d){return d.tenderRef;}).filter(Boolean))].map(function(id){return(tenders||[]).find(function(t){return t.id===id;});}).filter(Boolean);

  return <div style={{padding:"16px 20px",overflowY:"auto",flex:1}}>
    <div className="page-hdr">
      <div>
        <div className="page-title">📬 Client Submissions</div>
        <div className="page-sub">Everything currently with the client, awaiting response — RFI, FCR, ACC/ACONEX, MAR, MSS, ITP, WMS, SD, Contract docs</div>
      </div>
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
    ...
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16,alignItems:"center"}}>
      <button className={"fchip"+(fOverdueOnly?" on":"")} onClick={function(){setFOverdueOnly(!fOverdueOnly);}} style={fOverdueOnly?{borderColor:"#c62828",background:"#c62828",color:"#fff"}:{}}>⚠️ Overdue only</button>
      <select value={fStage} onChange={function(e){setFStage(e.target.value);}} style={{padding:"5px 10px",fontSize:12,border:"1px solid #e8e6df",borderRadius:8,fontFamily:"inherit"}}>
        <option value="all">All stages</option>
        {STAGES.map(function(s){return <option key={s.key} value={s.key}>{s.label}</option>;})}
      </select>
      ...
      {(fTender!=="all"||fPkg!=="all"||fOwner!=="all"||fStage!=="all"||fOverdueOnly)&&
        <button className="btn btn-sm" onClick={function(){setFTender("all");setFPkg("all");setFOwner("all");setFStage("all");setFOverdueOnly(false);}}>✕ Reset</button>}
    </div>
...
function ZoneView({tasks,saveTasks,rooms,saveRooms,zones,people,tags,memory,setMemory,peopleEmails,defaultCC,kpis,saveKpis}){
  var mem=memory||{};
  ...
  function buildZoneReport(){
    ...
    // Resolve recipients: owners of the filtered zone actions (To) + defaultCC people (Cc)
...
function App(){
  ...
  useEffect(()=>{
    const load=async()=>{
      try{
        const [t,x,td,ct,p,pk,g,tr,pr,imp,corr,awn,pko,prefs,zn,rm,pkgsub,pemail,dcc,kp]=await Promise.all([
          ...
          cloudStore.get(KEYS.tags),cloudStore.get(KEYS.tagrules),cloudStore.get(KEYS.pkgrules),cloudStore.get(KEYS_IMP),cloudStore.get(KEYS_CORR),cloudStore.get(KEYS_AWN),cloudStore.get(KEYS.pkgowners),cloudStore.get(KEYS_PREFS),cloudStore.get(KEYS.zones),cloudStore.get(KEYS_ROOMS),cloudStore.get(KEYS.pkgsubcontractors),cloudStore.get(KEYS.peopleemails),cloudStore.get(KEYS.defaultcc),cloudStore.get(KEYS_KPIS)
        ]);
        if(t){setTasks(t);}
        else{
  ...
  if(!currentUser)return <UserLogin people={people} onLogin={function(name){setCurrentUser(name);window._currentUser={name:name};}}/>;  return <div className="layout">
    ...
    <div className="main-area">
      <div className="content">
        ...
        {view==="zone"&&<ZoneView tasks={tasks} saveTasks={saveT} rooms={rooms} saveRooms={saveRooms} zones={zones} people={people} tags={tags} memory={memoryFor("zone")} setMemory={function(p){setMemoryFor("zone",p);}} peopleEmails={peopleEmails} defaultCC={defaultCC} kpis={kpis} saveKpis={saveKpis}/>}
        ...
        {view==="documents"&&<DocumentsView tasks={tasks} tenders={tenders} contractors={contractors} packages={packages} people={people} saveTasks={saveT} onNavTender={navToTender} memory={memoryFor("documents")} setMemory={function(p){setMemoryFor("documents",p);}}/> }
    ...
    {view==="global"&&<GlobalView tasks={tasks} trackers={trackers} tenders={tenders} contractors={contractors} people={people} packages={packages} tags={tags} saveTasks={saveT} saveTrackers={saveX} tagrules={tagrules} pkgrules={pkgrules} jumpOwner={jumpOwner} clearJump={function(){setJumpOwner(null);}} onNavTender={navToTender} memory={memoryFor("global")} setMemory={function(p){setMemoryFor("global",p);}} peopleEmails={peopleEmails} defaultCC={defaultCC} zones={zones}/>}
      ...
      <aside className={"rsidebar "+(sidebarOpen?"open":"closed")}>
        ...
        {sidebarOpen&&<QuickAdd people={people} packages={packages} tenders={tenders} contractors={contractors} trackers={trackers} tags={tags} zones={zones} onAdd={addTask} improvements={improvements} saveImprovements={saveImprovements} currentPage={view}/>}
      </aside>
    ...
    {mobileQAOpen&&<div className="overlay" style={{zIndex:600}} onClick={function(e){if(e.target===e.currentTarget)setMobileQAOpen(false);}}><div style={{background:"#fff",borderRadius:"16px 16px 0 0",width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto",position:"absolute",bottom:0}}><QuickAdd people={people} packages={packages} tenders={tenders} contractors={contractors} trackers={trackers} tags={tags} zones={zones} onAdd={function(t){addTask(t);setMobileQAOpen(false);}} improvements={improvements} saveImprovements={saveImprovements} currentPage={view}/></div></div>}
