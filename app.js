...
function newScheduleRow(kind,label){return{id:uuid(),kind:kind||"task",label:label||"",cells:{},afterId:"",lagWeeks:0,roomId:"",progress:"",weekProgress:{},group:"",startWeek:"",endWeek:"",qty:"",unit:""};}
...
function getDur(key){
  ...
  var v=d[key];
  return (v===undefined||v===null||v==="")?DEFAULT_DURATIONS[key]:Number(v);
}
      ...
      {(selTags.includes("RFI")||selTags.includes("FCR"))&&<div className="fg">
        ...
        <div style={{display:"flex",gap:6,padding:"8px",background:"#fff8f0",borderRadius:8,border:"1px solid #fed7aa"}}>
          <div style={{flex:1}}>
            ...
            <input type="date" value={rfiSub||""} onChange={function(e){setRfiSub(e.target.value);if(e.target.value){var d=new Date(e.target.value);d.setDate(d.getDate()+getDur("clientResponse"));setRfiDue(d.toISOString().slice(0,10));}}} style={{padding:"3px 6px",fontSize:11,border:"1px solid #fed7aa",borderRadius:5}}/>
          </div>
          <div style={{flex:1}}>
            ...
            {(localTags.includes("RFI")||localTags.includes("FCR"))&&<div style={{display:"flex",gap:8,padding:"8px",background:"#fff8f0",borderRadius:7,border:"1px solid #fed7aa"}}>
              <div style={{flex:1}}>
                ...
                <input type="date" value={localRfiSub} onChange={function(e){
                  ...
                  if(e.target.value){var d=new Date(e.target.value);d.setDate(d.getDate()+getDur("clientResponse"));var dd=d.toISOString().slice(0,10);setLocalRfiDue(dd);upd("rfiDue",dd);}
  ...
  return <div style={{marginTop:8,padding:"12px",background:"#f8f9ff",borderRadius:10,border:"1.5px solid #3949ab"}}>
    ...
    {(selTags.includes("RFI")||selTags.includes("FCR"))&&<div style={{display:"flex",gap:8,padding:"8px 10px",background:"#fff8f0",borderRadius:8,border:"1px solid #fed7aa",marginTop:8}}>
      <div style={{flex:1}}>
        ...
        <input type="date" value={rfiSub} onChange={function(e){setRfiSub(e.target.value);if(e.target.value){var d=new Date(e.target.value);d.setDate(d.getDate()+getDur("clientResponse"));setRfiDue(d.toISOString().slice(0,10));}}} style={{padding:"4px 7px",fontSize:11,border:"1px solid #fed7aa",borderRadius:5,width:"100%"}}/>
      </div>
      <div style={{flex:1}}>
...
function MaterialsPanel({td,updTd,saveT,tasks,tenders,saveTenders,setSelTender,pkgOwners}){
  ...
  function DocBlock({mat,mi,kind,color,bg}){
    ...
    var due14=subDone?(function(){var d=new Date(subDone);d.setDate(d.getDate()+getDur("clientResponse"));return d.toISOString().slice(0,10);}()):"";
    ...
    return <div style={{display:"flex",gap:16,alignItems:"flex-start"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        {(function(){
          ...
          var sdDue14=td.sdDone?(function(){var d=new Date(td.sdDone);d.setDate(d.getDate()+getDur("clientResponse"));return d.toISOString().slice(0,10);}()):"";
  ...
  return <div>
    ...
    <div className="card" style={{marginBottom:10}}>
      ...
      {[
        ...
        var due14=subDate?(function(){var d=new Date(subDate);d.setDate(d.getDate()+getDur("clientResponse"));return d.toISOString().slice(0,10);}()):"";
...
<div style={{position:"relative",display:"inline-flex",alignItems:"center"}}><input type="number" value={ad.instructionAmount||""} onChange={function(e){updateAdItem(ctr.id,ct.id,"addendums",i,"instructionAmount",e.target.value);}} placeholder="INS Amt" style={{width:110,padding:"3px 28px 3px 6px",fontSize:11}}/><span style={{position:"absolute",right:4,fontSize:9,fontWeight:700,color:"#aaa",pointerEvents:"none"}}>{ct.currency||"EUR"}</span></div>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {["acc","aconex"].map(function(dk){
                ...
                var due14=adDate?(function(){var d=new Date(adDate);d.setDate(d.getDate()+getDur("clientResponse"));return d.toISOString().slice(0,10);}()):"";
      ...
      (t.materials||[]).forEach(function(mat){
        ["mss","mar"].forEach(function(k){
          ...
          if(st==="approved")counts.approved++;
            ...
            var done=mat[k+"Done"]||"";
            if(done){var d=new Date(done);d.setDate(d.getDate()+getDur("clientResponse"));if(d.toISOString().slice(0,10)<todayStr)counts.overdue++;}
          }
        });
      });
      if(t.hasSD){
        ...
        if(st==="approved")counts.approved++;
          ...
          if(t.sdDone){var d2=new Date(t.sdDone);d2.setDate(d2.getDate()+getDur("clientResponse"));if(d2.toISOString().slice(0,10)<todayStr)counts.overdue++;}
        }
      }
    });
...
function buildTrackedDocs(tasks,tenders,contractors){
  ...
  (tasks||[]).forEach(function(t){
    ...
    var due=t.rfiDue||(submitted?(function(){var d=new Date(submitted);d.setDate(d.getDate()+getDur("clientResponse"));return d.toISOString().slice(0,10);}()):"");
  ...
  (tenders||[]).forEach(function(td){
    ...
    if(!subDone)return;
    var due14=(function(){var d=new Date(subDone);d.setDate(d.getDate()+getDur("clientResponse"));return d.toISOString().slice(0,10);}());
  ...
  (tenders||[]).forEach(function(td){
    TENDER_DOC_STEPS.forEach(function(step){
      ...
      var dueDateFromDone=doneDate?(function(){var d=new Date(doneDate);d.setDate(d.getDate()+getDur("clientResponse"));return d.toISOString().slice(0,10);}()):"";
      var dueDateFromTarget=targetDate?(function(){var d=new Date(targetDate);d.setDate(d.getDate()+getDur("clientResponse"));return d.toISOString().slice(0,10);}()):"";
  ...
  (tenders||[]).forEach(function(td){
    (td.materials||[]).forEach(function(mat){
      [{type:"mss",label:"MSS"},{type:"mar",label:"MAR"}].forEach(function(m){
        ...
        var due14=(function(){var d=new Date(done);d.setDate(d.getDate()+getDur("clientResponse"));return d.toISOString().slice(0,10);}());
...
function ScheduleView({curZone,schedules,saveSchedules,tasks,saveTasks,people,tags,zones,rooms,saveRooms,canEdit,roomBlockersOf}){
      ...
      :<div>
        ...
        <div className="sched-noprint" style={{display:"flex",gap:10,alignItems:"center",marginBottom:8,flexWrap:"wrap"}}>
          ...
          <span style={{display:"flex",gap:3,alignItems:"center",paddingRight:6,borderRight:"1px solid #e0ddd6"}}>
            ...
            <button className="btn btn-sm" onClick={function(){
              ...
              var idx=wks.findIndex(function(w){var e=new Date(w);e.setDate(e.getDate()+6);return w<=todayStr&&todayStr<=e.toISOString().slice(0,10);});
        ...
        <div className="sched-wrap" style={{overflow:"auto",maxHeight:"62vh",border:"1px solid #e8e6df",borderRadius:8,background:"#fff"}}>
          <table style={{borderCollapse:"collapse",width:"max-content",minWidth:"100%",fontSize:11}}>
            ...
            <tbody>
              {(sc.rows||[]).length===0&&<tr><td colSpan={wks.length+2} style={{padding:"14px",textAlign:"center",color:"#bbb",fontSize:12}}>No rows yet. Add a category or a task below.</td></tr>}
                ...
                return <tr key={row.id} style={{background:isCat?"#f0ede6":"#fff"}}>
                  <td style={{position:"sticky",left:0,zIndex:1,background:isCat?"#f0ede6":"#fff",padding:"3px 10px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df"}}>
                    <div style={{display:"flex",alignItems:"center",gap:3}}>
                      ...
                      {!isCat&&(function(){
                        var sp=rowSpan(row);
                        if(!sp||sp.rate===null)return null;
                        return <span title={"Quantity "+row.qty+" "+(row.unit||"")+" over "+sp.workingWeeks+" working week"+(sp.workingWeeks!==1?"s":"")}
                          style={{fontSize:9,fontWeight:800,padding:"1px 5px",borderRadius:8,flexShrink:0,background:"#fff8e1",color:"#b45309",cursor:"help"}}>
                          {sp.rate.toLocaleString(undefined,{maximumFractionDigits:1})}{row.unit?" "+row.unit:""}/wk
                        </span>;
                      })()}
                      {!isCat&&(function(){
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        {currentMeeting&&showAttendance&&<div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
            ...
            <input type="date" value={currentMeeting.date||""} onChange={function(e){setMeetingDate(e.target.value);}} style={{padding:"3px 7px",fontSize:11,border:"1px solid #e8e6df",borderRadius:5}}/>
          </div>
...
{subTab==="schedule"&&<ScheduleView curZone={curZone} schedules={schedules} saveSchedules={saveSchedules} tasks={tasks} saveTasks={saveTasks} people={people} tags={tags} zones={zones} rooms={rooms} saveRooms={saveRooms} canEdit={canEditZoneSchedule(zoneOwners,curZone,window._currentUser?window._currentUser.name:"")} roomBlockersOf={roomBlockers}/>}
