  ...
  setTimeout(function(){
    ...
    {(function(){
      ...
      return <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>
        ...
        {totTraite>0&&<div className="card" style={{flex:1,minWidth:120,marginBottom:0,padding:"10px 14px",background:"#f3e5f5"}}>
          <div style={{fontSize:10,color:"#7b1fa2",marginBottom:2}}>Budget Traité</div>
          <div style={{fontSize:18,fontWeight:800,color:"#7b1fa2"}}>{totTraite.toLocaleString()}</div>
          <div style={{fontSize:10,color:"#aaa"}}>{releasedTenders.length} released</div>
        </div>}
        {totBudgetAll>0&&<div className="card" style={{flex:1,minWidth:120,marginBottom:0,padding:"10px 14px",background:totRestant<0?"#fff0f0":"#fff8e1"}}>
          <div style={{fontSize:10,color:totRestant<0?"#c62828":"#f57f17",marginBottom:2}}>Budget Restant à traiter</div>
          <div style={{fontSize:18,fontWeight:800,color:totRestant<0?"#c62828":"#f57f17"}}>{totRestant.toLocaleString()}</div>
          <div style={{fontSize:10,color:"#aaa"}}>{totRestant<0?"⚠️ Dépassement":"Sur "+releasedTenders.length+" released"}</div>
        </div>}
      </div>;
    })()}
    {filtered.length===0
