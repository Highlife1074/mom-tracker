...
function newScheduleRow(kind,label){return{id:uuid(),kind:kind||"task",label:label||"",cells:{},afterId:"",lagWeeks:0,roomId:"",progress:""};}
...
function ScheduleView({curZone,schedules,saveSchedules,tasks,saveTasks,people,tags,zones,rooms,saveRooms}){
                  ...
                  <td style={{position:"sticky",left:0,zIndex:1,background:isCat?"#f0ede6":"#fff",padding:"3px 10px",borderBottom:"1px solid #f5f4f0",borderRight:"1.5px solid #e8e6df"}}>
                    <div style={{display:"flex",alignItems:"center",gap:3}}>
                      ...
                      {!isCat&&(function(){
                        ...
                        var manual=isManualProgress(row);
                        return <span className="sched-noprint" style={{display:"flex",alignItems:"center",flexShrink:0}}>
                          <input type="number" min="0" max="100" value={manual?row.progress:(pg===null?"":pg)}
                            onChange={function(e){updRow(row.id,{progress:e.target.value});}}
                            title={manual?"Manual progress — clear the field to go back to auto":"Auto from actual vs planned — type a value to override"}
                            style={{width:38,padding:"1px 3px",fontSize:9,fontWeight:800,textAlign:"right",borderRadius:5,
                              border:"1px solid "+(manual?"#7b1fa2":"#e8e6df"),
                              background:pg===null?"#fff":pg>=100?"#e8f5e9":pg>0?"#e3f2fd":"#f5f4f0",
                              color:pg===null?"#bbb":pg>=100?"#2e7d32":pg>0?"#1565c0":"#bbb"}}/>
                          <span style={{fontSize:8,color:"#bbb",marginLeft:1}}>%</span>
                        </span>;
                      })()}
...
{subTab==="schedule"&&<ScheduleView curZone={curZone} schedules={schedules} saveSchedules={saveSchedules} tasks={tasks} saveTasks={saveTasks} people={people} tags={tags} zones={zones} rooms={rooms} saveRooms={saveRooms}/>}
